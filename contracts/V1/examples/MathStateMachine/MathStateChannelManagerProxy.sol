...
contract MathStateChannelManagerProxy is AStateChannelManagerProxy {
        ...
        override
    {
        ...
        require(isValid, "MathStateChannelManager: openChannel (openChannel <> signatures) singatures don't match");

        require(channelId != bytes32(0), "MathStateChannelManager: openChannel channelId cannot be 0x0");

        require(!isChannelOpen(channelId), "MathStateChannelManager: openChannel - channel already open");
        for (uint256 i = 0; i < joinChannels.length; i++) {
            require(
                channelId == joinChannels[i].channelId, "MathStateChannelManager: openChannel channelId doesn't match"
            );

            require(
                joinChannels[i].balance.amount > 0, "MathStateChannelManager: openChannel amount must be greater than 0"
            );
