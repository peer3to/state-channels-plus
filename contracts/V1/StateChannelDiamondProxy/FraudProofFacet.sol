...
contract FraudProofFacet is StateChannelCommon {
    ...
    function _handleDisputeInvalidPreviousRecursive(
        bytes memory encodedProof,
        FraudProofVerificationContext memory fraudProofVerificationContext
    ) internal returns (address) {
        DisputeInvalidPreviousRecursiveProof memory disputeInvalidPreviousRecursiveProof =
            abi.decode(encodedProof, (DisputeInvalidPreviousRecursiveProof));
        ...
        uint256 originalDisputeTimestamp = disputeInvalidPreviousRecursiveProof.originalDisputeTimestamp;
        uint256 invalidRecursiveDisputeTimestamp = disputeInvalidPreviousRecursiveProof.invalidRecursiveDisputeTimestamp;
        bytes memory latestStateSnapshot = disputeInvalidPreviousRecursiveProof.latestStateSnapshot;
        bytes memory invalidRecursiveDisputeOutputState =
            disputeInvalidPreviousRecursiveProof.invalidRecursiveDisputeOutputState;
        Block memory originalDisputeLastBlock = abi.decode(
            originalDispute.stateProof.signedBlocks[originalDispute.stateProof.signedBlocks.length - 1].encodedBlock,
            (Block)
        );
        Block memory invalidRecursiveDisputeLastBlock = abi.decode(
            invalidRecursiveDispute.stateProof.signedBlocks[invalidRecursiveDispute.stateProof.signedBlocks.length - 1]
                .encodedBlock,
            (Block)
        );
        ...
        bytes32 recursiveDisputeCommitment =
            keccak256(abi.encode(invalidRecursiveDispute, invalidRecursiveDisputeTimestamp));
        bytes32 originalDisputeCommitment = keccak256(abi.encode(originalDispute, originalDisputeTimestamp));

        (bool isAvailable, bytes32 commitment) =
            getDisputeCommitment(fraudProofVerificationContext.channelId, invalidRecursiveDispute.disputeIndex);
        require(!isAvailable && commitment != recursiveDisputeCommitment, ErrorDisputeCommitmentNotAvailable());

        require(
            invalidRecursiveDispute.previousRecursiveDisputeIndex == type(uint256).max
                || invalidRecursiveDispute.previousRecursiveDisputeIndex != originalDispute.disputeIndex,
        ...
        require(
            !isOriginalDisputeAvailable && originalCommitment != originalDisputeCommitment,
            ErrorDisputeCommitmentNotAvailable()
        );
        require(
            invalidRecursiveDisputeTimestamp > originalDisputeTimestamp + getChallengeTime(),
            ErrorWithinChallengePeriod()
        );
        ...
        require(
            invalidRecursiveDisputeLastBlock.transaction.header.transactionCnt
                > originalDisputeLastBlock.transaction.header.transactionCnt,
        ...
        require(
            afterDisputeParticipants.length < initialParticipants.length, ErrorRecursiveDisputeNotExtendingSlashes()
        );
