// External libraries
import { ethers, ZeroHash, EventLog, Log, TransactionResponse } from "ethers";
...
class StateManager {
    ...
    logger: Logger;
    /**
     * One-shot chain log replay into LocalDiamond before spectate verify (late joiners).
     * Not run on connectToChannel — that path must stay lightweight for live participants.
     */
    private localDiamondChainReplayForSpectateDone = false;
    ...
    /**
     * Replay historical chain logs into LocalDiamond so late joiners (listeners only
     * see future events) match on-chain fork/dispute/outbound state before spectate verify.
     * @returns false if the channel open log is not yet indexable (caller should retry refresh).
     */
    private async catchUpLocalDiamondFromChainLogs(
        channelId: ChannelId
    ): Promise<boolean> {
        const contract = this.stateChannelManagerContract;
        const ld = this.diamondStateMachine.localDiamondContract;
        const channelOpened = await contract.queryFilter(
        ...
        if (channelOpened.length === 0) {
            this.logger.debug(
                "LocalDiamond catch-up: ChannelOpened not found yet; will retry on next refresh"
            );
            return false;
        }
        const fromBlock = Math.min(...channelOpened.map((l) => l.blockNumber));
        ...
        const collect = (logs: Log[], kind: ReplayKind) => {
        ...
        collect(
            await contract.queryFilter(
                contract.filters.StateSnapshotUpdated(channelId),
                fromBlock,
                to
            ),
            "StateSnapshotUpdated"
        ...
        collect(
            await contract.queryFilter(
                contract.filters.BlockCalldataPosted(channelId),
                fromBlock,
                to
            ),
            "BlockCalldataPosted"
        ...
        collect(
            await contract.queryFilter(
                contract.filters.DisputeCommitted(channelId),
                fromBlock,
                to
            ),
            "DisputeCommitted"
        ...
        collect(
            await contract.queryFilter(
                contract.filters.DisputeCommittedWithAuditingData(channelId),
                fromBlock,
                to
            ),
            "DisputeCommittedWithAuditingData"
        ...
        collect(
            await contract.queryFilter(
                contract.filters.ChainSlashed(channelId),
                fromBlock,
                to
            ),
            "ChainSlashed"
        ...
        collect(
            await contract.queryFilter(
                contract.filters.DisputeReducedResultCommitted(channelId),
                fromBlock,
                to
            ),
            "DisputeReducedResultCommitted"
        ...
        collect(
            await contract.queryFilter(
                contract.filters.DisputeKilled(channelId),
                fromBlock,
                to
            ),
            "DisputeKilled"
        ...
        collect(
            await contract.queryFilter(
                contract.filters.WithdrawalsUpdated(channelId),
                fromBlock,
                to
            ),
            "WithdrawalsUpdated"
        ...
        collect(
            await contract.queryFilter(
                contract.filters.ChannelStorageCleared(channelId),
                fromBlock,
                to
            ),
            "ChannelStorageCleared"
        ...
        this.logger.debug(
            `LocalDiamond catch-up: replaying ${entries.length} logs from block ${fromBlock}`
        );

        for (const { kind, log } of entries) {
            const a = log.args;
            switch (kind) {
                case "ChannelOpened":
                    await ld.onChannelOpened(
                        a.channelId,
                        a.stateSnapshot,
                        a.encodedState
                    );
                ...
                case "StateSnapshotUpdated":
                    await ld.onStateSnapshotUpdated(
                        a.channelId,
                        a.stateSnapshot
                    );
                    break;
                case "BlockCalldataPosted":
                    await ld.onBlockCalldataPosted(
                        a.channelId,
                        a.commitmentHash,
                        a.sender,
                        a.signedBlock,
                ...
                case "DisputeCommittedWithAuditingData": {
                    const dispute = Codec.decode(
                        a.disputeConfirmation.signedDispute.encodedDispute,
                        Type.Dispute
                    );
                    await ld.onDisputeCommitted(
                        a.channelId,
                        dispute,
                        Number(a.disputeCreationTimestamp),
                        a.isFinal,
                        Number(a.windowCreationTimestamp)
                    );
                    break;
                }
                case "ChainSlashed":
                    await ld.onOnChainSlashAdded(
                        a.channelId,
                        a.participant,
                        a.timestamp
                    );
                ...
                case "DisputeReducedResultCommitted": {
                    const forkId = a.forkId;
                    const reducedForkId = a.reducedForkId;
                    const windows = await ld.getDisputeWindows(channelId, [
                        forkId
                    ]);
                    const existing = windows[0];
                    if (
                        existing &&
                        existing.reducedResult &&
                        String(existing.reducedResult.forkId) ===
                            String(reducedForkId)
                    ) {
                        break;
                    }
                    await ld.onDisputeReducedResultCommitted(
                        a.channelId,
                        forkId,
                        reducedForkId,
                        a.reductionTimestamp,
                        a.reducer
                    );
                    break;
                }
                case "DisputeKilled":
                    await ld.onDisputeKilled(
                        a.channelId,
                        a.forkId,
                        a.disputer,
                        a.disputeHash
                ...
                case "WithdrawalsUpdated":
                    await ld.onWithdrawalsUpdated(
                        a.channelId,
                        a.totalWithdrawals
                    );
                    break;
                case "ChannelStorageCleared":
                    await ld.onChannelStorageCleared(
                        a.channelId,
                        a.latestInboundMessageBlockHash
                    );
                    break;
                default: {
    ...
    /**
     * Hydrates LocalDiamond from chain logs once per channel lifetime before spectate payload
     * verification (listeners alone miss historical disputes/forks).
     */
    public async ensureLocalDiamondChainHistoryForSpectate(): Promise<void> {
        if (!this.channelId || this.channelId === NULL) return;
        if (this.localDiamondChainReplayForSpectateDone) return;
        const ok = await this.catchUpLocalDiamondFromChainLogs(this.channelId);
        if (ok) {
            this.localDiamondChainReplayForSpectateDone = true;
        }
    }
    ...
    public setChannelId(channelId: ChannelId) {
        ...
        this.localDiamondChainReplayForSpectateDone = false;
    ...
    private async success(
        ...
        // step 9 - potentially change status: OPENED | SYNCED | PENDING_PARTICIPANT → PARTICIPATING
        if (
