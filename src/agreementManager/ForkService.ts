...
import { BlockUtils, EvmUtils, SignatureUtils } from "@/utils";
...
export default class ForkService {
    ...
    public addBlock(
        ...
        this.addAgreement(forkCnt, {
            blockConfirmation,
            encodedState,
            addressesInThreshold: snapShot.participants,
            snapShot
        });
    }
    ...
    *agreementsIterator(
    ...
    getLatestBlockTimestamp(forkCnt: number): number {
        ...
        const latestTimestamp = latestBlock
            ? BlockUtils.getTimestamp(
                  EvmUtils.decodeBlock(latestBlock.encodedBlock)
              )
            : 0;
