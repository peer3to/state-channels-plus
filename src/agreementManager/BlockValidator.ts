...
export default class BlockValidator {
    ...
    isBlockDuplicate(sb: SignedBlockStruct): boolean {
        return (
            this.isBlockInChain(sb) ||
            this.queues.isBlockQueued(EvmUtils.decodeBlock(sb.encodedBlock))
        );
    }
    ...
    check(signed: SignedBlockStruct): AgreementFlag {
        ...
        const signer = BlockUtils.getBlockAuthor(
        ...
        if (existing) {
            if (
                BlockUtils.getBlockAuthor(
                    EvmUtils.decodeBlock(existing.encodedBlock)
                ) === signer
            ) {
