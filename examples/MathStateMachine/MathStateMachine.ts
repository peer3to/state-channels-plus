...
import MathStateChannelManagerJSON from "../../artifacts/contracts/V1/MathStateMachine/MathStateChannelManger.sol/MathStateChannelManager.json";
...
const createJoinChannelTestObject = (
    ...
    let jc: JoinChannelStruct = {
        participant: address,
        channelId: ethers.keccak256("0x2371"),
        amount: 500,
...
const deploy = async () => {
    // let randomSinger = getSigner(
    //     "0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80"
    // );
    let randomSinger = getRandomSigner();
    let deployUtils = new DeployUtils();
    ...
    let mathSmcFactory = new ethers.ContractFactory(
        MathStateChannelManagerJSON.abi,
        MathStateChannelManagerJSON.bytecode,
        randomSinger
    ) as MathStateChannelManager__factory;
    ...
    console.log(
        "Deployed MathStateChannelManager at ",
...
const main = async () => {
    ...
    let mathStateChannelManagerInstance = new ethers.Contract(
        ContractsJSON.MathStateChannelManager.address,
        ContractsJSON.MathStateChannelManager.abi,
        signer
    ) as unknown as MathStateChannelManager;

    //P2P disovery/matchamking (this is not done here - just the end result)
    let joinChannelCommitment = createJoinChannelTestObject(signerAddress);
