...
author's original signature already counts for that signer. A value that recovers to no signer is
not added. A confirmation for an already-stored block that brings no new signer changes nothing and
is not relayed; one that brings new signers stores and relays only the first signature of each new
signer. Dropping a repeated signer's signature is silent: it is not treated as a fault of its
signer or its supplier.
