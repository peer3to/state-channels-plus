...
## Checkpoint base metadata

The settled checkpoint's exact snapshot and state, the authentic encoded checkpoint block whose
result commits it, and its derived block hash persist as base metadata: the successor links to
`keccak256(encodedBlock)`, not to the snapshot hash, so continuation is impossible without the
raw block witness ([history-and-commitments.md §3.2](../protocol-model/history-and-commitments.md#32-hash-linking)).
Out-of-order or duplicate chain events never regress the base; unknown checkpoint data triggers
