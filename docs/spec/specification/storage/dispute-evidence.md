...
replaces an earlier chain-event copy. Storing a signed dispute without confirmations creates an
entry with an empty co-signature set.
...
supply at most one unverified copy per dispute (the first one stored, before any chain event), cannot
grow it afterwards, and a chain-event copy displaces it. The residual risk is that first sync copy:
its co-signatures are not verified and are served onward in later sync payloads until a chain-event
copy replaces it, bounded to one payload per dispute. Off-chain logic reads only the decoded dispute,
never these co-signatures; first-write-wins prevents a later,
