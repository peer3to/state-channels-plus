...
replaces an earlier chain-event copy.
...
after storing the sync copy, which is rare: a sync carries only windows whose kill period has
expired, so their commitment events are already in the past, and the node does not re-fetch past
commitment events for disputes it already stores. For a node that synced a finished window, the first
sync copy therefore normally stays. The residual risk is that copy: its co-signatures are not
