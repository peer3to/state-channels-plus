...
outstanding. A committed pair accepts no further lobby work. An accepted lease (a reservation on the
advertiser, an in-flight commitment on the selector) is an agreement-window liability: from that point
loss of the counterparty's profile is no longer neutral. Before commitment, explicit leave,
...
- Loss of the selected profile's final live transport while a pick is still unanswered is a neutral abort.
  The lease is released without exclusion and matching resumes immediately. After an accepted lease the
  loss is an agreement-window liability on both sides, at each side's own timing: the advertiser keeps
  its reservation bound running and excludes the absent selector when that bound fires; the selector
  whose commitment is already in flight excludes the absent advertiser at once, when the closed
  transport rejects that commitment. Neither exclusion depends on which side's bound fires first. A
  pending cancellation does not cancel the exclusion; it settles `true` after it. Replacing one
  transport while another remains live is not profile loss.
...
resume matching without accepting a late result from the expired attempt. Loss of the counterparty's
profile after an accepted lease does not cancel the punishment: the advertiser excludes at its
reservation bound and the selector excludes when its in-flight commitment is rejected, in either order.

**<a id="req-lobby-8-31be0f"></a>`REQ-LOBBY-8-31BE0F` — Profile-loss recovery.** Loss of a candidate's final live
transport before a pick is accepted releases matching immediately without exclusion; after an accepted
lease the loss is punished under [`REQ-LOBBY-7-BXQ1QA`](lobby-matching.md#req-lobby-7-bxq1qa) at each
side's stated timing. A healthy replacement or
...
ban or blacklist MUST prevent a replacement connection to that peer. Repeated or concurrent observation
of the same topic by one runtime MUST reuse its existing discovery membership, including an unfinished
join, without replacing active connection ownership. Ending that observation also ends a pending join.
