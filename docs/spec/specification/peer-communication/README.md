...
- **Ordering and concurrency.** Frame dispatch follows a fixed verification order
  ([rpc.md](./rpc.md) [`REQ-RPC-6-E60S4J` (Ordered ingress verification)](rpc.md#req-rpc-6-e60s4j)); handlers run outside the execution serialization boundary of
  block progression and hand validated input to the owning system, which enforces its own ordering.
- **Invariants (owned).** [`INV-RPC-1-SJS2T6` (Identity-bound dispatch)](rpc.md#inv-rpc-1-sjs2t6), [`REQ-RPC-1-FF89Z0` (Typed wire contract)](rpc.md#req-rpc-1-ff89z0)–[`REQ-RPC-8-44XECF` (Compatibility before protected calls)](rpc.md#req-rpc-8-44xecf) ([rpc.md](./rpc.md)), plus the
...
- **Verification evidence.** The requirement matrix in [rpc.md](./rpc.md); the ingress edge into
  block progression is proven under [`REQ-IX-1-WTJ0D1` (Peer block ingress)](../interactions.md#req-ix-1-wtj0d1).
...
([`REQ-IX-1-WTJ0D1` (Peer block ingress)](../interactions.md#req-ix-1-wtj0d1)); carrier for the join-authorization and synchronization
flows of settlement ([`REQ-IX-3-H8WCVY` (Inbound inclusion and join flow)](../interactions.md#req-ix-3-h8wcvy)). Gossip rate limiting is open —
[`OQ-6-4JPNE5` (P2P gossip rate limiting)](../open-questions.md#oq-6-4jpne5).
