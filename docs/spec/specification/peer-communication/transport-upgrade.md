...
bootstrap fallback. The suppression a peer's bootstrap handle carries MUST be derived from two
independent facts held about that peer — it is excluded for a fault, and its fallback is suppressed in
favour of a direct transport — and exclusion MUST always win, so releasing the upgrade preference
never restores an excluded peer. Explicit policy release MUST inspect the full live transport set and
clear identity exclusion without releasing the fallback until no live direct transport remains.
Explicit identity exclusion MUST remain in force across every transport change. A late bootstrap
connection that reaches authentication MUST still be refused while the direct transport is healthy or
the identity is excluded. After current-direct retirement, a non-excluded bootstrap connection may
