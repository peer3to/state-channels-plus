...
> bootstrap transport to a direct transport, the session-continuity rules of the migration, and the
> bootstrap connectivity lifecycle those sessions rest on — fallback availability, relay retry, and
> which discovery keys a node observes. Shared communication rules: [rpc.md](./rpc.md).
...
[`REQ-UPG-5-YQV7MJ`](transport-upgrade.md#req-upg-5-yqv7mj),
[`REQ-UPG-6-BC60XD`](transport-upgrade.md#req-upg-6-bc60xd), and
[`REQ-UPG-7-KQPXRE`](transport-upgrade.md#req-upg-7-kqpxre) below. These behaviors are observable
...
- **Fallback availability.** A healthy current direct transport keeps its authenticated bootstrap
  transport unavailable. During explicit policy release, any live direct transport, including a
  non-preferred transport retained during grace, keeps the fallback ban. Explicit exclusion always wins.
...
- **Discovery membership and disconnection.** A node knows the full set of discovery keys it is
  currently observing. Ending participation — the channel closing, or the integrator's explicit
  disconnect-from-peers operation — stops observing every one of them before the transports close,
  because closing a transport to a peer that still shares an observed key only pauses it: discovery
  hands the peer straight back. Stopping observation is neutral bookkeeping, not a penalty, and a
  later channel connection re-observes its key so ordinary reconnection resumes.
...
Before retirement, a response arriving on a transport for the same authenticated peer may settle a
pending request. Retiring the original request transport rejects that request even when another
same-peer transport exists. Request handlers send responses by authenticated peer address when
available, so the normal address router may choose the current transport without a separate rebinding
protocol. A foreign peer still cannot settle the request.
...
**<a id="req-upg-4-m2xdba"></a>`REQ-UPG-4-M2XDBA` — Fallback ban and explicit exclusion.** A healthy current direct transport MUST suppress its
bootstrap fallback. Explicit policy release MUST inspect the full live transport set and clear identity
exclusion without releasing the fallback until no live direct transport remains. Explicit
...
**<a id="req-upg-7-kqpxre"></a>`REQ-UPG-7-KQPXRE` — Ending participation stops discovery observation before closing transports.**
A node MUST know the complete set of discovery keys it is currently observing. Ending participation —
the selected channel closing, or the integrator's explicit disconnect-from-peers operation — MUST stop
observing every key in that set _before_ the corresponding transports are closed, and MUST then close
them. Ordering is normative: a transport closed while its peer still shares an observed key is
rediscovered and reconnected, so a close in the other order is a pause, not a disconnection. While no
key is observed, no previously reached peer may be re-dialed and none may be admitted through
discovery. Replacing a route the node already accepted is not a discovery admission: while a peer
still holds another live authenticated transport, a further transport for that same identity MUST
stay admissible, so an ordinary route replacement is not refused by the observation state. Stopping
observation MUST NOT exclude, penalize, or suspend any identity, and MUST NOT be
durable against a later intent to connect: a subsequent channel connection re-observes its own key and
ordinary reconnection resumes through it.

Observation begins when the node starts observing a key, not when discovery confirms it: starting is
already what makes the node reachable on that key, so a peer whose handshake completes while the
start is still in progress MUST be admitted. A start that fails MUST leave the key unobserved.

Some keys, transports, and suspensions belong to a subsystem that owns their whole lifecycle rather
than to the node's ordinary connection set — a rendezvous lobby is the example. The disconnect-from-peers
operation MUST end participation through each such owner first, so its topic, its transports, and the
suspensions it placed all end with the operation; leaving that owner's key from outside it would strand
its open transports and its unliftable suspensions, and leave its pending operation unsettled.
...
trade-off is reduced observability of abuse, which the rate limiter must compensate for. Discovery
membership carries its own small surface: while a key is observed, any peer that shares it can force
repeated dial-and-handshake work, so ending participation must stop observing before closing
([`REQ-UPG-7-KQPXRE`](transport-upgrade.md#req-upg-7-kqpxre)) — otherwise a node cannot actually leave.
The residual is that leaving is all-or-nothing at the key level: it is not a per-identity control, and
refusing one identity while staying in the rendezvous is the exclusion/suspension decision of
[`REQ-AUTH-4-JWCF71`](handshake.md#req-auth-4-jwcf71), not of this document.
...
| <a id="req-upg-4-m2xdba.t1"></a>`REQ-UPG-4-M2XDBA.T1` | [`REQ-UPG-4-M2XDBA`](transport-upgrade.md#req-upg-4-m2xdba) | Replace bootstrap and direct transports, close stale and current direct transports, explicitly exclude and release the identity, and vary the preferred transport during direct-transport overlap.                                                                                                                                                                                                                        | Bootstrap admission follows the full live direct-transport set; policy release clears exclusion but preserves the fallback ban during overlap; fallback becomes usable only after the last direct transport retires.                                                                                                                                                                                                                                                                  | <a id="req-upg-4-m2xdba.t1.p1"></a>`REQ-UPG-4-M2XDBA.T1.P1` — bootstrap-to-direct ban; <a id="req-upg-4-m2xdba.t1.p2"></a>`REQ-UPG-4-M2XDBA.T1.P2` — stale direct close remains banned; <a id="req-upg-4-m2xdba.t1.p3"></a>`REQ-UPG-4-M2XDBA.T1.P3` — current direct close releases fallback; <a id="req-upg-4-m2xdba.t1.p4"></a>`REQ-UPG-4-M2XDBA.T1.P4` — explicit exclusion remains banned; <a id="req-upg-4-m2xdba.t1.p5"></a>`REQ-UPG-4-M2XDBA.T1.P5` — unauthenticated-profile explicit ban; <a id="req-upg-4-m2xdba.t1.p6"></a>`REQ-UPG-4-M2XDBA.T1.P6` — ordinary unauthenticated-profile close does not ban; <a id="req-upg-4-m2xdba.t1.p7"></a>`REQ-UPG-4-M2XDBA.T1.P7` — authenticated bootstrap attempt is refused while direct transport is healthy; <a id="req-upg-4-m2xdba.t1.p8"></a>`REQ-UPG-4-M2XDBA.T1.P8` — current-direct close permits an authenticated usable bootstrap fallback; <a id="req-upg-4-m2xdba.t1.p9"></a>`REQ-UPG-4-M2XDBA.T1.P9` — excluded identity's authenticated bootstrap attempt is banned and refused; <a id="req-upg-4-m2xdba.t1.p10"></a>`REQ-UPG-4-M2XDBA.T1.P10` — policy release with selected direct transport preserves fallback ban; <a id="req-upg-4-m2xdba.t1.p11"></a>`REQ-UPG-4-M2XDBA.T1.P11` — policy release with selected bootstrap and non-preferred live direct transport preserves fallback ban; <a id="req-upg-4-m2xdba.t1.p12"></a>`REQ-UPG-4-M2XDBA.T1.P12` — policy release with selected bootstrap and no live direct transport releases fallback. |
...
| <a id="req-upg-7-kqpxre.t1"></a>`REQ-UPG-7-KQPXRE.T1` | [`REQ-UPG-7-KQPXRE`](transport-upgrade.md#req-upg-7-kqpxre) | Observe several discovery keys with live peers, then close the channel and, in a separate run, invoke the public disconnect-from-peers operation; record the order of leave and close, watch for redials for longer than the discovery redial interval, then connect to a channel again; separately, complete a peer handshake while an observation start is still in progress, and run one observation start that fails. | Every observed key is left before its transports close; the observed set becomes empty; no peer is re-dialed or admitted afterwards and no identity is excluded, penalized, or suspended; a later channel connection re-observes its key and the peers reconnect; a handshake completing while an observation start is in progress is admitted and unpenalized, with the key reported observed once the start completes, and a failed start reports its failure and observes nothing. | <a id="req-upg-7-kqpxre.t1.p1"></a>`REQ-UPG-7-KQPXRE.T1.P1` — channel close stops observing every discovery key before closing transports; <a id="req-upg-7-kqpxre.t1.p2"></a>`REQ-UPG-7-KQPXRE.T1.P2` — disconnect-from-peers stops observing every key and nobody re-dials; <a id="req-upg-7-kqpxre.t1.p3"></a>`REQ-UPG-7-KQPXRE.T1.P3` — a later channel connect re-observes and reconnects. <a id="req-upg-7-kqpxre.t1.p4"></a>`REQ-UPG-7-KQPXRE.T1.P4` — while no key is observed, a replacement transport for a peer that still holds a live admitted transport is admitted, and one for an unreached identity is refused; <a id="req-upg-7-kqpxre.t1.p5"></a>`REQ-UPG-7-KQPXRE.T1.P5` — disconnect-from-peers during an active rendezvous session settles it, closes its own transports, and lifts the suspensions it placed; <a id="req-upg-7-kqpxre.t1.p6"></a>`REQ-UPG-7-KQPXRE.T1.P6` — a peer whose handshake completes while the key is still being observed is admitted, unpenalized, and the key is reported as observed once the start completes; <a id="req-upg-7-kqpxre.t1.p7"></a>`REQ-UPG-7-KQPXRE.T1.P7` — a failed start reports its failure and leaves the key unobserved.                                                                                                                                                                                                                                                                                                                     |
