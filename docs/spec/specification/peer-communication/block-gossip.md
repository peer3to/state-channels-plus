...
4. The pipeline's verdict maps back to a communication-layer consequence: acceptable knowledge
   ...
   attributable to the confirmation terminates and excludes every peer the confirmation is
   attributed to — its suppliers and the block's declared author; non-attributable junk is dropped
...
| Attributable objective violation (per pipeline verdict)        | Terminate and exclude the attributed suppliers and the block's author.    |
...
pipeline classifies as acceptable knowledge, and never forgives a violation the pipeline attributes to
the sender - and exclusion follows the pipeline's full attribution set, the suppliers and the
block's author, not the delivering sender alone.
