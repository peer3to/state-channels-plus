...
First joins and top-ups construct `deadlineTimestamp` from chain time plus the standard 120-second join
deadline at execution. No public lobby, connect, worker, or harness option supplies it. A supplied full
balance preserves `amount` and `data`; omitted requested membership uses the simple default. Participants
validate remote balances with the state machine's neutral zero value and existing lesser-than operation
before signing.

After exact-channel sync, a first join waits up to two agreement windows for every threshold participant to
be directly reachable, then uses the normal per-request signature deadline. Missing reachability or a failed
signature leaves the caller uncommitted, so it aborts and disposes. Receipt-confirmed
`PENDING_PARTICIPANT` is already a successful membership result; later cooperative block inclusion or forced
inclusion is separate. A pending or participating call with no balance sends no transaction. A supplied
balance sends exactly one top-up and waits for its receipt.
...
**<a id="inv-membership-pending-1-2h1t75"></a>`INV-MEMBERSHIP-PENDING-1-2H1T75` — Submitted joins are locally
pending before acknowledgement.** A first join MUST move the local runtime from `SYNCED` to
`PENDING_PARTICIPANT` before invoking transaction submission or crossing any asynchronous boundary that may
send it. This protects a potentially funded in-flight transaction from cleanup that applies only to an
uncommitted observer. A failure restores `SYNCED` and clears force-join state only when the outcome proves
that no commitment exists. An uncertain submission keeps local pending protection while chain state is
reconciled. Force-join escalation MUST wait until the signer is present in the authoritative on-chain
participant union and the ordinary dispute window is usable; an absent signer, an expired window, or an
unavailable authoritative read defers the check to a later relevant trigger without a duplicate submission.
Once those gates pass, escalation MUST NOT start before
`joinSubmissionHeight + current participant count + 1`; that exact height is the first eligible block.
...
| ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <a id="inv-membership-pending-1-2h1t75.t1"></a>`INV-MEMBERSHIP-PENDING-1-2H1T75.T1` | [`INV-MEMBERSHIP-PENDING-1-2H1T75`](join-authorization.md#inv-membership-pending-1-2h1t75) | Hold a first join before contract invocation and at receipt; inject a fault while pending; produce proven and uncertain failures; reach force-join height before and after authoritative membership and dispute-window eligibility. | Local pending protection starts before contract invocation. The fault never aborts the pending runtime. Proven failure restores `SYNCED`; uncertain outcome preserves pending. Force join waits for on-chain membership and a usable window, does not submit one block early, and submits once at the exact trigger height. | <a id="inv-membership-pending-1-2h1t75.t1.p1"></a>`INV-MEMBERSHIP-PENDING-1-2H1T75.T1.P1` — pending fault followed by transaction failure restores `SYNCED` without abort; <a id="inv-membership-pending-1-2h1t75.t1.p2"></a>`INV-MEMBERSHIP-PENDING-1-2H1T75.T1.P2` — pending fault followed by transaction success yields on-chain pending membership and the inbound join message without abort; <a id="inv-membership-pending-1-2h1t75.t1.p3"></a>`INV-MEMBERSHIP-PENDING-1-2H1T75.T1.P3` — status is pending before contract invocation; <a id="inv-membership-pending-1-2h1t75.t1.p4"></a>`INV-MEMBERSHIP-PENDING-1-2H1T75.T1.P4` — an uncertain submission preserves pending status and its force-join marker; <a id="inv-membership-pending-1-2h1t75.t1.p5"></a>`INV-MEMBERSHIP-PENDING-1-2H1T75.T1.P5` — absent on-chain membership and an expired window each defer escalation, then later eligibility produces one dispute submission; <a id="inv-membership-pending-1-2h1t75.t1.p6"></a>`INV-MEMBERSHIP-PENDING-1-2H1T75.T1.P6` — force join does not submit one block early and submits exactly once at `joinSubmissionHeight + current participant count + 1`. |
