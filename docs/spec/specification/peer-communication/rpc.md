...
1. **Bound the frame** by its UTF-8 byte length against the maximum frame size _before_ any
   parsing; an oversized frame terminates the session without parse work.
...
3. **Verify the envelope**: the frame must identify a service, a method, an argument list, and — for
   ...
   selects request delivery, including an empty string. Undecodable frames terminate the session.
...
have its defined consequence, and a probe against a gated service MUST NOT reveal whether a method exists.
