...
Every consequence is one of exactly two outcomes, and each stage states which one it applies:
**close with reconnect allowed** (the transport goes, the identity keeps its standing) or **close and
exclude** (the identity is refused on reconnect). No close inherits another caller's outcome, and an
expected or upgrade-driven close is always the first kind.

An authenticated sender that violates a frame-size or envelope contract is excluded as well as
disconnected, so discovery cannot immediately recreate the rejected session. Before identity is known,
the same wire fault can only terminate its transport. An unnamed service or endpoint is a clean
refusal rather than a fault, because it is exactly what a version skew between two deployments looks
like on the wire ([`REQ-RPC-8-44XECF`](rpc.md#req-rpc-8-44xecf)). Transport loss, response-send
failure, timeout, and an unclassified local dispatch exception are likewise disconnect-only because
they do not prove peer misconduct.
...
3. **Verify the envelope**: the frame must identify a service, a method, an argument list, and — for
   ...
   consequence.
4. **Resolve the service.** An unknown service is refused cleanly: the frame is rejected and the
   transport closes with reconnect allowed, without exclusion.
...
6. **Resolve the method.** Resolve only a declared endpoint from the methods object's application-owned
   surface, stopping before both the RPC methods base and the language object base. An unknown,
   base-owned, accessor, or non-function method is refused the same way as an unknown service: the
   transport closes with reconnect allowed, without exclusion.
...
and an incompatible peer MUST be refused cleanly (a declared refusal, not an escalating penalty). With
no negotiation scheme in place, the only observable symptom is a call naming a service or endpoint the
receiver does not have, so that call takes the clean refusal rather than an exclusion.
...
have its defined consequence, and every consequence MUST state one of exactly two outcomes: close with
reconnect allowed, or close and exclude the identity. No close may inherit an outcome from another
caller, and expected or upgrade-driven closes MUST always be the reconnect-allowed outcome.
Authenticated frame-size and envelope violations MUST disconnect and exclude the sender. An unnamed
service or endpoint MUST be refused cleanly with reconnect allowed, because it is indistinguishable
from deployment version skew ([`REQ-RPC-8-44XECF`](rpc.md#req-rpc-8-44xecf)). Unknown identities and
failures that do not prove peer misconduct MUST only lose the affected transport. A probe against a
gated service MUST NOT reveal whether a method exists.
...
handshake domain. Until a negotiation scheme exists, a call naming a service or endpoint this
deployment does not have is the observable form of that incompatibility and MUST take the same clean
refusal. (Scheme unresolved: [`OQ-34-FY08V2` (RPC boundary decisions)](../open-questions.md#oq-34-fy08v2).)
