...
4. **Responder.** A duplicate request for a fork already answered to this peer is a protocol
   ...
   peer (its own defense against later false tolerance claims). Writing that record a second time is
   a different case: it reads only the responder's own bookkeeping and is reached only when two
   requests race past the duplicate check above, so it closes the transport with reconnect allowed
   and excludes nobody.
...
| Repeated write of the responder's own acknowledgment record for a fork it already answered to this peer | Close with reconnect allowed. The record is this node's own; a race past the duplicate check proves nothing about the peer.                        |
...
disputed fork toward each peer, and a peer answers each fork at most once; a duplicate request or a
duplicate answer observed in the peer's record is a protocol violation and excludes that peer. The
responder's own acknowledgment record MUST be idempotent and MUST NOT be a penalty source: a repeated
write, which only a race past the duplicate check can reach, closes the transport with reconnect
allowed and excludes nobody.
...
duplicate rule. The responder's own record is deliberately outside the penalty surface: only what the
peer did — its request, its answer, its silence — can cost it standing.
...
| ------------------------------------------------------- | ------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <a id="req-dack-1-eseggg.t1"></a>`REQ-DACK-1-ESEGGG.T1` | [`REQ-DACK-1-ESEGGG`](dispute-acknowledgment.md#req-dack-1-eseggg) | Run rounds normally, repeat requests for the same fork, answer a fork twice, and race two requests past the duplicate check into the responder's own record. | Single round per fork per pair; duplicates observed in the peer's record are violations with their consequences, while a repeated write of this node's own record only closes the transport. | <a id="req-dack-1-eseggg.t1.p1"></a>`REQ-DACK-1-ESEGGG.T1.P1` — normal round; <a id="req-dack-1-eseggg.t1.p2"></a>`REQ-DACK-1-ESEGGG.T1.P2` — duplicate request violation; <a id="req-dack-1-eseggg.t1.p3"></a>`REQ-DACK-1-ESEGGG.T1.P3` — duplicate answer violation; <a id="req-dack-1-eseggg.t1.p4"></a>`REQ-DACK-1-ESEGGG.T1.P4` — distinct forks are distinct rounds; <a id="req-dack-1-eseggg.t1.p5"></a>`REQ-DACK-1-ESEGGG.T1.P5` — a repeated write of the responder's own acknowledgment record closes the transport with reconnect allowed and does not exclude the peer. |
