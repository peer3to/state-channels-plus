...
**<a id="req-auth-4-jwcf71"></a>`REQ-AUTH-4-JWCF71` — Penalty requires proof; timing failures withdraw instead.** A penalty is an
identity-scoped exclusion that outlives the connection, and it applies only to proven identities. An
authenticated transport retains that proof during later re-handshake validation, so malformed or
invalid-signature traffic on it penalizes the known sender. A timing failure is never a penalty: an
out-of-window request or response reading, an exchange beyond the round-trip bound, a missing
acknowledgement, and a refused or unanswered exchange MUST instead withdraw the peer for the
remainder of the node's session — the exchange closes, the node stops redialling that peer for the
session, and no identity-scoped penalty is recorded. A withdrawn peer MUST be refused at admission
exactly as a penalized identity is. A failure on an unknown transport, including silence before
proof, still never penalizes an identity. A node that refuses an exchange MUST deliver that refusal
before it closes, so the counterparty withdraws symmetrically instead of retrying.
...
excluding an identity that never proved itself ([`REQ-AUTH-4-JWCF71`](handshake.md#req-auth-4-jwcf71)) — a timing
failure withdraws the peer for the session instead, which costs the node nothing it can be tricked
into spending on an unproven identity. Completion grants no channel
...
| <a id="req-auth-4-jwcf71.t1"></a>`REQ-AUTH-4-JWCF71.T1` | [`REQ-AUTH-4-JWCF71`](handshake.md#req-auth-4-jwcf71) | Fail exchanges before proof, time out or refuse them in each direction, and repeat malformed, invalid-signature, or silent exchanges on an already authenticated transport.                                                        | Timing failures withdraw the peer for the session with no identity penalty; malformed or invalid-signature faults attributable to an already proven transport penalize that identity.                                    | <a id="req-auth-4-jwcf71.t1.p1"></a>`REQ-AUTH-4-JWCF71.T1.P1` — timeout before proof withdraws without penalty; <a id="req-auth-4-jwcf71.t1.p2"></a>`REQ-AUTH-4-JWCF71.T1.P2` — verified-but-silent peer withdrawn without penalty; <a id="req-auth-4-jwcf71.t1.p3"></a>`REQ-AUTH-4-JWCF71.T1.P3` — penalized identity refused at verification; <a id="req-auth-4-jwcf71.t1.p4"></a>`REQ-AUTH-4-JWCF71.T1.P4` — out-of-window reading withdraws without penalty; <a id="req-auth-4-jwcf71.t1.p5"></a>`REQ-AUTH-4-JWCF71.T1.P5` — refused exchange withdraws the refusing side's counterparty; <a id="req-auth-4-jwcf71.t1.p6"></a>`REQ-AUTH-4-JWCF71.T1.P6` — refusal delivered before the close; <a id="req-auth-4-jwcf71.t1.p7"></a>`REQ-AUTH-4-JWCF71.T1.P7` — withdrawn peer refused at admission; <a id="req-auth-4-jwcf71.t1.p8"></a>`REQ-AUTH-4-JWCF71.T1.P8` — closed exchange withdraws nothing.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
