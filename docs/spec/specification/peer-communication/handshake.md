...
An out-of-window time is an environment fault rather than misconduct, so it MUST NOT record a
verdict: each such failure only closes the connection and counts against one stated per-peer bound
for the session, and the failure that reaches the bound excludes the identity for the remainder of
the session instead. A failure on an unknown transport, including silence before proof, never
penalizes an identity.
...
| <a id="req-auth-4-jwcf71.t1"></a>`REQ-AUTH-4-JWCF71.T1` | [`REQ-AUTH-4-JWCF71`](handshake.md#req-auth-4-jwcf71) | Fail exchanges before proof and repeat malformed, timing-invalid, invalid-signature, or silent exchanges on an already authenticated transport, taking the timing-invalid case both below and up to its per-peer bound.                                                                                     | Pre-proof failures drop the transport with no identity penalty; malformed and invalid-signature faults on an already proven transport exclude that identity with a recorded verdict; timing faults only close until their bound, then exclude for the session without one.                                    | <a id="req-auth-4-jwcf71.t1.p1"></a>`REQ-AUTH-4-JWCF71.T1.P1` — timeout before proof; <a id="req-auth-4-jwcf71.t1.p2"></a>`REQ-AUTH-4-JWCF71.T1.P2` — verified-but-silent peer excluded; <a id="req-auth-4-jwcf71.t1.p3"></a>`REQ-AUTH-4-JWCF71.T1.P3` — excluded identity refused at verification; <a id="req-auth-4-jwcf71.t1.p4"></a>`REQ-AUTH-4-JWCF71.T1.P4` — out-of-window exchanges below the bound leave the proven identity reconnectable and unpenalized; <a id="req-auth-4-jwcf71.t1.p5"></a>`REQ-AUTH-4-JWCF71.T1.P5` — the out-of-window exchange reaching the bound excludes the proven identity for the session without recording a verdict.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
