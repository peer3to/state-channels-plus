...
4. **Initiator verification.** The initiator independently bounds the exchange round trip and the
   ...
   identities it has excluded or whose reconnects it has locally suspended, stores the provisional
   proof against the transport, and sends an acknowledgement. The public transport address remains
   absent until mutual completion.
...
times out: a peer already verified but never acknowledging _while its transport stays open_ is
excluded by identity; an unverified peer is merely dropped — there is no proven identity to
penalize. Silence is attributable only while the transport that owed the acknowledgement is still
open. Once that transport has closed, the missing acknowledgement is explained by the close and
proves nothing about the peer, so the exchange is dropped with no identity penalty even when the
peer was already verified. Refusal is one of the ways that transport closes: a peer that refuses
this exchange — because it excluded this identity, or because it locally suspended its reconnects —
closes without acknowledging, and the refused side must not turn that refusal into an exclusion of
its own.
...
| Failure                                                                      | Outcome                                                                                                                                                                             |
| ---------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Malformed challenge or non-finite/out-of-window time at the responder        | Reject before signing; exclude only if this transport already has an authenticated identity.                                                                                        |
| Response timeout                                                             | Drop the transport without identity penalty.                                                                                                                                        |
| Round-trip or skew bound exceeded, or undecodable signature                  | Reject; exclude only if this transport already has an authenticated identity.                                                                                                       |
| Signature verifying to an excluded identity                                  | Refuse with no new penalty; the identity remains excluded, and the arriving transport also loses discovery reachability so a rotated discovery handle cannot bypass the exclusion.  |
| Signature verifying to an identity whose reconnects are locally suspended    | Session refused at admission; no exclusion recorded and no discovery reachability change at all, so the identity keeps arriving and keeps being refused until the suspension lifts. |
| Duplicate acknowledgement                                                    | Protocol violation: terminate and exclude.                                                                                                                                          |
| Verified peer never acknowledges within the window on a still-open transport | Exclude by proven identity.                                                                                                                                                         |
| Transport closes before the acknowledgement arrives                          | Drop with no identity penalty, whether or not the peer was already verified; the close explains the silence. A refusal that closes the transport is this case.                      |
...
An authenticated transport retains that proof during later re-handshake validation, so malformed,
out-of-window, or invalid-signature traffic on it excludes the known sender. A failure on an unknown
transport, including silence before proof, never penalizes an identity. Silence is attributable only
while the transport that owed the message is still open: a missing acknowledgement on a transport
that has already closed MUST NOT penalize the identity, even one this node has already verified,
because the close — not the peer's choice — explains the silence. This is what keeps a peer's own
refusal from being converted into an exclusion by the peer it refused: both refusals below close the
transport without acknowledging, so a node that penalized that silence would turn the refuser's
one-sided standing into a mutual permanent exclusion. Two refusals at verification carry no new
penalty and MUST be distinguished from each other:

- **Already-excluded identity.** An identity this node has excluded MUST be refused at verification
  and MUST NOT establish a session. Because a peer may present the same identity over a fresh
  discovery handle, the exclusion MUST also suppress the discovery reachability of the transport the
  refused attempt arrived on, so the identity is not rediscovered through that handle. This adds no
  penalty beyond the exclusion that already exists.
- **Locally suspended reconnects.** An identity whose reconnects this node has locally suspended MUST
  also be refused at verification. The refusal MUST hold at every boundary a session can be
  established through, including a suspension placed after the identity's response was already
  accepted but before the exchange completes; no such attempt may become a live session. The
  suspension acts at admission only: it MUST NOT change the discovery reachability of the identity or
  of any handle its attempts arrive on, so the suspended peer keeps being discovered and keeps
  repeating the dial/prove/refuse cycle for as long as the suspension lasts. That repetition is the
  price of keeping the suspension liftable without telling anyone: because no attempt was ever
  suppressed, the first attempt after the suspension lifts is admitted, and no node has to dial a
  peer back on the strength of a close it observed. Suspension is not an exclusion: it records no
  fault against the identity, produces no portable evidence, and MUST lift when the local condition
  that caused it ends (for example when the node stops observing the rendezvous the suspension
  belonged to). Lifting a suspension MUST NOT lift an exclusion, and an exclusion MUST continue to
  refuse the identity for as long as it stands.
...
excluding an identity that never proved itself ([`REQ-AUTH-4-JWCF71`](handshake.md#req-auth-4-jwcf71)). Two refusal paths belong to that
last defense. An excluded identity that reappears is refused at verification, and because a peer can
rotate the discovery handle it announces, the exclusion also suppresses that handle — otherwise
exclusion degrades into a pause the excluded peer can undo by rediscovering itself. A locally
suspended identity is refused too, but suspension is deliberately not a penalty: it carries no fault,
no portable evidence, and no durability beyond its local cause, so a node can stop readmitting a peer
without manufacturing an exclusion it cannot prove. It deliberately leaves discovery alone — the
suspended peer keeps arriving and keeps being refused — because a suspension nobody is told about
must not be able to strand a pair, and the repeated refusals cost no more than any stranger's
refused exchange. Attributing silence only to an
open transport belongs to the same defense: both refusals close without acknowledging, so a node
that penalized the missing acknowledgement would let any peer manufacture an exclusion against
itself simply by refusing, and would silently upgrade a one-sided suspension into a mutual permanent
exclusion neither side can appeal. Completion grants no channel
...
| <a id="req-auth-4-jwcf71.t1"></a>`REQ-AUTH-4-JWCF71.T1` | [`REQ-AUTH-4-JWCF71`](handshake.md#req-auth-4-jwcf71) | Fail exchanges before and after identity proof, repeat malformed, timing-invalid, or invalid-signature exchanges on an already authenticated transport, then present an already-excluded identity and an identity with locally suspended reconnects over fresh transports and observe discovery reachability afterwards, and separately let a refusal close the transport before the acknowledgement and hold one full window. | Pre-proof failures drop the transport with no identity penalty; faults attributable to an already proven transport exclude that identity; post-proof non-acknowledgement on a still-open transport excludes the proven identity; a missing acknowledgement after the transport closed excludes nobody on either side; an excluded identity is refused and the transport it arrived on is no longer rediscovered; a suspended identity is refused with no exclusion recorded and with its discovery reachability untouched. | <a id="req-auth-4-jwcf71.t1.p1"></a>`REQ-AUTH-4-JWCF71.T1.P1` — timeout before proof; <a id="req-auth-4-jwcf71.t1.p2"></a>`REQ-AUTH-4-JWCF71.T1.P2` — verified-but-silent peer excluded; <a id="req-auth-4-jwcf71.t1.p3"></a>`REQ-AUTH-4-JWCF71.T1.P3` — excluded identity refused at verification; <a id="req-auth-4-jwcf71.t1.p4"></a>`REQ-AUTH-4-JWCF71.T1.P4` — excluded identity's arriving transport is not rediscovered; <a id="req-auth-4-jwcf71.t1.p5"></a>`REQ-AUTH-4-JWCF71.T1.P5` — reconnect-suspended identity refused without exclusion. <a id="req-auth-4-jwcf71.t1.p6"></a>`REQ-AUTH-4-JWCF71.T1.P6` — suspension placed after the response is accepted still refuses at completion; <a id="req-auth-4-jwcf71.t1.p7"></a>`REQ-AUTH-4-JWCF71.T1.P7` — a suspended identity's arriving handle stays reachable, so its attempts keep arriving and keep being refused until the suspension lifts; <a id="req-auth-4-jwcf71.t1.p8"></a>`REQ-AUTH-4-JWCF71.T1.P8` — a refusal that closes the transport before the acknowledgement leaves the refused peer's identity unexcluded after a full window, and the refusing peer records no exclusion either. |
