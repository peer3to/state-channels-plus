...
times out and its transport closes with reconnect allowed, whether or not this node had already
verified the counterparty: silence bounds nothing but liveness, so it proves no fault and penalizes
no identity.
...
| Failure                                                   | Outcome                                                                                                     |
| --------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| Malformed challenge at the responder                      | Reject before signing; exclude only if this transport already has an authenticated identity.                |
| Non-finite or out-of-window claimed time at the responder | Reject before signing and close with reconnect allowed; a clock reading outside the window proves no fault. |
| Response timeout                                          | Close with reconnect allowed; no identity penalty.                                                          |
| Round-trip or skew bound exceeded at the initiator        | Reject the exchange and close with reconnect allowed; no identity penalty.                                  |
| Undecodable signature                                     | Reject; exclude only if this transport already has an authenticated identity.                               |
| Signature verifying to an excluded identity               | Close and re-apply the exclusion, so the newly offered transport also carries it.                           |
| Duplicate acknowledgement                                 | Protocol violation: terminate and exclude.                                                                  |
| Verified peer never acknowledges within the window        | Close with reconnect allowed; no identity penalty.                                                          |
| Unverified peer stalls                                    | Close with reconnect allowed.                                                                               |
...
**<a id="req-auth-4-jwcf71"></a>`REQ-AUTH-4-JWCF71` — Penalty requires an attributable fault.** Every handshake
failure MUST close its transport under exactly one stated outcome: close with reconnect allowed, or
close and exclude. Exclusion consequences apply to an attributable fault on the handshake exchange
itself and reach whatever profile carries the faulting transport, proven or still anonymous: malformed
input, an undecodable or invalid signature, or a duplicate acknowledgement excludes the sender even
before an identity exists, because the fault is in the exchange this node can attribute to that
transport. An authenticated transport retains its proof during later re-handshake validation, so the
same faults on it exclude the known sender. Silence, an expired window, and clock-skew or round-trip
incompatibility are liveness and environment symptoms rather than faults: they MUST close the transport
with reconnect allowed even after this node has verified the counterparty's identity. Whether an
anonymous exclusion is the right cost is the open fault-taxonomy decision recorded in
[`DEF-5-E8TP9N`](../../audit/open-findings.md#def-5-e8tp9n). An identity this node has already excluded, answering on a
newly offered transport, MUST be closed and re-excluded so that transport carries the exclusion too.
...
excluding an identity that never proved itself, and its mirror image — provoking a timeout or a clock
difference so an honest proven peer is excluded ([`REQ-AUTH-4-JWCF71`](handshake.md#req-auth-4-jwcf71)). Completion grants no channel
...
| <a id="req-auth-4-jwcf71.t1"></a>`REQ-AUTH-4-JWCF71.T1` | [`REQ-AUTH-4-JWCF71`](handshake.md#req-auth-4-jwcf71) | Fail exchanges before proof; then repeat malformed, invalid-signature, clock-incompatible, and silent exchanges on an already authenticated transport, and answer a fresh exchange with an already excluded identity.               | Every failure closes its transport under exactly one stated outcome; attributable faults on the exchange exclude whatever profile carries that transport, proven or anonymous, while silence and clock incompatibility keep reconnect allowed. | <a id="req-auth-4-jwcf71.t1.p1"></a>`REQ-AUTH-4-JWCF71.T1.P1` — timeout before proof; <a id="req-auth-4-jwcf71.t1.p3"></a>`REQ-AUTH-4-JWCF71.T1.P3` — excluded identity refused at verification; <a id="req-auth-4-jwcf71.t1.p4"></a>`REQ-AUTH-4-JWCF71.T1.P4` — out-of-window claimed time at the responder closes with reconnect allowed; <a id="req-auth-4-jwcf71.t1.p5"></a>`REQ-AUTH-4-JWCF71.T1.P5` — skew or round-trip refusal at the initiator closes with reconnect allowed; <a id="req-auth-4-jwcf71.t1.p6"></a>`REQ-AUTH-4-JWCF71.T1.P6` — an excluded identity answering on a newly offered transport is closed and re-excluded; <a id="req-auth-4-jwcf71.t1.p7"></a>`REQ-AUTH-4-JWCF71.T1.P7` — a verified-but-silent peer is closed with reconnect allowed and not excluded.                                                                                                                                                                                                                                        |
