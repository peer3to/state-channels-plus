...
**<a id="req-auth-4-jwcf71"></a>`REQ-AUTH-4-JWCF71` — Penalty requires proof and an attributable fault.** Every handshake
failure MUST close its transport under exactly one stated outcome: close with reconnect allowed, or
close and exclude the identity. Exclusion consequences apply only to a proven identity that committed
an attributable fault. An authenticated transport retains that proof during later re-handshake
validation, so malformed input, an undecodable or invalid signature, or a duplicate acknowledgement on
it excludes the known sender. Silence, an expired window, and clock-skew or round-trip incompatibility
are liveness and environment symptoms rather than faults: they MUST close the transport with reconnect
allowed even after this node has verified the counterparty's identity. A failure on an unknown
transport never penalizes an identity. An identity this node has already excluded, answering on a
