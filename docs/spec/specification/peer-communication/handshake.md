...
transport, including silence before proof, never penalizes an identity. An exclusion is scoped to the
...
| <a id="req-auth-4-jwcf71.t1"></a>`REQ-AUTH-4-JWCF71.T1` | [`REQ-AUTH-4-JWCF71`](handshake.md#req-auth-4-jwcf71) | Fail exchanges before proof and repeat malformed, timing-invalid, invalid-signature, or silent exchanges on an already authenticated transport; then return the runtime to its pre-channel state and read the exclusion and the rest of the peer set back. | Pre-proof failures drop the transport with no identity penalty; faults attributable to an already proven transport exclude that identity, and that exclusion still refuses the identity after the channel change while the remaining peer records are gone. | <a id="req-auth-4-jwcf71.t1.p1"></a>`REQ-AUTH-4-JWCF71.T1.P1` — timeout before proof; <a id="req-auth-4-jwcf71.t1.p2"></a>`REQ-AUTH-4-JWCF71.T1.P2` — verified-but-silent peer excluded; <a id="req-auth-4-jwcf71.t1.p3"></a>`REQ-AUTH-4-JWCF71.T1.P3` — excluded identity refused at verification. <a id="req-auth-4-jwcf71.t1.p4"></a>`REQ-AUTH-4-JWCF71.T1.P4` — identity scope across a channel change: an identity excluded on proof while the runtime was in one channel is still excluded after the runtime returns to its pre-channel state, while every other record the runtime held about its peers is released.                                                                                                                                                                                                                                                                                                                                                                                                        |
