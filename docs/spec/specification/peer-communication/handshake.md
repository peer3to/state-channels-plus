...
| Signature verifying to an identity whose reconnects are locally suspended    | Session refused; no exclusion recorded and no discovery reachability change beyond the suspension itself.                                                                          |
...
- **Locally suspended reconnects.** An identity whose reconnects this node has locally suspended MUST
  ...
  accepted but before the exchange completes; no such attempt may become a live session. Because a
  suspended peer otherwise repeats the dial/prove/refuse cycle for as long as the suspension lasts,
  the suspension MUST also suppress the discovery reachability of the handle each refused attempt
  arrived on, and that suppression MUST be reachable from the identity so it lifts with the
  suspension. Suspension is not an exclusion: it records no fault against the
  identity, produces no portable evidence, and MUST lift when the local condition that caused it ends
  (for example when the node stops observing the rendezvous the suspension belonged to). Lifting a
  suspension MUST NOT lift an exclusion, and an exclusion MUST continue to refuse the identity for as
  long as it stands.
...
no portable evidence, and no durability beyond its local cause, so a node can stop an unwanted
reconnect loop without manufacturing an exclusion it cannot prove. Attributing silence only to an
