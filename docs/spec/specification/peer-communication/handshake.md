...
| Failure                                                 | Outcome                                                                                                                                        |
| ------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| Malformed challenge or non-finite time at the responder | Reject before signing; exclude with a recorded verdict, by identity when this transport already has one and by its transport handle otherwise. |
| Out-of-window request time at the responder             | Reject before signing; counted close against the sender, keyed by its transport key before proof and by its identity after.                    |
| Response timeout, or a refused challenge                | Counted close; no verdict.                                                                                                                     |
| Round-trip or skew bound exceeded                       | Counted close; no verdict.                                                                                                                     |
| Undecodable signature                                   | Reject; exclude with a recorded verdict, by identity when this transport already has one and by its transport handle otherwise.                |
| Signature verifying to an excluded identity             | Drop the transport; the identity remains excluded.                                                                                             |
| Duplicate acknowledgement                               | Protocol violation: terminate and exclude.                                                                                                     |
| Verified peer never acknowledges within the window      | Counted close against the transport key; the close that reaches the bound suspends the key and the proven identity together.                   |
...
**<a id="req-auth-4-jwcf71"></a>`REQ-AUTH-4-JWCF71` — Penalty requires proof, and clock faults are not proof.** Exclusion consequences apply only to proven
identities. An authenticated transport retains that proof during later re-handshake validation, so
malformed or invalid-signature traffic on it excludes the known sender with a recorded verdict.
An out-of-window time, a response that never arrives or is refused, a round trip beyond the window,
and a verified peer's missing acknowledgement are environment or liveness faults rather than
misconduct, so they MUST NOT record a verdict: each such failure only closes the connection and counts
against one stated per-peer bound for the session, and the failure that reaches the bound suspends the
peer for the remainder of the session instead. Before proof the count is keyed by the transport key the
peer presented, so a first contact that keeps failing is refused by that key; after proof it is keyed
by the identity, and a suspension reached on a verified-but-unacknowledged transport bars the identity
and the key together. A failure on an unknown transport never records a verdict against an identity.
An exclusion is scoped to the
...
channel, including the count against the per-peer bound and any suspension that count reached: a
suspension is scoped to the channel that produced it, not to the identity. Exclusions are not durable, so none of them survives a restart. Because the scope is the identity
...
proof of an attributable fault by that identity, so it is evidence about the identity rather than about the
channel in which the proof happened to arrive. Rejected alternative: channel-scoping the exclusion, so that
the return to the pre-channel state drops it with the rest of the peer set. It was rejected because it hands
a proven cheater a clean slate the moment its victim changes channel, which is cheap for the attacker and
free to repeat; the argument that exclusions vanish on restart anyway does not justify a second, attacker-
triggerable way to clear them. Accepted consequences: an identity proven faulty in one channel is refused in
the next channel the same runtime serves, including when that channel's participants have seen nothing wrong
with it; there is no reevaluation or expiry rule short of a restart, and the broader subjective engagement
policy that could add one remains open under [`OQ-45-ACZCDE` (Subjective post-authentication engagement policy)](../open-questions.md#oq-45-aczcde), with
exclusion durability across restarts still open under [`OQ-34-FY08V2` (RPC boundary decisions)](../open-questions.md#oq-34-fy08v2).
Affected layers: this requirement, [settlement/lifecycle.md](../settlement/lifecycle.md)'s return to the
pre-channel state, and the peer-registry implementation and tests.

**Decision ([`REQ-AUTH-4-JWCF71`](handshake.md#req-auth-4-jwcf71), engineer decision, 2026-09-23, PR #494).**
The count against the per-peer bound and the suspension it reaches are channel-scoped: returning to the
pre-channel state releases them with the rest of the peer set. They record unproven faults (silence, a
refusal, a lagging chain view, a clock outside the window), which are evidence about the channel's
conditions rather than about the identity, so carrying them forward could bar an honest participant of the
next channel the runtime serves. Rejected alternative: keeping them for the runtime's lifetime, like an
exclusion. Accepted consequence: a peer suspended in one channel starts the next with a fresh count and can
be suspended again only by failing again there. Only the exclusion recorded on proof follows the identity.
...
| <a id="req-auth-4-jwcf71.t1"></a>`REQ-AUTH-4-JWCF71.T1` | [`REQ-AUTH-4-JWCF71`](handshake.md#req-auth-4-jwcf71) | Fail exchanges before proof and repeat malformed, timing-invalid, invalid-signature, or silent exchanges on unproven and already authenticated transports, taking the timing-invalid and silent cases both below and up to their per-peer bound; then return the runtime to its pre-channel state and read the exclusion and the rest of the peer set back. | Pre-proof failures count against the transport key with no identity verdict; malformed and invalid-signature faults exclude with a recorded verdict; timing and silence faults only close until their bound, then suspend for the session without one, and a suspension reached after proof bars key and identity together; an exclusion recorded on proof still refuses the identity after the channel change while the remaining peer records are gone. | <a id="req-auth-4-jwcf71.t1.p1"></a>`REQ-AUTH-4-JWCF71.T1.P1` — timeout before proof counts against the transport key and records no identity verdict; <a id="req-auth-4-jwcf71.t1.p2"></a>`REQ-AUTH-4-JWCF71.T1.P2` — verified-but-silent peer counted once and reconnected, with no verdict; <a id="req-auth-4-jwcf71.t1.p3"></a>`REQ-AUTH-4-JWCF71.T1.P3` — excluded identity refused at verification; <a id="req-auth-4-jwcf71.t1.p4"></a>`REQ-AUTH-4-JWCF71.T1.P4` — out-of-window exchanges below the bound leave the peer reconnectable and unpenalized; <a id="req-auth-4-jwcf71.t1.p5"></a>`REQ-AUTH-4-JWCF71.T1.P5` — the out-of-window exchange reaching the bound suspends the peer for the session without recording a verdict; <a id="req-auth-4-jwcf71.t1.p6"></a>`REQ-AUTH-4-JWCF71.T1.P6` — first-contact skew on an unproven transport is counted by the transport key and the bound refuses that key; <a id="req-auth-4-jwcf71.t1.p7"></a>`REQ-AUTH-4-JWCF71.T1.P7` — identity scope across a channel change: an identity excluded on proof while the runtime was in one channel is still excluded after the runtime returns to its pre-channel state, while every other record the runtime held about its peers is released; <a id="req-auth-4-jwcf71.t1.p8"></a>`REQ-AUTH-4-JWCF71.T1.P8` — a count against the per-peer bound and a suspension it reached in one channel are both released when the runtime returns to its pre-channel state, and the suspended peer's discovery handle is admitted again. |
