...
| Remote abort — the peer ends the negotiation with the protocol's own abort message              | Not a violation: treat it as an exit from the lobby session. Close with reconnect allowed, bar the peer from rematching in that session ([`REQ-LOBBY-10-V8MA22`](lobby-matching.md#req-lobby-10-v8ma22)), clear the unsigned attempt.                                                                                    |
| Lower peer does not initiate within two agreement windows                                       | Bar the lower peer from the lobby session and release the unsigned attempt; a burned window is a matching fault, not a wire fault.                                                                                                                                                                                       |
...
| Counterparty abandonment after commitment but before local signature                            | Bar from the lobby session, clear attempt, return to discovery when still active.                                                                                                                                                                                                                                        |
| Counterparty abandonment after local signature                                                  | Bar from the lobby session and observe chain until open or payload expiry.                                                                                                                                                                                                                                               |
| Derived ID already open at the peer's match, or an opening receipt this node could not complete | The peer's view may be stale and the failure may be local: bar from the lobby session, never exclude.                                                                                                                                                                                                                    |
...
violation and excludes the identity. An abort the peer sends through the protocol's own abort message is
an exit from the lobby session, not a violation: it MUST close with reconnect allowed and bar the peer
under [`REQ-LOBBY-10-V8MA22` (Session-scoped do-not-rematch set)](lobby-matching.md#req-lobby-10-v8ma22). A failure that may be this node's
own — an opening receipt that did not complete, a match into a channel already open — MUST take the same
bar and MUST NOT exclude.
