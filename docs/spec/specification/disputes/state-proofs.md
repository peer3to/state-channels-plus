...
directly — its retained confirmation signatures (direct participant signatures and valid frozen
selected-tower confirmations, [`REQ-BLOCK-PIPE-11-DCHAJ2`](../block-progression/block-processing.md#req-block-pipe-11-dchaj2))
supply the required effective participant credits, deduplicated only at threshold calculation
([`REQ-FIN-7-RTZWQZ`](../protocol-model/finality.md#req-fin-7-rtzwqz)) — or **virtually**, because later,
cryptographically linked confirmations inside the milestone carry their signatures back through
the ancestry ([finality.md §4](../protocol-model/finality.md)). Every signature artifact stays in
the proof; the tower's signature is validated against the channel's historical frozen assignment.
...
a 4→5 join is final only when the four original participants _and_ the joiner have supplied
their effective credits — a direct or virtual signature, or a valid confirmation from that
participant's historically frozen selected tower for this channel
([`REQ-FIN-7-RTZWQZ`](../protocol-model/finality.md#req-fin-7-rtzwqz)) — proof that both the
original set and the joiner authorized the change. A
...
**[`REQ-SP-1-9YABY1`](state-proofs.md#req-sp-1-9yaby1).** A milestone is a finality anchor: its first block is final directly or via virtual votes from later linked confirmations within the milestone.
...
**[`REQ-SP-3-SP1JG4`](state-proofs.md#req-sp-3-sp1jg4).** Membership changes require milestone hops proven under the old∪new (plus pending joiners) union threshold.

**[`REQ-SP-4-NCSEX4`](state-proofs.md#req-sp-4-ncsex4).** Fork genesis is the implicit final anchor: empty proofs claim the genesis snapshot; signed-block proofs must start at `transactionCnt == 0` and hash-link forward.
...
**[`INV-SP-6-GNW74H`](state-proofs.md#inv-sp-6-gnw74h).** Non-final suffixes are safe: conflicting commitments expose slashable double-signs, and reduction selects the longest valid proved history.

**<a id="req-sp-7-70emat"></a>`REQ-SP-7-70EMAT`.** Linkage checks: hash linkage, fork identity, authentic author signatures, threshold coverage, and latest-state commitment, exactly as listed in §7. Threshold coverage is counted in effective participant credits: each retained direct or frozen selected-tower confirmation derives credit, deduplicated only at threshold calculation, with a tower signature validated against the channel's historical frozen assignment ([`REQ-FIN-7-RTZWQZ`](../protocol-model/finality.md#req-fin-7-rtzwqz)).
