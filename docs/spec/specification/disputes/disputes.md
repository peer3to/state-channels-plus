...
**<a id="req-dis-2-pkvz7e"></a>`REQ-DIS-2-PKVZ7E`.** Upload is limited to eligible disputers: `disputer == msg.sender`, dispute-eligible, ≤1 dispute per window per participant, throttled to one upload per `evidenceTime` per sender, and anchored exactly at the chain's latest inbound message block (hash and height equal to the on-chain inbound head at upload; a dispute built before a newer inbound block landed is refused and must be rebuilt).
