...
A dispute stating none of the five inputs is itself objectively invalid. The
...
| Type                                                                         | Role                                                                                                                                                                                                                                                                                                                                                       |
| ---------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Dispute` = `DisputeInput` + `postedAuditingData` + `outputSnapshotDataHash` | The claim: channel, `forkId` (hash of the disputed fork's genesis `SnapshotData`), latest state snapshot hash, inbound tip, `StateProof`, listed on-chain slashes, auditing-data hash, disputer, optional `Timeout`, optional `selfRemoval`, the signed `requireExistingDisputeWindow` flag, plus the disputer-computed hash of the output `SnapshotData`. |
...
stateDiagram-v2
    ...
    Idle --> Open: uploadDispute[WithCalldata] with an independent reason (opener commitment recorded immediately)
    Open --> Open: valid state contributions committed (≤1 per participant, each resets the kill period)
...
- The disputer MUST be `msg.sender` and MUST be dispute-eligible (`canParticipateInDisputes`:
  in the current snapshot participant set or an eligible JOIN strictly after the snapshot inbound boundary through the latest inbound head, and not in the on-chain slash set). Current snapshot participants remain eligible regardless of join age. A nonparticipant whose JOIN is at or older than that consumed boundary is ineligible. Excluding
...
- The dispute's inbound anchor (`latestInboundMessageBlockHash` and its height) MUST equal the chain's
  inbound head at upload; any other anchor is refused as a race and the attempt ends without a retry.
  A later dispute on that fork is built fresh from current state
...
- **Throttle:** a sender may upload at most one dispute per `evidenceTime` per channel
  (`disputerThrottle`), and at most one dispute per window (`hasPosted`) ([`REQ-DIS-2-PKVZ7E`](disputes.md#req-dis-2-pkvz7e)).
...
- **Upload-time race checks** (skipped when `isForced` — used when the target committed to a
  ...
  was created before `minTimeStamp` ([`REQ-DIS-10-SAHJBN`](disputes.md#req-dis-10-sahjbn)).
...
**<a id="req-dis-1-xaj1va"></a>`REQ-DIS-1-XAJ1VA`.** A dispute MUST state at least one of the five valid inputs (timeout, valid on-chain slashes, self-removal, forced inbound inclusion, or an accepted existing-window state contribution); fraud proofs are not a dispute input.

**<a id="req-dis-2-pkvz7e"></a>`REQ-DIS-2-PKVZ7E`.** Upload is limited to eligible disputers: `disputer == msg.sender`, dispute-eligible, ≤1 dispute per window per participant, throttled to one upload per `evidenceTime` per sender, and anchored exactly at the chain's latest inbound message block (hash and height equal to the on-chain inbound head at upload; a dispute built before a newer inbound block landed is refused and not retried; a later dispute on that fork is built fresh from current state).
...
| ------------------------------------------------------- | ---------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <a id="req-dis-1-xaj1va.t1"></a>`REQ-DIS-1-XAJ1VA.T1`   | [`REQ-DIS-1-XAJ1VA`](disputes.md#req-dis-1-xaj1va)   | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | A dispute MUST state at least one of the five valid inputs (timeout, valid on-chain slashes, self-removal, forced inbound inclusion, or an accepted existing-window state contribution); fraud proofs are not a dispute input.                                                                                                                                                                                                                                      | <a id="req-dis-1-xaj1va.t1.p1"></a>`REQ-DIS-1-XAJ1VA.T1.P1` — valid case<br><a id="req-dis-1-xaj1va.t1.p2"></a>`REQ-DIS-1-XAJ1VA.T1.P2` — before deadline<br><a id="req-dis-1-xaj1va.t1.p3"></a>`REQ-DIS-1-XAJ1VA.T1.P3` — new participant<br><a id="req-dis-1-xaj1va.t1.p4"></a>`REQ-DIS-1-XAJ1VA.T1.P4` — malformed input<br><a id="req-dis-1-xaj1va.t1.p5"></a>`REQ-DIS-1-XAJ1VA.T1.P5` — direct invalid/opposite case<br><a id="req-dis-1-xaj1va.t1.p6"></a>`REQ-DIS-1-XAJ1VA.T1.P6` — at deadline<br><a id="req-dis-1-xaj1va.t1.p7"></a>`REQ-DIS-1-XAJ1VA.T1.P7` — after deadline<br><a id="req-dis-1-xaj1va.t1.p8"></a>`REQ-DIS-1-XAJ1VA.T1.P8` — maximum honest skew<br><a id="req-dis-1-xaj1va.t1.p9"></a>`REQ-DIS-1-XAJ1VA.T1.P9` — existing participant<br><a id="req-dis-1-xaj1va.t1.p10"></a>`REQ-DIS-1-XAJ1VA.T1.P10` — removed participant<br><a id="req-dis-1-xaj1va.t1.p11"></a>`REQ-DIS-1-XAJ1VA.T1.P11` — slashed participant<br><a id="req-dis-1-xaj1va.t1.p12"></a>`REQ-DIS-1-XAJ1VA.T1.P12` — concurrent membership change<br><a id="req-dis-1-xaj1va.t1.p13"></a>`REQ-DIS-1-XAJ1VA.T1.P13` — adversarial input<br><a id="req-dis-1-xaj1va.t1.p14"></a>`REQ-DIS-1-XAJ1VA.T1.P14` — partial failure<br><a id="req-dis-1-xaj1va.t1.p15"></a>`REQ-DIS-1-XAJ1VA.T1.P15` — retry and recovery                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| <a id="req-dis-2-pkvz7e.t1"></a>`REQ-DIS-2-PKVZ7E.T1`   | [`REQ-DIS-2-PKVZ7E`](disputes.md#req-dis-2-pkvz7e)   | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | Upload is limited to eligible disputers: `disputer == msg.sender`, dispute-eligible, ≤1 dispute per window per participant, throttled to one upload per `evidenceTime` per sender, and anchored exactly at the chain's latest inbound message block (hash and height equal to the on-chain inbound head at upload; a dispute built before a newer inbound block landed is refused and not retried; a later dispute on that fork is built fresh from current state). | <a id="req-dis-2-pkvz7e.t1.p1"></a>`REQ-DIS-2-PKVZ7E.T1.P1` — valid case<br><a id="req-dis-2-pkvz7e.t1.p2"></a>`REQ-DIS-2-PKVZ7E.T1.P2` — correct identity/signature<br><a id="req-dis-2-pkvz7e.t1.p3"></a>`REQ-DIS-2-PKVZ7E.T1.P3` — before deadline<br><a id="req-dis-2-pkvz7e.t1.p4"></a>`REQ-DIS-2-PKVZ7E.T1.P4` — malformed input<br><a id="req-dis-2-pkvz7e.t1.p5"></a>`REQ-DIS-2-PKVZ7E.T1.P5` — direct invalid/opposite case<br><a id="req-dis-2-pkvz7e.t1.p6"></a>`REQ-DIS-2-PKVZ7E.T1.P6` — wrong identity/signature<br><a id="req-dis-2-pkvz7e.t1.p7"></a>`REQ-DIS-2-PKVZ7E.T1.P7` — missing identity/signature<br><a id="req-dis-2-pkvz7e.t1.p8"></a>`REQ-DIS-2-PKVZ7E.T1.P8` — duplicate identity/signature<br><a id="req-dis-2-pkvz7e.t1.p9"></a>`REQ-DIS-2-PKVZ7E.T1.P9` — forged identity/signature<br><a id="req-dis-2-pkvz7e.t1.p10"></a>`REQ-DIS-2-PKVZ7E.T1.P10` — membership boundary<br><a id="req-dis-2-pkvz7e.t1.p11"></a>`REQ-DIS-2-PKVZ7E.T1.P11` — at deadline<br><a id="req-dis-2-pkvz7e.t1.p12"></a>`REQ-DIS-2-PKVZ7E.T1.P12` — after deadline<br><a id="req-dis-2-pkvz7e.t1.p13"></a>`REQ-DIS-2-PKVZ7E.T1.P13` — maximum honest skew<br><a id="req-dis-2-pkvz7e.t1.p14"></a>`REQ-DIS-2-PKVZ7E.T1.P14` — adversarial input<br><a id="req-dis-2-pkvz7e.t1.p15"></a>`REQ-DIS-2-PKVZ7E.T1.P15` — partial failure<br><a id="req-dis-2-pkvz7e.t1.p16"></a>`REQ-DIS-2-PKVZ7E.T1.P16` — retry and recovery; <a id="req-dis-2-pkvz7e.t1.p17"></a>`REQ-DIS-2-PKVZ7E.T1.P17` — current snapshot member remains eligible despite an old JOIN, in both upload modes; <a id="req-dis-2-pkvz7e.t1.p18"></a>`REQ-DIS-2-PKVZ7E.T1.P18` — JOIN at the latest inbound head is eligible, in both upload modes; <a id="req-dis-2-pkvz7e.t1.p19"></a>`REQ-DIS-2-PKVZ7E.T1.P19` — JOIN inside the unconsumed interval is eligible, in both upload modes; <a id="req-dis-2-pkvz7e.t1.p20"></a>`REQ-DIS-2-PKVZ7E.T1.P20` — nonparticipant JOIN at the consumed boundary is rejected in both upload modes; <a id="req-dis-2-pkvz7e.t1.p21"></a>`REQ-DIS-2-PKVZ7E.T1.P21` — older out-of-bound nonparticipant is rejected in both upload modes; <a id="req-dis-2-pkvz7e.t1.p22"></a>`REQ-DIS-2-PKVZ7E.T1.P22` — on-chain-slashed snapshot participant is rejected in both upload modes; <a id="req-dis-2-pkvz7e.t1.p23"></a>`REQ-DIS-2-PKVZ7E.T1.P23` — on-chain-slashed pending JOIN is rejected in both upload modes; <a id="req-dis-2-pkvz7e.t1.p28"></a>`REQ-DIS-2-PKVZ7E.T1.P28` — anchor one block below the inbound head is refused without mutation, in both upload modes; <a id="req-dis-2-pkvz7e.t1.p29"></a>`REQ-DIS-2-PKVZ7E.T1.P29` — anchor at the inbound head is accepted, in both upload modes; <a id="req-dis-2-pkvz7e.t1.p30"></a>`REQ-DIS-2-PKVZ7E.T1.P30` — an otherwise valid signed dispute anchored below the inbound head is refused end to end and opens no dispute window; <a id="req-dis-2-pkvz7e.t1.p31"></a>`REQ-DIS-2-PKVZ7E.T1.P31` — a dispute blind to a pending join is refused at upload, so the join is adopted as normal; <a id="req-dis-2-pkvz7e.t1.p32"></a>`REQ-DIS-2-PKVZ7E.T1.P32` — anchor naming the head hash with a different height is refused |
