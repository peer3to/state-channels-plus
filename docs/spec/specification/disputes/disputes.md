...
- **One ordinary-timeout acknowledgement defense ([`REQ-DIS-11-JJ9FG3`](disputes.md#req-dis-11-jj9fg3)).**
  ...
  wrong-tower, wrong-epoch, wrong-slot, or unavailable receipt proves nothing and leaves the
...
- **Receipts can replace calldata at the availability threshold
  ...
  off-chain availability threshold over the union of the block's previous and resulting
  participant sets, with the established on-chain eligibility adjustments. The accounting is by
  deduplicated **availability credit**: a participant's own acknowledgement credits only that
  ...
  acknowledgements, so a single tower signature can satisfy the full availability threshold when every eligible
...
- **A valid restricted AFK block backs the AFK-mode timeout
  ...
  from the target's assignment-epoch tower, committed with the timeout input together with its
  slot-validation context ([data-types.md §7.1](../protocol-model/data-types.md#71-disputes)) and
  validated on-chain — authority, canonical removal body, target and epoch binding, restricted
