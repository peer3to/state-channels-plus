...
([`REQ-BLOCK-PIPE-1-SS24D1`](block-processing.md#req-block-pipe-1-ss24d1)). Classify each new confirmation independently: unrecoverable bytes are a strategy deviation, never an exception that aborts classification of later good values. Recover
...
### Stage 3 — Entry-lifetime expiry for admitted sources
...
and the author ([`REQ-BLOCK-PIPE-4-CF52J6`](block-processing.md#req-block-pipe-4-cf52j6), [synchronization.md](../peer-communication/synchronization.md)) — this is the expiry probe for already-admitted sources; unknown-source sync is a separate trigger; eligible height → schedule execution ([`REQ-BLOCK-PIPE-6-XQ0RTT`](block-processing.md#req-block-pipe-6-xq0rtt)); still in the
future → discard and probe the same way ([`REQ-BLOCK-PIPE-5-WJ31RG`](block-processing.md#req-block-pipe-5-wj31rg): a block that matters is recoverable from chain data; junk must not
accumulate). A probe decides the source's fate ([`REQ-BLOCK-PIPE-4-CF52J6`](block-processing.md#req-block-pipe-4-cf52j6)): a source that fails to prove takes the
counted close of the synchronization rules (one strike, suspension at the bound, no verdict), and a source whose proven lineage does not carry the block it supplied
...
- **Provable participant fault.** A canonically checkable violation pinned to a specific
  participant by that participant's own signature: double-signing (same author, two different
  blocks at one fork/height); an invalid state transition by the author (failed execution,
...
- **Subjective lateness.** The agreement-window judgment. It is observer-relative by definition,
  ...
  ([`REQ-BLOCK-PIPE-8-N529VH`](block-processing.md#req-block-pipe-8-n529vh)). It does not apply to a block replayed from a
  verified synchronization proof ([synchronization.md](../peer-communication/synchronization.md) step 13): that block is proven
  history, and parking it would leave a suffix older than one window unapplied forever.
...
A pending participant has an on-chain stake: it reacts to provable participant fraud with a fraud proof and dispute, like a participant, but never counter-signs.

**Spectator** — active while the node has no on-chain stake (`NOT_OPENED`, `OPENED`, `SYNCED`). Same gates as
...
able to force an abort. Not-yet-ready → restore. Synchronization proof replay accepts historical subjective timestamps. A committed peer replaying such history retains its fraud-proof and dispute reactions. Note the
...
second block. A slot has one author: a submission whose claimed coordinate is no longer the current
fork's next height when it reaches the serialized boundary (the writer's own earlier submission, a
...
retry. Admitted-source expiry probes run once per attributed source and the author. Separately, an authenticated sender still absent after the cache refresh triggers immediate ordinary sync under [`REQ-GOSSIP-4-J5Z4DF` (Eligible transport contribution)](../peer-communication/block-gossip.md#req-gossip-4-j5z4df). Recovered work MUST re-enter the same validation and commitment pipeline. For an admitted-source expiry probe, a source whose synchronization succeeds without its proven lineage carrying the probed block MUST be
excluded: it supplied junk. A source whose lineage carries the block MUST NOT be excluded.
...
A queued entry records each source's admitted signatures. Dequeue passes that attribution to ordinary
validation and removes the entry from the queue. Restoration merges it with concurrent queued copies
under the source limits. Proof and calldata inputs keep their existing validation paths. The fixed queue deadline cannot be extended by membership refresh
or repeated copies ([`REQ-QSTORE-2-VYWJAQ` (Independent source allowances)](../storage/queue.md#req-qstore-2-vywjaq), [`REQ-QSTORE-3-DEKYG6` (Queue scheduling)](../storage/queue.md#req-qstore-3-dekyg6)).
