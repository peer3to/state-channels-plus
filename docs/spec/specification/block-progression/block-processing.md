...
MUST surface as an internal error rather than be silently accepted. The context follows the work
item, not its arrival: an item proved committed on the base layer stays in the chain-committed
context for as long as it is in the pipeline, including after it merges with gossip copies of the
same block and after any not-ready park and restore, and a node resolves the context when the item
is executed rather than when it is queued, so a membership change decided meanwhile cannot leave it
judged by a stale context. Because such an item can carry merged gossip signatures and sources, the
chain-committed context classifies every deviation the live context does and adds only the
consequences the chain commitment makes possible.
...
author — but recovered work MUST re-enter the same validation and commitment pipeline. A probed
source whose synchronization succeeds without its proven lineage carrying the probed block MUST be
