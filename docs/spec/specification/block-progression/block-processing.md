...
accumulate). A probe decides the source's fate ([`REQ-BLOCK-PIPE-4-CF52J6`](block-processing.md#req-block-pipe-4-cf52j6)): a source that fails to prove is excluded by
the synchronization rules, and a source whose proven lineage does not carry the block it supplied
