...
departure and may dispose immediately. A new channel uses a fresh runtime. If an authored exit cannot settle by snapshot and its dispute fallback fails, the pending leave rejects. A delayed failure from an older fork must not reject a leave on the current fork.
