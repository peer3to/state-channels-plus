...
1. **Same-fork finality.** Every required participant supplied its finality vote (directly or via
   virtual votes — [finality.md](../protocol-model/finality.md), with the single target-only AFK
   credit); the proof of finality (milestone proofs + milestone snapshots —
   ...
   consumed. **Every later accepted history preserves the exact latest settled snapshot** — the
   checkpoint: its state allocation, membership, balances, messages, and history commitment.
   Same-height conflicts and higher conflicting branches cannot replace it; total-withdrawal and
   deposit checks remain necessary but never prove per-user allocation or ancestry; the proof
   must extend the checkpoint through the authentic bridge block and continuation
   ([state-proofs.md](../disputes/state-proofs.md),
   [history-and-commitments.md §3.2](../protocol-model/history-and-commitments.md#32-hash-linking)).
   The checkpoint changes only after successful atomic settlement; an observed off-chain
   certificate or a submitted transaction is not a checkpoint. Conflicting tower-certified
   same-fork histories (possible only in the fully malicious exception of
   [`INV-WT-1-ST9SHX`](../runtime/watchtowers.md#inv-wt-1-st9shx)) race this entry point under
   ordinary transaction order: the first valid update settles the winning history's checkpoint,
   every later incompatible update — equal or higher — is rejected as a conflicting branch,
   settlement and withdrawal effects stay deposit-bounded and occur at most once, and no
   double-sign proof, tower contradiction proof, or dispute transaction is a prerequisite for —
   or can later replace — a settled checkpoint. Checkpoint updates stay allowed while disputes
   run, with one freeze: a same-fork update whose destination fork carries an on-chain dispute
   commitment is refused from that first commitment onward, including after the reduction
   finalizes ([`REQ-LIF-8-GXAGXR`](lifecycle.md#req-lif-8-gxagxr)); adoption leaves such a fork
   only through path 2. The treatment of already committed claims and finalized reduction
   references as the checkpoint advances remains the bounded open question
   [`OQ-50-YSDG8S`](../open-questions.md#oq-50-ysdg8s) for the cases that freeze and the
   snapshot-bound tower approval
   ([`REQ-DIS-14-032T4M`](../disputes/disputes.md#req-dis-14-032t4m)) do not decide — until
   decided, checkpoint preservation is required and unsafe adoption is rejected.
2. **Dispute resolution.** A dispute window reduces deterministically to a successor fork
   ...
   adopted in one call; the walk passes disputed intermediate forks through their finalized
   reduction links, and the adopted destination must itself be undisputed
   ([`REQ-LIF-8-GXAGXR`](lifecycle.md#req-lif-8-gxagxr)). **For this protocol version:** when the reduction is computed and finalized
...
**<a id="req-lif-8-gxagxr"></a>`REQ-LIF-8-GXAGXR` — Disputed destination forks are frozen.** A snapshot
update whose destination fork carries an on-chain dispute commitment MUST be refused, from the
fork's first commitment onward and including after its reduction finalizes. Adoption may still
leave a disputed fork for a resolved successor whose destination is not itself disputed, following
verified reduction links past disputed intermediate forks. The freeze changes no history rule:
every accepted result and adoption preserves the exact settled checkpoint and message history,
including the canonical checkpoint bridge and outbound continuity; the longest
checkpoint-compatible history and the smaller-latest-block-hash tie-break still select the
history; timeout selection stays independent of that history and subject to slash precedence; and
an AFK removal already present in the selected valid history is not undone by the channel-slash
suppression of additional timeout removals. If a dispute lands before a same-fork settlement, that
settlement fails and settlement proceeds through the resolved successor; if the settlement lands
first, the checkpoint stands and every later result must preserve it
([`REQ-DIS-14-032T4M`](../disputes/disputes.md#req-dis-14-032t4m)). Delayed settlement and
withdrawals and the pre-settlement partition race are accepted; no short-duration or
single-lost-transaction bound is claimed. A window's creation timestamp continues to control
storage cleanup only; it is not the freeze.
...
The four deployment windows are unchanged by the watchtower model; the restricted AFK block adds
only its own allowed authoring timestamp range `[D, F]` derived from these windows
([time.md §5.2](../protocol-model/time.md#52-objective-timestamp-validity-fraud-provable)).
...
**[`REQ-LIF-2-Z3Z9Y3`](lifecycle.md#req-lif-2-z3z9y3).** Only two paths yield a snapshot-updating state: same-fork finality proof, or dispute reduction after the challenge window; every later accepted history preserves the exact latest settled checkpoint, and conflicting branches cannot replace it.
...
**[`REQ-LIF-8-GXAGXR`](lifecycle.md#req-lif-8-gxagxr).** A snapshot update whose destination fork carries an on-chain dispute commitment is refused from that commitment onward, including after finalization; adoption leaves a disputed fork only through verified reduction links to an undisputed successor, preserving the exact settled checkpoint and message history.
...
| <a id="req-lif-2-z3z9y3.t1"></a>`REQ-LIF-2-Z3Z9Y3.T1` | [`REQ-LIF-2-Z3Z9Y3`](lifecycle.md#req-lif-2-z3z9y3) | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals.                                                                                                     | Only two paths yield a snapshot-updating state: same-fork finality proof, or dispute reduction after the challenge window; every later accepted history preserves the exact latest settled checkpoint, and conflicting branches cannot replace it.                                                           | <a id="req-lif-2-z3z9y3.t1.p1"></a>`REQ-LIF-2-Z3Z9Y3.T1.P1` — valid case<br><a id="req-lif-2-z3z9y3.t1.p5"></a>`REQ-LIF-2-Z3Z9Y3.T1.P5` — direct invalid/opposite case<br><a id="req-lif-2-z3z9y3.t1.p2"></a>`REQ-LIF-2-Z3Z9Y3.T1.P2` — matching commitment<br><a id="req-lif-2-z3z9y3.t1.p6"></a>`REQ-LIF-2-Z3Z9Y3.T1.P6` — mismatched commitment<br><a id="req-lif-2-z3z9y3.t1.p7"></a>`REQ-LIF-2-Z3Z9Y3.T1.P7` — predecessor snapshot<br><a id="req-lif-2-z3z9y3.t1.p8"></a>`REQ-LIF-2-Z3Z9Y3.T1.P8` — genesis snapshot<br><a id="req-lif-2-z3z9y3.t1.p9"></a>`REQ-LIF-2-Z3Z9Y3.T1.P9` — stale fork<br><a id="req-lif-2-z3z9y3.t1.p10"></a>`REQ-LIF-2-Z3Z9Y3.T1.P10` — foreign fork<br><a id="req-lif-2-z3z9y3.t1.p3"></a>`REQ-LIF-2-Z3Z9Y3.T1.P3` — before deadline<br><a id="req-lif-2-z3z9y3.t1.p11"></a>`REQ-LIF-2-Z3Z9Y3.T1.P11` — at deadline<br><a id="req-lif-2-z3z9y3.t1.p12"></a>`REQ-LIF-2-Z3Z9Y3.T1.P12` — after deadline<br><a id="req-lif-2-z3z9y3.t1.p13"></a>`REQ-LIF-2-Z3Z9Y3.T1.P13` — maximum honest skew<br><a id="req-lif-2-z3z9y3.t1.p4"></a>`REQ-LIF-2-Z3Z9Y3.T1.P4` — malformed input<br><a id="req-lif-2-z3z9y3.t1.p14"></a>`REQ-LIF-2-Z3Z9Y3.T1.P14` — adversarial input<br><a id="req-lif-2-z3z9y3.t1.p15"></a>`REQ-LIF-2-Z3Z9Y3.T1.P15` — partial failure<br><a id="req-lif-2-z3z9y3.t1.p16"></a>`REQ-LIF-2-Z3Z9Y3.T1.P16` — retry and recovery<br><a id="req-lif-2-z3z9y3.t1.p17"></a>`REQ-LIF-2-Z3Z9Y3.T1.P17` — checkpoint preservation: branch A settles first, then a same-height conflicting update and a **higher** conflicting non-descendant update are each rejected as conflicting branches, while a compatible descendant extending the checkpoint through the authentic bridge block is accepted; matching withdrawal totals alone never admit an incompatible per-user allocation<br><a id="req-lif-2-z3z9y3.t1.p18"></a>`REQ-LIF-2-Z3Z9Y3.T1.P18` — settlement-order race: with two conflicting certified histories, both transaction orders settle exactly the first valid checkpoint, later punishments and disputes neither precede nor replace it, and settlement and withdrawal effects occur at most once<br><a id="req-lif-2-z3z9y3.t1.p19"></a>`REQ-LIF-2-Z3Z9Y3.T1.P19` — checkpoint moves during a dispute: a compatible advance leaves the active window usable and later claims are re-based or rejected per the recorded rule, with no freeze other than the disputed-destination rule of [`REQ-LIF-8-GXAGXR`](lifecycle.md#req-lif-8-gxagxr), no invented waiting period, and no automatic fraud verdict for chain-progress staleness |
...
| <a id="req-lif-8-gxagxr.t1"></a>`REQ-LIF-8-GXAGXR.T1` | [`REQ-LIF-8-GXAGXR`](lifecycle.md#req-lif-8-gxagxr) | Commit a dispute against a fork, then attempt same-fork snapshot updates to that fork before and after its reduction finalizes, successor-fork adoption past it, and the settlement-versus-dispute race in both transaction orders.                       | Updates whose destination is the disputed fork are refused from the first commitment onward; adoption reaches an undisputed successor through verified reduction links with exact checkpoint and outbound continuity; whichever of settlement and dispute lands first stands and the other route follows it. | <a id="req-lif-8-gxagxr.t1.p1"></a>`REQ-LIF-8-GXAGXR.T1.P1` — same-fork update to a fork with an open dispute commitment is refused<br><a id="req-lif-8-gxagxr.t1.p2"></a>`REQ-LIF-8-GXAGXR.T1.P2` — same-fork update to that fork after its reduction finalized is still refused<br><a id="req-lif-8-gxagxr.t1.p3"></a>`REQ-LIF-8-GXAGXR.T1.P3` — dispute lands first: the target's later same-fork settlement fails and settlement proceeds through the resolved successor<br><a id="req-lif-8-gxagxr.t1.p4"></a>`REQ-LIF-8-GXAGXR.T1.P4` — settlement lands first: the checkpoint stands and the later dispute result preserves it<br><a id="req-lif-8-gxagxr.t1.p5"></a>`REQ-LIF-8-GXAGXR.T1.P5` — successor adoption traverses one or more disputed intermediate forks through finalized reduction links to an undisputed destination<br><a id="req-lif-8-gxagxr.t1.p6"></a>`REQ-LIF-8-GXAGXR.T1.P6` — a disputed source with an undisputed destination is not frozen; a disputed destination is frozen regardless of source<br><a id="req-lif-8-gxagxr.t1.p7"></a>`REQ-LIF-8-GXAGXR.T1.P7` — every accepted adoption carries the exact checkpoint bridge and outbound-message continuity, and settled messages and withdrawals are preserved<br><a id="req-lif-8-gxagxr.t1.p8"></a>`REQ-LIF-8-GXAGXR.T1.P8` — an AFK removal present in the selected valid history survives the channel-slash suppression of additional timeout removals, and the longest checkpoint-compatible history with the smaller-latest-block-hash tie-break still selects the history                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
