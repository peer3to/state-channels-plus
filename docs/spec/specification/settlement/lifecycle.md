...
1. **Same-fork finality.** Every required participant signed (directly or via virtual votes —
   ...
   consumed. A newer snapshot need not descend from the current settled snapshot: it may jump
   forward when it passes the existing threshold, finality, objective state-validity, balance,
   accounting, and collective-deposit conservation checks. Conflicting tower-certified same-fork
   histories (possible only in the fully malicious exception of
   [`INV-WT-1-ST9SHX`](../runtime/watchtowers.md#inv-wt-1-st9shx)) race this entry point under
   ordinary transaction order: the first valid update settles the winning history, a later
   conflicting same-fork update at the same height is rejected as not newer, settlement and
   withdrawal effects stay deposit-bounded and occur at most once, and no double-sign proof,
   tower contradiction proof, or dispute transaction is a prerequisite for — or can later
...
**[`REQ-LIF-2-Z3Z9Y3`](lifecycle.md#req-lif-2-z3z9y3).** Only two paths yield a snapshot-updating state: same-fork finality proof, or dispute reduction after the challenge window; a newer same-fork snapshot may jump forward past the settled snapshot without ancestry when it passes the existing validity checks.
