...
**<a id="req-lif-10-qr8nq9"></a>`REQ-LIF-10-QR8NQ9` — Terminal runtime departure.** An application-facing
leave is terminal for its runtime. A committed participant first reaches either a fully signed snapshot update
or the normal self-removal dispute fallback, then waits for settled removal and local `SYNCED` (reached only once the chain lists the signer neither as a participant nor as a pending participant) before runtime
disposal. If posting any fully signed application-authored exit snapshot fails, the same self-removal dispute
fallback applies, whether or not the exit came from the application-facing leave operation. An observer owes no
departure and may dispose immediately. A new channel uses a fresh runtime. A self-removal dispute that does not land because a dispute window already covers the fork — another participant won the race to upload for it — does not fail the leave: the leave waits for that window to settle and, if the signer is still a participant on the fork the settlement produces, retries its departure once on that fork. If an authored exit cannot settle by snapshot and its dispute fallback fails with no window covering the fork, the pending leave rejects. A fallback refused because the fork's dispute window already holds commitments and its evidence period has ended has not failed: that window settles the fork, and the leave waits for the settlement. A delayed failure or refusal from an older fork must not reject or decide a leave on the current fork.
...
| <a id="req-lif-10-qr8nq9.t1"></a>`REQ-LIF-10-QR8NQ9.T1` | [`REQ-LIF-10-QR8NQ9`](lifecycle.md#req-lif-10-qr8nq9) | Leave through the public runtime API from observer and participant states; drive fast settlement and dispute fallback.                                                                                                                                                                   | The promise resolves only after removal is settled and observed, then the runtime is disposed; an observer disposes immediately; a self-removal lost to a covering window waits for that window instead of rejecting. | <a id="req-lif-10-qr8nq9.t1.p1"></a>`REQ-LIF-10-QR8NQ9.T1.P1` — fast fully signed snapshot; <a id="req-lif-10-qr8nq9.t1.p2"></a>`REQ-LIF-10-QR8NQ9.T1.P2` — self-removal dispute; <a id="req-lif-10-qr8nq9.t1.p3"></a>`REQ-LIF-10-QR8NQ9.T1.P3` — observer immediate disposal; <a id="req-lif-10-qr8nq9.t1.p4"></a>`REQ-LIF-10-QR8NQ9.T1.P4` — fresh runtime selects another channel; <a id="req-lif-10-qr8nq9.t1.p5"></a>`REQ-LIF-10-QR8NQ9.T1.P5` — a disposed leaver cannot sign a later block and remaining honest peers do not blacklist each other; <a id="req-lif-10-qr8nq9.t1.p6"></a>`REQ-LIF-10-QR8NQ9.T1.P6` — a locally reduced fork that drops the leaver keeps its status and its leave pending until the chain's snapshot records the removal. A posted snapshot that drops a leaver cannot leave that leaver's JOIN pending: a JOIN is refused while the fork carries a dispute window, a same-fork post must consume every pending JOIN, and a reduction consumes the JOINs up to its window's expiry, so the removal alone proves the signer is in neither on-chain set.; <a id="req-lif-10-qr8nq9.t1.p7"></a>`REQ-LIF-10-QR8NQ9.T1.P7` — fully signed exit: snapshot failure and a failed dispute submission reject leave; <a id="req-lif-10-qr8nq9.t1.p8"></a>`REQ-LIF-10-QR8NQ9.T1.P8` — fully signed exit: snapshot failure and an evidence-expired fallback refusal keep leave pending, and the settled fork that drops the leaver resolves it; <a id="req-lif-10-qr8nq9.t1.p9"></a>`REQ-LIF-10-QR8NQ9.T1.P9` — unsigned exit: a failed dispute submission rejects leave; <a id="req-lif-10-qr8nq9.t1.p10"></a>`REQ-LIF-10-QR8NQ9.T1.P10` — unsigned exit: an evidence-expired fallback refusal that no window on the fork explains rejects leave; <a id="req-lif-10-qr8nq9.t1.p11"></a>`REQ-LIF-10-QR8NQ9.T1.P11` — unsigned exit: one self-removal dispute settles departure; <a id="req-lif-10-qr8nq9.t1.p36"></a>`REQ-LIF-10-QR8NQ9.T1.P36` — a bound-driven self-removal refused after another participant's window stopped taking evidence: the leave waits for that window, retries once on the settled fork and settles; <a id="req-lif-10-qr8nq9.t1.p37"></a>`REQ-LIF-10-QR8NQ9.T1.P37` — fully signed exit whose snapshot post fails and whose self-removal loses the race to a covering window: the leave waits for that window and settles; <a id="req-lif-10-qr8nq9.t1.p38"></a>`REQ-LIF-10-QR8NQ9.T1.P38` — a self-removal refusal that returns after the leave re-armed on a newer fork does not decide the leave; the newer fork's attempt settles it |
