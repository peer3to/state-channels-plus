...
departure and may dispose immediately. A new channel uses a fresh runtime. If an authored exit cannot settle by snapshot and its dispute fallback fails, the pending leave rejects. A fallback refused because the fork's dispute window already holds commitments and its evidence period has ended has not failed: that window settles the fork, and the leave waits for the settlement. A delayed failure from an older fork must not reject a leave on the current fork.
