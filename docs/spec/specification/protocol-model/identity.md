...
The same attribution decides delegated-dispute authorship: a dispute whose recovered signer is
the named participant is the participant's direct submission with ordinary stale-state liability,
even when that address is also its own selected tower or the signed delegation mode claims tower
authorship; the delegated attribution exception of
[`REQ-WT-11-DZ3AAE`](../runtime/watchtowers.md#req-wt-11-dz3aae) applies only to a distinct
selected-tower signer. Key reuse itself is not a slash offense, and no registration, selection,
or join rule rejects an address merely for holding both roles; equally, no role tags, signing precedence, coordination or
