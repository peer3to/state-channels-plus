...
the same (message, signature) pair MUST recover the same address. For block confirmation
signatures, an encoding that on-chain recovery rejects MUST NOT count as recovering any address
off-chain either.
