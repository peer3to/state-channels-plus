...
**<a id="req-time-5-s9nqxk"></a>`REQ-TIME-5-S9NQXK`.** Every local contract execution observes the runtime's current estimated chain time (§2) as ambient `block.timestamp`, read at call time so it keeps advancing; an isolated executor receives the Clock's adjustment when it is initialized and derives the same perception from its own wall clock. This is the time that manager and protocol views defined against current time (kill periods, evidence windows) see in the local mirror. State-machine transitions stay deterministic: they MUST use `_tx.header.timestamp` and MUST NOT read ambient `block.timestamp`.
...
| <a id="req-time-5-s9nqxk.t1"></a>`REQ-TIME-5-S9NQXK.T1` | [`REQ-TIME-5-S9NQXK`](time.md#req-time-5-s9nqxk) | Execute a contract returning `block.timestamp` through the inline and the isolated executor under a non-zero clock adjustment, and read it again after a second. | Both values match the runtime Clock within the one-second sampling boundary and advance.                                  | <a id="req-time-5-s9nqxk.t1.p1"></a>`REQ-TIME-5-S9NQXK.T1.P1` — inline executor; <a id="req-time-5-s9nqxk.t1.p2"></a>`REQ-TIME-5-S9NQXK.T1.P2` — Node isolated executor; <a id="req-time-5-s9nqxk.t1.p3"></a>`REQ-TIME-5-S9NQXK.T1.P3` — browser isolated executor.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
