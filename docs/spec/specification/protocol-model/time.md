...
objectively invalid; the clamp converts clock error into using the boundary value instead. A
selected tower authoring the restricted AFK block stamps it the same way, clamped into the
restricted window of §5.2 instead of the ordinary ceiling.
...
- Arrival window: a block whose `|now − block.timestamp| > agreementTime` (by the local estimated
  ...
  judgment ([`REQ-WT-4-PNMYMP`](../runtime/watchtowers.md#req-wt-4-pnmymp)). This exception never
  makes a peer switch a signed history: a peer already committed to a conflicting candidate keeps
  its commitment and stores the confirmation as evidence
  ([`INV-FIN-2-MK27J6`](finality.md#inv-fin-2-mk27j6)).
  Finality exception (towers): a selected tower skips this arrival-window gate for a block that
  already carries full participant finality — every required participant accepted it, so the
  tower accepts it too and stays in sync
  ([`REQ-WT-4-PNMYMP`](../runtime/watchtowers.md#req-wt-4-pnmymp)).
  Issuance versus timestamps: a tower's duty to author the restricted AFK block only after the
  agreement window elapsed without receiving the normal block is likewise subjective. An in-range
  signed timestamp does not prove when the tower signed, when it received anything, or that the
  participant was globally absent; an early-created but future-dated artifact cannot be proved
  early from its bytes alone. Live observers keep their local timing gates, and replay never
  applies current wall time to old evidence.
...
**Restricted AFK block window.** The tower-authored AFK removal block of
[`REQ-WT-3-DT0GDX`](../runtime/watchtowers.md#req-wt-3-dt0gdx) has its own allowed timestamp
range beside the ordinary author ceiling. Let `A` be the existing relevant predecessor timestamp
for the represented participant's slot — derived from their direct or frozen-tower predecessor
credit and the authenticated predecessor calldata posting timing exactly as above — and let
`G = evidenceTime` at height zero and `0` otherwise. With `D = A + G + p2pTime + agreementTime`
and `F = D + chainFallbackTime`:

```
AFK block valid  ⇔  D ≤ ts ≤ F   (both bounds inclusive), and ts ≥ predecessor timestamp
```

The participant's ordinary ceiling `A + G + p2pTime` is unchanged; exact equality at `D` means
the preceding agreement window has ended, and no extra delay until `F` is introduced. Honest
issuance additionally requires that the agreement window elapsed without the tower receiving their
normal block — a subjective duty under §5.1, never a consumer-verifiable condition. The window
anchors to the **attested slot's proved predecessor**: no new slot anchor is derived from the
tower's identity, the AFK block's arrival time, its receipt, or a submitted latest head, and AFK
timeout timing uses this anchor even when the latest proof is much longer. The authoring interval
limits the artifact timestamp, not how long otherwise-valid evidence stays usable within the
existing fork and dispute lifecycle. A verifier MUST recover and verify predecessor-posting
preimages before rejecting a time claim — missing local chain data is not evidence of invalidity —
and MUST use checked arithmetic with explicit zero-length-window fallback: no wrap-around or
silent extension is allowed. The next block after an accepted AFK block follows the ordinary
predecessor rules above, including the forfeit-of-extra-time rule.
...
**Delegated watchtower evidence changes three timings** (rules owned by
[disputes.md §6.4](../disputes/disputes.md#64-delegated-watchtower-evidence)):

- A valid `BlockConfirmationReceipt` availability credit counts as that participant's
  acknowledgement of the exact block: it forfeits that participant's additional chain-fallback
  time exactly as its own acknowledgement would, and it never creates a new timestamp anchor.
  Credits race the author's own calldata submission, not a deadline comparator — the agreement
  deadline is the honest tower's subjective signing cutoff under §5.1, never a
  consumer-verifiable condition on the signature
  ...
  every objective check still applies: availability credits arriving first can complete the
  availability threshold so the author skips calldata and the next author's deadline stays derived
...
- The restricted AFK block has its own allowed authoring timestamp range `[D, F]` under §5.2,
  anchored to the attested slot's proved predecessor.
- A timeout carried by a valid restricted AFK block uses the reduced deadline `D` from that
  attested slot — the ordinary deadline **without** the `chainFallbackTime` term — and waives the
  target's acknowledgement and calldata defenses for that exact timeout regardless of posting
  order ([`REQ-DIS-13-1WWHS0`](../disputes/disputes.md#req-dis-13-1wwhs0)). Every other timeout
  keeps the full first-block grace plus peer-to-peer, agreement, and chain-fallback periods. The
...
| ------------------------------------------------------- | ------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| <a id="req-time-1-fm4651.t1"></a>`REQ-TIME-1-FM4651.T1` | [`REQ-TIME-1-FM4651`](time.md#req-time-1-fm4651) | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | Chain time is authoritative; wall clocks are not a source of truth                                                        | <a id="req-time-1-fm4651.t1.p1"></a>`REQ-TIME-1-FM4651.T1.P1` — valid case<br><a id="req-time-1-fm4651.t1.p2"></a>`REQ-TIME-1-FM4651.T1.P2` — correct identity and signature<br><a id="req-time-1-fm4651.t1.p3"></a>`REQ-TIME-1-FM4651.T1.P3` — before deadline<br><a id="req-time-1-fm4651.t1.p4"></a>`REQ-TIME-1-FM4651.T1.P4` — direct invalid/opposite case<br><a id="req-time-1-fm4651.t1.p5"></a>`REQ-TIME-1-FM4651.T1.P5` — wrong-identity signature<br><a id="req-time-1-fm4651.t1.p6"></a>`REQ-TIME-1-FM4651.T1.P6` — missing signature<br><a id="req-time-1-fm4651.t1.p7"></a>`REQ-TIME-1-FM4651.T1.P7` — duplicate signature<br><a id="req-time-1-fm4651.t1.p8"></a>`REQ-TIME-1-FM4651.T1.P8` — forged signature<br><a id="req-time-1-fm4651.t1.p9"></a>`REQ-TIME-1-FM4651.T1.P9` — membership-boundary signer<br><a id="req-time-1-fm4651.t1.p10"></a>`REQ-TIME-1-FM4651.T1.P10` — at deadline<br><a id="req-time-1-fm4651.t1.p11"></a>`REQ-TIME-1-FM4651.T1.P11` — after deadline<br><a id="req-time-1-fm4651.t1.p12"></a>`REQ-TIME-1-FM4651.T1.P12` — maximum honest skew                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| <a id="req-time-2-vg94s7.t1"></a>`REQ-TIME-2-VG94S7.T1` | [`REQ-TIME-2-VG94S7`](time.md#req-time-2-vg94s7) | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | Honest participants keep estimated chain time within the skew bound (estimated from recent block timestamps)              | <a id="req-time-2-vg94s7.t1.p1"></a>`REQ-TIME-2-VG94S7.T1.P1` — valid case<br><a id="req-time-2-vg94s7.t1.p2"></a>`REQ-TIME-2-VG94S7.T1.P2` — correct identity and signature<br><a id="req-time-2-vg94s7.t1.p3"></a>`REQ-TIME-2-VG94S7.T1.P3` — before deadline<br><a id="req-time-2-vg94s7.t1.p4"></a>`REQ-TIME-2-VG94S7.T1.P4` — direct invalid/opposite case<br><a id="req-time-2-vg94s7.t1.p5"></a>`REQ-TIME-2-VG94S7.T1.P5` — wrong-identity signature<br><a id="req-time-2-vg94s7.t1.p6"></a>`REQ-TIME-2-VG94S7.T1.P6` — missing signature<br><a id="req-time-2-vg94s7.t1.p7"></a>`REQ-TIME-2-VG94S7.T1.P7` — duplicate signature<br><a id="req-time-2-vg94s7.t1.p8"></a>`REQ-TIME-2-VG94S7.T1.P8` — forged signature<br><a id="req-time-2-vg94s7.t1.p9"></a>`REQ-TIME-2-VG94S7.T1.P9` — membership-boundary signer<br><a id="req-time-2-vg94s7.t1.p10"></a>`REQ-TIME-2-VG94S7.T1.P10` — at deadline<br><a id="req-time-2-vg94s7.t1.p11"></a>`REQ-TIME-2-VG94S7.T1.P11` — after deadline<br><a id="req-time-2-vg94s7.t1.p12"></a>`REQ-TIME-2-VG94S7.T1.P12` — maximum honest skew                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| <a id="req-time-3-mt1mmf.t1"></a>`REQ-TIME-3-MT1MMF.T1` | [`REQ-TIME-3-MT1MMF`](time.md#req-time-3-mt1mmf) | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | Window values and skew bound are explicit configuration trade-offs; windows dominate honest skew                          | <a id="req-time-3-mt1mmf.t1.p1"></a>`REQ-TIME-3-MT1MMF.T1.P1` — valid case<br><a id="req-time-3-mt1mmf.t1.p2"></a>`REQ-TIME-3-MT1MMF.T1.P2` — before deadline<br><a id="req-time-3-mt1mmf.t1.p3"></a>`REQ-TIME-3-MT1MMF.T1.P3` — zero value<br><a id="req-time-3-mt1mmf.t1.p4"></a>`REQ-TIME-3-MT1MMF.T1.P4` — static review of every named alternative<br><a id="req-time-3-mt1mmf.t1.p5"></a>`REQ-TIME-3-MT1MMF.T1.P5` — direct invalid/opposite case<br><a id="req-time-3-mt1mmf.t1.p6"></a>`REQ-TIME-3-MT1MMF.T1.P6` — at deadline<br><a id="req-time-3-mt1mmf.t1.p7"></a>`REQ-TIME-3-MT1MMF.T1.P7` — after deadline<br><a id="req-time-3-mt1mmf.t1.p8"></a>`REQ-TIME-3-MT1MMF.T1.P8` — maximum honest skew<br><a id="req-time-3-mt1mmf.t1.p9"></a>`REQ-TIME-3-MT1MMF.T1.P9` — exact balance/boundary value<br><a id="req-time-3-mt1mmf.t1.p10"></a>`REQ-TIME-3-MT1MMF.T1.P10` — one beyond boundary<br><a id="req-time-3-mt1mmf.t1.p11"></a>`REQ-TIME-3-MT1MMF.T1.P11` — maximum value<br><a id="req-time-3-mt1mmf.t1.p12"></a>`REQ-TIME-3-MT1MMF.T1.P12` — conservation check<br><a id="req-time-3-mt1mmf.t1.p13"></a>`REQ-TIME-3-MT1MMF.T1.P13` — omitted category<br><a id="req-time-3-mt1mmf.t1.p14"></a>`REQ-TIME-3-MT1MMF.T1.P14` — changed assumption                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| <a id="req-time-4-83v27z.t1"></a>`REQ-TIME-4-83V27Z.T1` | [`REQ-TIME-4-83V27Z`](time.md#req-time-4-83v27z) | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | Timeouts/fraud proofs/slashing use only objectively validated timestamps; subjective assessments gate local behavior only | <a id="req-time-4-83v27z.t1.p1"></a>`REQ-TIME-4-83V27Z.T1.P1` — valid case<br><a id="req-time-4-83v27z.t1.p2"></a>`REQ-TIME-4-83V27Z.T1.P2` — before deadline<br><a id="req-time-4-83v27z.t1.p3"></a>`REQ-TIME-4-83V27Z.T1.P3` — new participant<br><a id="req-time-4-83v27z.t1.p4"></a>`REQ-TIME-4-83V27Z.T1.P4` — malformed input<br><a id="req-time-4-83v27z.t1.p5"></a>`REQ-TIME-4-83V27Z.T1.P5` — direct invalid/opposite case<br><a id="req-time-4-83v27z.t1.p6"></a>`REQ-TIME-4-83V27Z.T1.P6` — at deadline<br><a id="req-time-4-83v27z.t1.p7"></a>`REQ-TIME-4-83V27Z.T1.P7` — after deadline<br><a id="req-time-4-83v27z.t1.p8"></a>`REQ-TIME-4-83V27Z.T1.P8` — maximum honest skew<br><a id="req-time-4-83v27z.t1.p9"></a>`REQ-TIME-4-83V27Z.T1.P9` — existing participant<br><a id="req-time-4-83v27z.t1.p10"></a>`REQ-TIME-4-83V27Z.T1.P10` — removed participant<br><a id="req-time-4-83v27z.t1.p11"></a>`REQ-TIME-4-83V27Z.T1.P11` — slashed participant<br><a id="req-time-4-83v27z.t1.p12"></a>`REQ-TIME-4-83V27Z.T1.P12` — concurrent membership change<br><a id="req-time-4-83v27z.t1.p13"></a>`REQ-TIME-4-83V27Z.T1.P13` — adversarial input<br><a id="req-time-4-83v27z.t1.p14"></a>`REQ-TIME-4-83V27Z.T1.P14` — partial failure<br><a id="req-time-4-83v27z.t1.p15"></a>`REQ-TIME-4-83V27Z.T1.P15` — retry and recovery<br><a id="req-time-4-83v27z.t1.p16"></a>`REQ-TIME-4-83V27Z.T1.P16` — restricted AFK window boundaries: timestamps at `D−1` and `F+1` are invalid, at `D` and `F` valid (both bounds inclusive), with monotonicity against the predecessor<br><a id="req-time-4-83v27z.t1.p17"></a>`REQ-TIME-4-83V27Z.T1.P17` — genesis grace in the AFK anchor: a height-zero slot uses `G = evidenceTime`, later slots use zero, and genesis is checked before any height subtraction<br><a id="req-time-4-83v27z.t1.p18"></a>`REQ-TIME-4-83V27Z.T1.P18` — AFK anchor derivation: `A` comes from the target slot's direct or frozen-tower predecessor credit and the authenticated predecessor calldata posting; a recoverable posting preimage is recovered and re-evaluated before any time claim is rejected<br><a id="req-time-4-83v27z.t1.p19"></a>`REQ-TIME-4-83V27Z.T1.P19` — checked arithmetic: no wrap-around, and a zero-length window falls back explicitly instead of silently extending<br><a id="req-time-4-83v27z.t1.p20"></a>`REQ-TIME-4-83V27Z.T1.P20` — objective timestamp versus local time: an in-range future-dated artifact is not provably early from its bytes, live observers keep local gates, and replay never applies current wall time to old evidence |
