...
| ------------------------------------------------------------------------ | ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `_tx.header.participant`                                                 | injected by `stateTransition`                                       | The logical author of this transition. The **only** valid author identity. For the restricted AFK block the block header's author is the **tower key** and this stays the transition identity the state machine observes — the tower is never made the scheduled participant, never enters membership, and the represented participant is never substituted as the recovered signer. |
...
- **[`REQ-SM-5-3GS7A7`](state-machines.md#req-sm-5-3gs7a7)** — `getNextToWrite()` returns the scheduled participant authorized to author the next
  **block**. The authorization rule is block-level: it constrains who may produce and sign the
  next block on the fork, not who may author an individual transaction inside it. The complete
  author predicate has exactly two cases: an ordinary block authored and signed by that scheduled
  participant, or the restricted AFK block authored and signed by that participant's frozen
  selected tower with the exact canonical removal body and a timestamp inside the restricted
  window ([`REQ-WT-3-DT0GDX`](../runtime/watchtowers.md#req-wt-3-dt0gdx),
  [time.md §5.2](time.md#52-objective-timestamp-validity-fraud-provable)). The schedule itself
  never returns the tower.
...
- **[`REQ-SM-6-BJZVQ5`](state-machines.md#req-sm-6-bjzvq5)** — Turn authorization is a protocol-layer responsibility, enforced generically for
  ...
  in-contract as defense in depth, but the protocol MUST NOT depend on in-contract checks. The
  same complete two-case predicate applies before execution in live intake, local production,
  calldata recovery, dispute replay, synchronization, and on-chain validation; the restricted
  removal operation MUST be inaccessible as an arbitrary application call or unchecked direct
  removal wrapper, and replay receives the same validated context — it cannot consult ambient
  sender, wall time, or mutable live assignment state.
...
The non-punitive exit is triggered by a validated **timeout** (unavailability) or by the
validated restricted AFK block, which routes through this same hook: the AFK action may change
only the target's membership, its removal balance accounting, the schedule derived from that
state, and the resulting canonical exit and commitments — no arbitrary application operation,
no voluntary exit by someone else, no extra withdrawal, and no punishment is authorized, and
deterministic required bookkeeping uses the existing message rules without tower-selected
application effects. The participant leaves with their balance; they are not treated as a
fraudster. Removal is idempotent for the selected state: a target already absent is a no-op that
calls no value-changing hook, adds no exit, advances no outbound tip, and changes no balances,
totals, or schedule; overlapping timeout and self-removal targets deduplicate; and when an
inbound operation legitimately rejoins the target before removals apply, actual membership at the
existing application point decides — never a lifetime "ever removed" flag. Returns
`(bool success, ExitChannel)`.

### 6.3 `_slashParticipant(address)` — punitive removal

The punitive exit for objectively proven fraud. For most offenses the state machine decides how
the penalty is applied to the offender's balance; the one explicit exception is participant
equivocation proved by the same-key `BlockDoubleSign` handler — an explicit signature pair in
any author or confirmation role — which carries the **full channel-stake slash** ([fraud-proofs.md §4](../disputes/fraud-proofs.md#4-the-on-chain-slash-set)) —
the generic application-defined penalty is insufficient for that supported offense. Returns
`(bool success, ExitChannel)`.
