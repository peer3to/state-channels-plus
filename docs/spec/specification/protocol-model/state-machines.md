...
- **[`REQ-SM-8-8CHSQ8`](state-machines.md#req-sm-8-8chsq8)** — Successful `slashParticipant` and `removeParticipant` calls MUST record their resulting
...
**<a id="req-sm-11-vvp01c"></a>`REQ-SM-11-VVP01C` — Application-defined participant insertion.** A consumer MAY expose an
ordinary deterministic transition that adds a participant without an inbound message. If it does, its
authorization and configured roster limit MUST apply, and value assigned to the new participant MUST
come from existing balances so total value and message-chain anchors remain unchanged. Invalid targets,
unauthorized authors, and insufficient funds MUST fail without partially changing the roster or balances.
A valid request at capacity MAY succeed as a roster/balance no-op with the application's normal turn
advance. The protocol does not require every consumer to expose such a transition.

Local application and signature finality are separate from chain adoption. A peer added this way can
participate in the verified off-chain state before it has on-chain dispute standing. The existing chain
eligibility rule remains in force during that window. Prompt publication after the promotion becomes
final is future work; local finality or transaction submission alone does not close the window.
...
**<a id="req-sm-8-8chsq8"></a>`REQ-SM-8-8CHSQ8`.** A successful slash or removal MUST return and record exactly one corresponding `ExitChannel` through `_addExitChannel`. Only successful removal produces an exit; hooks retain their distinct balance semantics. Consumers MUST not deliver the returned and recorded exit twice.
...
| <a id="req-sm-8-8chsq8.t1"></a>`REQ-SM-8-8CHSQ8.T1`   | [`REQ-SM-8-8CHSQ8`](state-machines.md#req-sm-8-8chsq8)   | Execute successful soft-removal and slashing paths and hook/output failures, then inspect state and outbound messages.                                                                                       | Successful paths emit one canonical exit with the balance set by the application hook; failures leave no partial state or message.                   | <a id="req-sm-8-8chsq8.t1.p1"></a>`REQ-SM-8-8CHSQ8.T1.P1` — Zero balance<br><a id="req-sm-8-8chsq8.t1.p2"></a>`REQ-SM-8-8CHSQ8.T1.P2` — existing participant<br><a id="req-sm-8-8chsq8.t1.p3"></a>`REQ-SM-8-8CHSQ8.T1.P3` — hook success<br><a id="req-sm-8-8chsq8.t1.p5"></a>`REQ-SM-8-8CHSQ8.T1.P5` — concurrent membership change<br><a id="req-sm-8-8chsq8.t1.p6"></a>`REQ-SM-8-8CHSQ8.T1.P6` — outbound processing success<br><a id="req-sm-8-8chsq8.t1.p7"></a>`REQ-SM-8-8CHSQ8.T1.P7` — nonzero balance<br><a id="req-sm-8-8chsq8.t1.p8"></a>`REQ-SM-8-8CHSQ8.T1.P8` — maximum balance<br><a id="req-sm-8-8chsq8.t1.p12"></a>`REQ-SM-8-8CHSQ8.T1.P12` — hook failure<br><a id="req-sm-8-8chsq8.t1.p14"></a>`REQ-SM-8-8CHSQ8.T1.P14` — outbound processing failure.<br><a id="req-sm-8-8chsq8.t1.p15"></a>`REQ-SM-8-8CHSQ8.T1.P15` — remove present: state, balance, returned exit and outbound recording <br><a id="req-sm-8-8chsq8.t1.p16"></a>`REQ-SM-8-8CHSQ8.T1.P16` — slash present: state, balance, returned exit and outbound recording <br><a id="req-sm-8-8chsq8.t1.p17"></a>`REQ-SM-8-8CHSQ8.T1.P17` — self-removal dispute delivers one exit and one matching chain withdrawal delta                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| <a id="req-sm-10-jd8tsf.t1"></a>`REQ-SM-10-JD8TSF.T1` | [`REQ-SM-10-JD8TSF`](state-machines.md#req-sm-10-jd8tsf) | Apply slash and removal to absent and previously removed targets, including stale chain membership.                                                                                                          | State, balances, exits and withdrawals remain unchanged.                                                                                             | <a id="req-sm-10-jd8tsf.t1.p1"></a>`REQ-SM-10-JD8TSF.T1.P1` — absent slash leaves state and exits unchanged; <a id="req-sm-10-jd8tsf.t1.p2"></a>`REQ-SM-10-JD8TSF.T1.P2` — absent soft removal leaves state and exits unchanged; <a id="req-sm-10-jd8tsf.t1.p3"></a>`REQ-SM-10-JD8TSF.T1.P3` — local leave with stale chain membership: repeated slash/removal preserves state and withdrawal totals with no new exit <br><a id="req-sm-10-jd8tsf.t1.p4"></a>`REQ-SM-10-JD8TSF.T1.P4` — remove absent: state, balance, returned exit and outbound recording <br><a id="req-sm-10-jd8tsf.t1.p5"></a>`REQ-SM-10-JD8TSF.T1.P5` — remove repeated: state, balance, returned exit and outbound recording <br><a id="req-sm-10-jd8tsf.t1.p6"></a>`REQ-SM-10-JD8TSF.T1.P6` — slash absent: state, balance, returned exit and outbound recording <br><a id="req-sm-10-jd8tsf.t1.p7"></a>`REQ-SM-10-JD8TSF.T1.P7` — slash repeated: state, balance, returned exit and outbound recording                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| <a id="req-sm-11-vvp01c.t1"></a>`REQ-SM-11-VVP01C.T1` | [`REQ-SM-11-VVP01C`](state-machines.md#req-sm-11-vvp01c) | Use an application exposing balance-funded insertion, a funded current author, a fresh target, and a configured roster maximum.                                                                              | Deterministic insertion preserves total balance and message anchors; invalid inputs make no partial change; capacity preserves the roster.           | <a id="req-sm-11-vvp01c.t1.p1"></a>`REQ-SM-11-VVP01C.T1.P1` — valid transfer changes only author/new-member balances and membership; <a id="req-sm-11-vvp01c.t1.p2"></a>`REQ-SM-11-VVP01C.T1.P2` — zero transfer from a funded author; <a id="req-sm-11-vvp01c.t1.p3"></a>`REQ-SM-11-VVP01C.T1.P3` — zero transfer from a zero-balance author; <a id="req-sm-11-vvp01c.t1.p4"></a>`REQ-SM-11-VVP01C.T1.P4` — insufficient balance rejects atomically; <a id="req-sm-11-vvp01c.t1.p5"></a>`REQ-SM-11-VVP01C.T1.P5` — existing target rejects atomically; <a id="req-sm-11-vvp01c.t1.p6"></a>`REQ-SM-11-VVP01C.T1.P6` — self insertion rejects atomically; <a id="req-sm-11-vvp01c.t1.p7"></a>`REQ-SM-11-VVP01C.T1.P7` — zero target rejects atomically; <a id="req-sm-11-vvp01c.t1.p8"></a>`REQ-SM-11-VVP01C.T1.P8` — wrong author rejects atomically; <a id="req-sm-11-vvp01c.t1.p9"></a>`REQ-SM-11-VVP01C.T1.P9` — nonparticipant author rejects atomically; <a id="req-sm-11-vvp01c.t1.p10"></a>`REQ-SM-11-VVP01C.T1.P10` — one remaining slot fills exactly to N; <a id="req-sm-11-vvp01c.t1.p11"></a>`REQ-SM-11-VVP01C.T1.P11` — valid at-capacity request advances only the normal turn; <a id="req-sm-11-vvp01c.t1.p12"></a>`REQ-SM-11-VVP01C.T1.P12` — already oversized roster does not grow; <a id="req-sm-11-vvp01c.t1.p13"></a>`REQ-SM-11-VVP01C.T1.P13` — exact remaining balance may be transferred; <a id="req-sm-11-vvp01c.t1.p14"></a>`REQ-SM-11-VVP01C.T1.P14` — off-chain promotion gains chain dispute standing only after actual snapshot adoption; <a id="req-sm-11-vvp01c.t1.p15"></a>`REQ-SM-11-VVP01C.T1.P15` — unpublished newcomer cannot start a timeout dispute but can resume verified history; <a id="req-sm-11-vvp01c.t1.p16"></a>`REQ-SM-11-VVP01C.T1.P16` — full-capacity request does not promote a connected spectator; <a id="req-sm-11-vvp01c.t1.p17"></a>`REQ-SM-11-VVP01C.T1.P17` — verified history promotes the inserted peer while authoritative chain membership is still unchanged; <a id="req-sm-11-vvp01c.t1.p18"></a>`REQ-SM-11-VVP01C.T1.P18` — existing chain-eligible participants still initiate a real timeout dispute |
