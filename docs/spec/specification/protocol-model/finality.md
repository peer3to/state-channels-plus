...
- On-chain historic threshold (dispute last-milestone probe, no posted data): the dispute commits the
  set. It is the participants of the dispute's latest state, plus the joiners the chain recorded between that
  state's inbound hash and the dispute's committed inbound anchor, minus the dispute's own `onChainSlashes`.
  The fraud prover supplies that latest-state snapshot and the chain checks it against the dispute's commitment:
  its `latestStateSnapshotHash`, or, for a state-proof fraud proof that shows that hash is inconsistent with the
  proof, the snapshot its latest proven block commits. No live snapshot is read, so no adoption, join, top-up or
  slash landing after commitment moves the verdict. The disputer picks the slash list from what it observed.
...
The on-chain formulas are its acceptance rules for such proofs, not additional agreement
thresholds:
...
- **Slash subtraction is a current-eligibility rule, never a historical one.** Subtracting the
  ...
  not an exception: it subtracts only the slashes the dispute commits in its own input, so its verdict is
  fixed by the dispute and replays the same at any later time.
...
**[`REQ-FIN-7-RTZWQZ`](finality.md#req-fin-7-rtzwqz).** The finality threshold is unanimous over the relevant union participant set (minus on-chain-slashed for live dispute decisions, minus the dispute's own listed slashes for the historic threshold).
