...
- On-chain historic threshold (dispute last-milestone probe, no posted data): the dispute commits the
  ...
  observed. Listing a slash the chain does not hold is provable fraud (`DisputeOnChainSlashesNotSubset`), and
  leaving one out only makes the threshold stricter. The live slash set is never read, so a slash landing
  at any time moves the verdict only through the committed list. The participant half comes only from data the
  ...
  ([`REQ-DIS-2-PKVZ7E`](../disputes/disputes.md#req-dis-2-pkvz7e)). Unlike the off-chain threshold, this set has an
