...
_Design note (non-normative)._ Optional participant-selected delegation with this broader authority
was chosen to maximize real-time off-chain progress and settlement and to minimize the on-chain
fallback; the price is greater trust in the one delegate each participant selects. Independent
observation and permissionless proof submission remain open to everyone but confer none of that
delegated authority. One tower may serve many participants; the protocol mandates no participant
service payment, providers may charge, and nothing here is an economic-sustainability guarantee.
On-chain complaint-and-penalty schemes request missing contributions on chain and, when a required
contribution stays missing, halt into a penalty path instead of continuing; paid-custodian schemes
make the delegate liable through a deposit and treat compensation as a separate extension. This
design instead keeps the offline participant's slot, dispute, and finality effects moving through
the ordinary protocol under its selected delegate's narrow authority, as a deliberate liveness
versus trust trade-off — not a claim of superiority over those designs or an adoption of their
guarantees. Anonymous sampled delegation stays future work.
...
**ordinary** timeout its own credit defeats — plus, for delegated submissions, the one
order-sensitive predicate of [`REQ-WT-11-DZ3AAE`](watchtowers.md#req-wt-11-dz3aae): a tower that
provably acknowledged newer state before committing its participant's stale dispute. Everything
else — silence, refusal, late delivery —
...
participant's availability. The tower's view is a spectator's view — the channel state and gossiped
messages any spectator receives, nothing more; confidentiality of application data is the
application's responsibility, and this delegation neither adds nor removes any such property (an
application whose private inputs are encrypted before they enter channel state, as in the poker
design, keeps them private from every spectator including a shared tower until the application
itself discloses them). It prunes that channel's data and leaves after settlement; long-lived
...
efficiency policy, not a correctness rule. A delegated tower MUST submit the latest state it
knows. A negligent or malicious delegated submission is a service failure inside the selection
trust boundary: its dispute-game consequences fall on the represented participant, whose remedies
are its prior tower choice, any objective equivocation evidence, and a different selection in later
joins; the protocol pays no compensation — with the single attribution exception of the
not-latest-state predicate, owned by [`REQ-WT-11-DZ3AAE`](watchtowers.md#req-wt-11-dz3aae): a
tower-authored stale dispute does not expose the participant merely because the participant signed
newer data the tower did not know. This
...
**<a id="req-wt-11-dz3aae"></a>`REQ-WT-11-DZ3AAE` — Delegated stale-state attribution.** Every
committed and signed dispute carries an explicit delegation field recording whether the named
disputer authored it directly or its frozen selected tower authored it
([`REQ-DIS-15-GH01J0`](../disputes/disputes.md#req-dis-15-gh01j0)); the field is authenticated
against the actual signature and the historical tower binding, and a self-declared value never
bypasses a penalty. Authorship follows the recovered signer under the central key policy of
[identity.md](../protocol-model/identity.md#identity): when the actual signer equals the named
participant, the dispute is directly authored and ordinary participant stale-state liability
applies, even if that same address is the participant's own selected tower or the signed
delegation mode claims tower authorship; the partition exception below applies only to a distinct
authorized selected-tower signer, and a participant acting as another participant's selected
tower is such a distinct signer for that other participant's disputes. The represented
participant remains the logical disputer for every other purpose. For the not-latest-state predicate specifically
([fraud-proofs.md §3](../disputes/fraud-proofs.md)): a newer block signed by the participant
exposes the participant to slashing only when the participant authored the stale dispute directly;
tower authorship MUST NOT transfer that liability to the participant merely because it signed newer
data the tower did not know. A delegated tower MUST submit the latest state it knows: provable
prior acknowledgement of newer state followed by a stale delegated submission is tower misconduct
answered by the tower's bond and eligibility only — the participant is not added to the channel
slash set for it, and no external tower penalty is routed through participant funds. An honest
partition without provable prior tower knowledge produces no stale-state slash for either actor,
and an eligible participant may submit a newer-state counter-dispute so reduction uses the latest
valid history, subject to the admission limits that remain open. This rule waives no other fraud
predicate and changes no other liability, does not decide the ordering question of
[`OQ-1-NTJBA1`](../open-questions.md#oq-1-ntjba1), and does not decide the stale commitment's
fate; the prior-knowledge ordering evidence, counter-dispute admission after a consumed slot or
expired window, the stale commitment's fate, and the field's exact shape are
[`OQ-53-81N5J4`](../open-questions.md#oq-53-81n5j4).
...
- `WatchtowerDisputeApproval` — binds the selected tower, its offline participant, the exact
  signed-dispute hash, and the exact canonical on-chain snapshot reference the tower relied on;
  the tower's authority for the dispute derives from the accepted join of the participant's
  relevant membership interval. It asserts that the tower independently audited that specific
  dispute — including its complete mode-defining evidence, latest proof, and output — against
  that snapshot, and approves threshold finality in place of its participant's signature only
  while that snapshot is still the channel's current on-chain snapshot at execution
...
artifact whose signer is not the tower established for the relevant membership interval; an
approval whose snapshot reference is no longer current is not rejected as malformed but contributes
no credit. These bindings carry no chain, manager, or version field: the chosen design assumption
that channel and fork identifiers are unique across the relevant deployments is recorded in
[data-types.md §7.5](../protocol-model/data-types.md#75-watchtower-evidence), and the general
signed-domain policy stays open under [`OQ-29-EFY4NF`](../open-questions.md#oq-29-efy4nf). This artifact
...
they promptly attempt a snapshot publication with the valid proof and transaction data through the
permitted settlement route of [`REQ-LIF-8-GXAGXR`](../settlement/lifecycle.md#req-lif-8-gxagxr):
an ordinary same-fork publication while their fork carries no dispute commitment, or, once it does,
adoption of the resolved undisputed successor through its verified reduction links — never a
same-fork update to a frozen destination. A finalized predecessor, an unrelated or conflicting block, an equal-height or
...
never become one signer through credit derivation. Beside them stands one delegation-specific
predicate that is not a contradiction pair: a tower that provably acknowledged newer state before
committing its participant's stale delegated dispute
([`REQ-WT-11-DZ3AAE`](watchtowers.md#req-wt-11-dz3aae)); it reaches the bond and eligibility only,
its timing evidence is [`OQ-53-81N5J4`](../open-questions.md#oq-53-81n5j4), and like every
external tower proof it never slashes participant funds. **Normal acknowledgement versus conflicting AFK
...
protocol-punishable tower misconduct is objective evidence of the three order-independent
contradiction classes ([`INV-WT-1-ST9SHX`](watchtowers.md#inv-wt-1-st9shx)) or of the separate
order-sensitive stale-submission predicate
([`REQ-WT-11-DZ3AAE`](watchtowers.md#req-wt-11-dz3aae), whose ordering evidence is
[`OQ-53-81N5J4`](../open-questions.md#oq-53-81n5j4)), enforced through a punishment entry point
...
block or artifact creates no bond offense beyond those contradiction classes and that predicate.
The punishment does
...
- Delegated artifacts rely on channel, fork, height, participant, and signer bindings only. The
  chosen design assumption that channel and fork identifiers are unique across the relevant
  deployments is recorded in
  [data-types.md §7.5](../protocol-model/data-types.md#75-watchtower-evidence); it is not a verified
  cross-deployment replay guarantee, and the signed-domain policy stays open under
...
- **Dispute approval** finalizes the complete output of one audited timeout dispute with no later
  kill-period challenge, but only while the snapshot it was audited against is still current — a
  checkpoint that settled first turns the approval into an authentic no-credit artifact; the
  dispute then finalizes immediately if the other valid credits still reach threshold, and
  otherwise remains an ordinary challengeable commitment when ordinary admission succeeds; an offline participant whose tower colludes with the remaining peers has
...
- **The restricted AFK block** removes its represented participant without their signature. The
  ...
  removal is non-punitive in the protocol sense — no protocol slash: the participant leaves through
  the common exit path with its held balance disposed by the state machine's soft-removal hook
  ([state-machines.md §6.2](../protocol-model/state-machines.md#62-_removeparticipantaddress--soft-removal)),
  and any application consequence of leaving mid-play (a folded hand, a forfeited pot) is the
  application's own rule that this delegation neither promises to prevent nor adds; a fully
  unavailable tower cannot author anything and only degrades the service to the ordinary fallback
  path. The participant's remedy is its prior tower choice, the
...
| <a id="req-wt-11-dz3aae.t1"></a>`REQ-WT-11-DZ3AAE.T1` | [`REQ-WT-11-DZ3AAE`](watchtowers.md#req-wt-11-dz3aae) | Author a newer valid block (illustratively 51 after 50) that every other participant receives while the author's selected tower is genuinely partitioned and knows only the previous state; have the tower, or the participant directly, commit a stale dispute; vary prior tower acknowledgement, ordering evidence, delegation-field authenticity, and the counter-dispute route.                                                                                                                                                                                                | Attribution follows authorship: a direct stale dispute exposes the participant to the not-latest-state predicate, an authenticated tower-authored stale dispute never does; only provable prior tower knowledge reaches the tower's bond and never participant funds; an honest partition slashes nobody and the newer history still wins through an eligible counter-dispute; forged delegation is rejected without a flag-based bypass; every other predicate keeps its behavior.                                                                                                                                                                                                                                                                                           | <a id="req-wt-11-dz3aae.t1.p1"></a>`REQ-WT-11-DZ3AAE.T1.P1` — honest partition with a newer-state counter-dispute (system scenario through real delivery isolation, not knowledge mocks): the partitioned tower submits an otherwise admissible stale delegated dispute with no prior acknowledgement of the newer block, another eligible participant with an unused slot submits a valid counter-dispute carrying the newer history inside the evidence window, reduction selects the newer checkpoint-compatible history into the successor with its block effects preserved, the participant does not enter the channel slash set, and the tower's bond is untouched; <a id="req-wt-11-dz3aae.t1.p2"></a>`REQ-WT-11-DZ3AAE.T1.P2` — direct stale dispute plus the participant's qualifying newer signed block: the existing participant stale-state liability applies when the complete predicate holds; <a id="req-wt-11-dz3aae.t1.p3"></a>`REQ-WT-11-DZ3AAE.T1.P3` — authenticated tower-authored stale dispute with only the participant's signature on the unknown newer block: no stale-state slash of the participant or the tower; <a id="req-wt-11-dz3aae.t1.p4"></a>`REQ-WT-11-DZ3AAE.T1.P4` — provable tower acknowledgement of the newer block before the stale commitment: tower bond and eligibility only, no participant channel slash and no participant-fund penalty; <a id="req-wt-11-dz3aae.t1.p5"></a>`REQ-WT-11-DZ3AAE.T1.P5` — tower acknowledgement after the commitment, or evidence whose ordering is unproved: no inference of prior knowledge and no stale-state tower penalty on that basis; <a id="req-wt-11-dz3aae.t1.p6"></a>`REQ-WT-11-DZ3AAE.T1.P6` — forged delegation, signature/field mismatch, unauthorized or wrong historical tower, wrong channel or fork, and invalid receipt evidence: rejected without any flag-based penalty bypass; <a id="req-wt-11-dz3aae.t1.p7"></a>`REQ-WT-11-DZ3AAE.T1.P7` — newer counter-dispute before and after the shared slot is consumed and at the evidence-period boundaries, with expectations fixed only once the admission question of [`OQ-53-81N5J4`](../open-questions.md#oq-53-81n5j4) is resolved; <a id="req-wt-11-dz3aae.t1.p8"></a>`REQ-WT-11-DZ3AAE.T1.P8` — immutable stale output and commitment handling, with expectations fixed once that question is resolved and checked for compatibility with the settled-history rules of [`REQ-LIF-8-GXAGXR`](../settlement/lifecycle.md#req-lif-8-gxagxr); <a id="req-wt-11-dz3aae.t1.p9"></a>`REQ-WT-11-DZ3AAE.T1.P9` — every other fraud predicate retains its established behavior, and tower-bond and participant-channel liability remain separate through proof submission and application. ; <a id="req-wt-11-dz3aae.t1.p10"></a>`REQ-WT-11-DZ3AAE.T1.P10` — same-key self-selection: the participant selected its own address as tower and its own key signs the stale dispute with the delegation mode set to tower authorship; the dispute is treated as directly authored, its newer signed block satisfies the ordinary participant predicate, and no partition exception applies; <a id="req-wt-11-dz3aae.t1.p11"></a>`REQ-WT-11-DZ3AAE.T1.P11` — distinct-signer delegation: one participant acting as another participant's frozen selected tower submits that other participant's stale dispute; the exception applies for the represented participant, while the tower-acting participant's own disputes and liabilities are unaffected                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| <a id="req-wt-3-dt0gdx.t1"></a>`REQ-WT-3-DT0GDX.T1`   | [`REQ-WT-3-DT0GDX`](watchtowers.md#req-wt-3-dt0gdx)   | Present each artifact — receipt, restricted AFK block, and approval — with correct and with mismatched channel, fork, height, participant, membership-interval, signer, body, and block bindings, including malformed encodings, duplicates, and shared-tower credit derivations (the overlapping-signer credit cases are [`INV-ID-1-B4FXJ4.T1.P9`](../protocol-model/identity.md#inv-id-1-b4fxj4.t1.p9), [`INV-ID-1-B4FXJ4.T1.P10`](../protocol-model/identity.md#inv-id-1-b4fxj4.t1.p10), and [`INV-ID-1-B4FXJ4.T1.P11`](../protocol-model/identity.md#inv-id-1-b4fxj4.t1.p11)). | Exactly-bound artifacts verify; availability credits derive deterministically from the membership intervals' accepted-join bindings and deduplicate; only the exact canonical AFK removal executes; every mismatched, malformed, replayed, or duplicate presentation proves nothing and changes nothing for the presented slot.                                                                                                                                                                                                                                                                                                                                                                                                                                               | <a id="req-wt-3-dt0gdx.t1.p1"></a>`REQ-WT-3-DT0GDX.T1.P1` — valid receipt, restricted AFK block, and approval encodings verify; <a id="req-wt-3-dt0gdx.t1.p2"></a>`REQ-WT-3-DT0GDX.T1.P2` — wrong channel/fork/height binding rejected; <a id="req-wt-3-dt0gdx.t1.p3"></a>`REQ-WT-3-DT0GDX.T1.P3` — wrong participant or tower rejected, including a tower established only for a different membership interval; <a id="req-wt-3-dt0gdx.t1.p4"></a>`REQ-WT-3-DT0GDX.T1.P4` — wrong exact-block or dispute-hash binding rejected; <a id="req-wt-3-dt0gdx.t1.p5"></a>`REQ-WT-3-DT0GDX.T1.P5` — replay for another slot or dispute rejected; <a id="req-wt-3-dt0gdx.t1.p6"></a>`REQ-WT-3-DT0GDX.T1.P6` — duplicate delivery is idempotent; <a id="req-wt-3-dt0gdx.t1.p7"></a>`REQ-WT-3-DT0GDX.T1.P7` — participant signature credits only that participant's availability; <a id="req-wt-3-dt0gdx.t1.p8"></a>`REQ-WT-3-DT0GDX.T1.P8` — one shared-tower signature credits every delegated eligible participant's availability; <a id="req-wt-3-dt0gdx.t1.p9"></a>`REQ-WT-3-DT0GDX.T1.P9` — with a recovered tower signer that is not itself a channel participant, one tower signature satisfies the full availability threshold when every eligible participant delegated to it, while supplying zero finality votes; <a id="req-wt-3-dt0gdx.t1.p10"></a>`REQ-WT-3-DT0GDX.T1.P10` — tower credit and the same participant's direct acknowledgement deduplicate to one availability credit; <a id="req-wt-3-dt0gdx.t1.p11"></a>`REQ-WT-3-DT0GDX.T1.P11` — malformed, truncated, or trailing-byte AFK block bytes are rejected everywhere with no effect; <a id="req-wt-3-dt0gdx.t1.p12"></a>`REQ-WT-3-DT0GDX.T1.P12` — wrong tag, arbitrary selector, extra call, extra target, or extra message in the AFK body is rejected; <a id="req-wt-3-dt0gdx.t1.p13"></a>`REQ-WT-3-DT0GDX.T1.P13` — an AFK block whose resulting state is anything other than the exact non-punitive removal of the bound target is rejected; <a id="req-wt-3-dt0gdx.t1.p14"></a>`REQ-WT-3-DT0GDX.T1.P14` — tower gossip, peer validation, and timeout calldata decode the same AFK block bytes and reach the same verdict; <a id="req-wt-3-dt0gdx.t1.p15"></a>`REQ-WT-3-DT0GDX.T1.P15` — approval snapshot binding: an approval verifies only with the exact snapshot reference it signed, a reference that no longer matches the current on-chain snapshot yields no credit without invalidating the artifact, and altering the reference invalidates the signature.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
