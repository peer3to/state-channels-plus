...
**<a id="req-config-3-j4h12f"></a>`REQ-CONFIG-3-J4H12F` — Declared bounds, enforced exactly.** Timing, memory, concurrency, payload,
retry, and persistence values MUST be rejected when they fall outside their declared range, rather
than silently clamped to a behavior peers cannot predict. A value inside the declared range MUST be
accepted as given, a value outside it MUST be rejected at startup naming the offending key, and no
value may be silently substituted. Rejection MUST happen before the node signs anything or mutates
durable state.

**Choosing a safe range is the deployment's responsibility, not the protocol's.** This requirement
is deliberately named for what it does: it enforces a declared range exactly. It cannot certify that
the range is itself safe, because the ranges are deployment-supplied and this document fixes no
minima or maxima — so conformance here is evidence that a node matches its operator's intent, and
is not evidence that the resulting timing or resource behavior is sound. An operator who declares a
dangerously short window gets a node that starts and honors it. Empirically validated profiles that
would let the protocol assert real minima are Future Work.

Two consequences worth stating plainly. No permutation can pin a specific bound; each is judged
against the range the deployment declares. And a configuration that is individually in range for
every key may still be incoherent across keys or against the chain that enforces it —
[`REQ-CONFIG-2-JA2SKN`](configuration.md#req-config-2-ja2skn) owns that compatibility, not this
requirement.
