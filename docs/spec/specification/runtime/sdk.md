...
surface and report it once when the participant is disposed. Only transactions that mined count, and each of them MUST
contribute to its key's transaction count, total, minimum, and maximum cost; a mined transaction that reverted MUST also be
counted as reverted, since it still cost what it burned. A transaction that never mines MUST NOT be counted, and a
transaction that was never sent MUST NOT appear at all. Observing cost MUST NOT change a sender's result, its ordering, or
