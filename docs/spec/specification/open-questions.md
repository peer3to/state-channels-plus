...
<a id="oq-spec-leave-1-9q4bv3"></a>

## OQ-SPEC-LEAVE-1-9Q4BV3 — Scope of peer exclusion across a channel change

A runtime that leaves a channel is now reusable, which creates a question the terminal-leave model never
had to answer: what happens to the exclusions a node accumulated while it was in the channel it just left.
The protocol specifies that an exclusion requires proof of an attributable fault
([`REQ-AUTH-4-JWCF71` (Penalty requires proof)](peer-communication/handshake.md#req-auth-4-jwcf71)) and
that it survives connection churn, but it does not say whether the exclusion is scoped to the channel whose
traffic produced the proof or to the identity for as long as the runtime lives.

Both readings are defensible. Channel-scoped: the fault was proven inside one participant set against one
fork's rules, the next channel has its own participant set, and carrying the exclusion forward refuses a
peer for something the new channel never saw — and, because exclusions are not persisted anyway, the same
peer would be re-admitted after any restart, so keeping it across a channel change buys little.
Identity-scoped: a proven protocol violation is evidence about the identity, not about the channel, and
dropping it lets a misbehaving peer regain a clean slate simply by waiting for the victim to change
channels, which is cheap for the attacker.

Requested decision: state in the specification whether exclusions are channel-scoped or identity-scoped
across a channel change, and, if identity-scoped, what their lifetime and reevaluation rules are. This
interacts with [`OQ-45-ACZCDE`](open-questions.md#oq-45-aczcde), which owns the broader subjective
engagement policy, and with the unresolved ban-persistence part of
...
[`REQ-AUTH-4-JWCF71.T1.P7`](peer-communication/handshake.md#req-auth-4-jwcf71.t1.p7); the current
implementation already conforms.

<a id="oq-spec-lobby-1-d65ytt"></a>

## OQ-SPEC-LOBBY-1-D65YTT — Deferred-admission queue bound

The two-agreement-window admission grace permits authenticated requests to wait while local service
readiness catches up. The current protocol does not define a per-transport queue cap or the exact
overflow result. A later hardening decision must choose a small bound, decide whether overflow is a
request rejection, transport disconnect, or identity exclusion, and state how already queued requests
settle. Until then, implementations must preserve the bounded wait duration and must not claim a bounded
message count for pre-readiness traffic.
...
## OQ-45-ACZCDE — Subjective post-authentication engagement policy
...
behavior; reevaluation triggers and outcome changes; persistence across reconnects or process
restarts; and the relationship between rejection, connection termination, and identity exclusion.
Candidate scenarios (non-normative, not accepted oracles): one node accepts and another rejects the
same proven identity without contradiction; a node ends later interaction without erasing the proof
result; the policy is unavailable or errors and the node must choose between continued interaction,
