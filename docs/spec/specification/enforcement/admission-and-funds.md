...
| Top-up                                        | The existing, unslashed participant itself                                                              | Same expiry/pin checks; participant **is** in snapshot ∪ pending and is not on-chain-slashed; fork not under dispute; countersignatures cover the same current eligibility threshold                                                                                                                                                                                   | Deposit; appended inbound `JOIN` block (balance semantics per the state machine's join-or-top-up rule, [state-machines.md](../protocol-model/state-machines.md)).    |
...
snapshot ∪ pending; top-up requires it present and not on-chain-slashed. Both MUST be refused while
the fork is disputed. A slashed participant remains recorded as a member but MUST NOT top up. The two cases MUST NOT be
