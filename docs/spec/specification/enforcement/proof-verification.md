...
| Milestone finality          | Direct and virtual finality per milestone in actual participant votes with the target-only AFK credit, finalized membership-union hops, and exact checkpoint rebasing of settled prefixes ([state-proofs.md](../disputes/state-proofs.md), [finality.md](../protocol-model/finality.md)) | Same-fork snapshot advance; dispute audit; sync.                                                                                    |
| Block linkage and structure | Hash linkage plus two-case author signatures (scheduled participant, or the frozen tower's restricted AFK block) on the non-final suffix; per-block structural validity with first-invalid-index reporting                                                                               | State-proof verification; dispute fraud proofs.                                                                                     |
...
**Carrier exclusivity (settled).** The state-proof checks accept a proof carrying milestones _or_
trailing signed blocks and reject one carrying both; on the milestone path the non-final suffix —
which may contain the AFK removal — rides inside the **last** milestone's confirmations as its
unfinalized tail ([state-proofs.md](../disputes/state-proofs.md)). Every empty, raw-block,
milestone, last-tail, and malformed-proof branch is additionally judged against the trusted-base
checkpoint predicate of [`REQ-SP-7-70EMAT`](../disputes/state-proofs.md#req-sp-7-70emat): the
first retained block's channel, fork, height, exact parent, and author validate, then every
continuation link — no fast path validates only one skipped crossing or the last milestone's
finality, missing bridge bytes route to recovery, and a different committed checkpoint or broken
link is invalid.
