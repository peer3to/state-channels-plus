...
`requireExistingDisputeWindow = true` requires the same channel/fork window to exist, remain
unfinalized, and remain inside its evidence period before admission changes any window state.
Otherwise submission fails with `RaceConditionDisputeWindowNotOpen`, including an expired window
