...
The suite covers `dispute.input.selfRemoval` from both sides. The valid case arms `forceExit` on
peer 1 through the shared `DisputeOrchestrator.selfRemove` action, which submits through the runtime and records its signing marker: the
