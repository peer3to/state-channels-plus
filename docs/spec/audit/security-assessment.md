...
- **Exclusions survive the change of channel, by decision.** The scope question is resolved (engineer
  ...
  [`REQ-AUTH-4-JWCF71.T1.P7`](../specification/peer-communication/handshake.md#req-auth-4-jwcf71.t1.p7).
...
- **No exclusion is recorded while the release itself is running.** This is the exclusion decision's own
  ...
  release-in-progress flag beside the generation bump and lowers it in a `finally`, and the disconnect policy
  switch downgrades every penalty (verdict, suspension, or strike) to a plain close while it is up. The residual exposure is the mirror image and is
