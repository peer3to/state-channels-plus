...
> **Scope:** Objective fraud-proof enforcement: the block fraud-proof family, the dispute
> fraud-proof family, the on-chain slash set they feed, and the separation of this mechanism from
> the dispute game specified in [disputes.md](./disputes.md).

Implementation entry points:
[`FraudProofFacet`](../../../../contracts/V1/StateChannelDiamondProxy/FraudProofFacet.sol) (block
proofs), [`DisputeFraudProofFacet`](../../../../contracts/V1/StateChannelDiamondProxy/DisputeFraudProofFacet.sol)
(dispute proofs), slash-set primitives in
[`StateChannelCommon`](../../../../contracts/V1/StateChannelDiamondProxy/StateChannelCommon.sol).
Proof types: [`ProofTypes.sol`](../../../../contracts/V1/types/ProofTypes.sol) (enums and
envelopes), with per-type payload structs in `FraudProofTypes.sol` / `DisputeFraudProofTypes.sol`
under [`contracts/V1/types`](../../../../contracts/V1/types). Off-chain builders:
[`FraudProofService`](../../../../src/stateManager/utils/FraudProofService.ts),
[`DisputeFraudProofService`](../../../../src/stateManager/utils/DisputeFraudProofService.ts),
driven by the [block-confirmation pipeline](../sdk/block-confirmation-pipeline.md) and the
[dispute pipeline](../sdk/dispute-pipeline.md) through the strategies in
[`src/stateManager/validationStrategy/`](../../../../src/stateManager/validationStrategy).
...
([../concepts/state-machines.md](../concepts/state-machines.md)) — on-chain re-execution is the
ground truth for transition proofs; canonical serialization of blocks and snapshots
([../concepts/history-and-commitments.md](../concepts/history-and-commitments.md)); the
[chain-time model](./time.md) for every timestamp rule; chain and RPC availability per
[trust-model.md](../security/trust-model.md) so an honest observer can submit within the relevant
windows.

## 2. Block fraud proofs

Dispatched by [`FraudProofFacet.runFraudProof`](../../../../contracts/V1/StateChannelDiamondProxy/FraudProofFacet.sol)
to a per-type handler that returns the address to slash, or `address(0)` when the proof does not
hold. `FraudProofType` (verified against the enum in
[`ProofTypes.sol`](../../../../contracts/V1/types/ProofTypes.sol) — five members):
...
[`DisputeFraudProofFacet.applyDisputeFraudProofs`](../../../../contracts/V1/StateChannelDiamondProxy/DisputeFraudProofFacet.sol);
...
| `DisputeInvalidBalanceInvariant`                 | referenced a latest finalized state that violates the channel-balance invariant (`verifyBalanceInvariantCheckSnapshot`; see [../concepts/state-machines.md](../concepts/state-machines.md) and [cross-layer-messages.md](./cross-layer-messages.md)).                                                                                                                                            |
...
Storage: `DisputeData.onChainSlashes` — an append-only, per-channel array of
`OnChainSlash { participant, timestamp }`
([`DisputeTypes.sol`](../../../../contracts/V1/types/DisputeTypes.sol)).

- **Ownership & update rule (`REQ-FP-3`).** Only contract code appends, exclusively through
  `addOnChainSlashedParticipant`
  ([`StateChannelCommon`](../../../../contracts/V1/StateChannelDiamondProxy/StateChannelCommon.sol)):
  deduplicated, stamped with `block.timestamp`, `ChainSlashed` event emitted. Writers:
  `applyFraudProofs` (successful block proof, or self-slash §5), `applyDisputeFraudProofs`
  (failed dispute proof → submitter), `killDispute` (killed dispute → disputer), and
  `challengeDisputeReduction` (wrong reducer or failed challenger; currently unreachable — see
  [disputes.md §4.3](./disputes.md)).
...
- **State-machine effect.** Slashes take effect on balances and membership only when a reduction
  ...
  ([../contracts/state-machine-base.md](../contracts/state-machine-base.md)).
- **Lifetime.** `Current:` the set is deleted in `_clearDisputeData` when channel storage is
...
`Current:` submission behavior as implemented:
...
- `applyFraudProofs(fraudProofs[], context)` is a public entry point on the proxy with **no
  ...
  participant).
- **Self-slashing guard (`Current:`).** In `applyFraudProofs`, if a submitted proof does not
...
- **SDK path.** The confirmation pipeline stores discovered proofs
  ([`FraudProofService`](../../../../src/stateManager/utils/FraudProofService.ts) →
  `storage.fraudProofs`) keyed by offender. `Current:` the SDK submits them on-chain bundled with
  its next dispute upload in a single `multicall` (`applyFraudProofs` + `uploadDispute*` in
  [`DisputeManager.dispute`](../../../../src/disputeManager/DisputeManager.ts)) rather than as a
  ...
  self-slash guard ([`DisputeValidationService`](../../../../src/stateManager/DisputeValidationService.ts)).

**Open question (`REQ-FP-6`):** whether and how a sender SHOULD be penalized for submitting an
invalid fraud proof is unresolved design. The self-slashing guard above is what the code does; it
...
[../security/open-security-review.md](../security/open-security-review.md); until it closes, this
...
- **Pipeline e2e:**
  ...
  [`E2E-ByzantineErrorAttribution.test.ts`](../../../../test/e2e/E2E-ByzantineErrorAttribution.test.ts)
  (attribution of Byzantine failures), and the dispute fraud-proof soundness matrix in
  [`test/e2e/disputeValidation/`](../../../../test/e2e/disputeValidation) (state-proof cases,
  input-field cases, not-latest-state, output-state, balance-invariant, inbound-hash).
- **Adversarial fuzzing:** [`E2E-Fuzz-Dispute-MVP.test.ts`](../../../../test/e2e/E2E-Fuzz-Dispute-MVP.test.ts)
  with [`test/e2e/fuzz/disputeSoundnessMenu.ts`](../../../../test/e2e/fuzz/disputeSoundnessMenu.ts).
...
- Extract the remaining implicit dispute-validity checks into dedicated fraud proofs (the code
  marks genesis-fork linking, signed-block linking, and latest-state checks inside
  `DisputeInvalidStateProof` as candidates for splitting).
...
| -------- | -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| REQ-FP-1 | Design pending | Fraud-proof enforcement is separate from the dispute game: an observed objective violation can be proven immediately, with a binary prove-and-slash / no-slash outcome; disputes consume recorded slashes instead of re-executing proofs. | [`FraudProofFacet.applyFraudProofs`](../../../../contracts/V1/StateChannelDiamondProxy/FraudProofFacet.sol); consumption in [`DisputeVerificationFacet.reduce`](../../../../contracts/V1/StateChannelDiamondProxy/DisputeVerificationFacet.sol)                     | [`E2E-FraudProofsBlockConfirmation.test.ts`](../../../../test/e2e/E2E-FraudProofsBlockConfirmation.test.ts), [`test/e2e/disputeValidation/disputeInputFields/onChainSlashes.test.ts`](../../../../test/e2e/disputeValidation/disputeInputFields/onChainSlashes.test.ts)                                                                                                       |
| REQ-FP-2 | Design pending | Every block fraud-proof handler is sound: a passing proof implies a real violation by the returned address (types: `BlockDoubleSign`, `BlockInvalidStateTransition`, `WrongGenesis`, `InvalidTimestamp`, `ForgedInboundMessageBlock`).    | handlers in [`FraudProofFacet`](../../../../contracts/V1/StateChannelDiamondProxy/FraudProofFacet.sol)                                                                                                                                                              | [`FraudProofFacet.t.sol`](../../../../test/V1/StateChannelDiamondProxy/FraudProofFacet.t.sol) (`InvalidTimestamp` properties), [`DisputeVerificationFacet.t.sol`](../../../../test/V1/StateChannelDiamondProxy/DisputeVerificationFacet.t.sol) (`BlockInvalidStateTransition`); `BlockDoubleSign`/`WrongGenesis`/`ForgedInboundMessageBlock` facet-boundary tests: none — gap |
| REQ-FP-3 | Design pending | Slashes are recorded only via `addOnChainSlashedParticipant`: per-channel, append-only, deduplicated, timestamped, evented.                                                                                                               | [`StateChannelCommon.addOnChainSlashedParticipant`](../../../../contracts/V1/StateChannelDiamondProxy/StateChannelCommon.sol)                                                                                                                                       | [`DisputeVerificationFacet.t.sol`](../../../../test/V1/StateChannelDiamondProxy/DisputeVerificationFacet.t.sol) (`getOnChainSlashedParticipantsUpToTimestamp` prefix test)                                                                                                                                                                                                    |
| REQ-FP-4 | Design pending | A recorded slash disqualifies the participant from dispute participation and from the on-chain threshold set.                                                                                                                             | [`canParticipateInDisputes`, `getOnChainThresholdSet`](../../../../contracts/V1/StateChannelDiamondProxy/StateChannelCommon.sol)                                                                                                                                    | [`E2E-StaleMembershipDispute.test.ts`](../../../../test/e2e/E2E-StaleMembershipDispute.test.ts); direct disqualification unit test: none — gap                                                                                                                                                                                                                                |
| REQ-FP-5 | Design pending | A dispute may list any subset of recorded slashes; listing an unrecorded slash is disprovable (`DisputeOnChainSlashesNotSubset`), and the reduction independently consumes all slashes recorded up to window expiry.                      | [`DisputeFraudProofFacet._handleDisputeOnChainSlashesNotSubset`](../../../../contracts/V1/StateChannelDiamondProxy/DisputeFraudProofFacet.sol), [`DisputeVerificationFacet.reduce`](../../../../contracts/V1/StateChannelDiamondProxy/DisputeVerificationFacet.sol) | [`test/e2e/disputeValidation/disputeInputFields/onChainSlashes.test.ts`](../../../../test/e2e/disputeValidation/disputeInputFields/onChainSlashes.test.ts)                                                                                                                                                                                                                    |
| REQ-FP-6 | Design pending | `Current:` an invalid fraud proof slashes its dispute-eligible submitter (self-slashing guard); the intended penalty rule for invalid submissions is an **open question** awaiting engineer decision.                                     | [`FraudProofFacet.applyFraudProofs`](../../../../contracts/V1/StateChannelDiamondProxy/FraudProofFacet.sol), [`DisputeFraudProofFacet.applyDisputeFraudProofs`](../../../../contracts/V1/StateChannelDiamondProxy/DisputeFraudProofFacet.sol)                       | none — gap                                                                                                                                                                                                                                                                                                                                                                    |
| REQ-FP-7 | Design pending | A valid dispute fraud proof applied within the kill period kills the committed dispute and slashes its disputer; uncommitted targets are no-ops; expired kill periods revert.                                                             | [`DisputeFraudProofFacet.applyDisputeFraudProofs`](../../../../contracts/V1/StateChannelDiamondProxy/DisputeFraudProofFacet.sol), [`DisputeVerificationFacet._killDispute`](../../../../contracts/V1/StateChannelDiamondProxy/DisputeVerificationFacet.sol)         | [`DisputeVerificationFacet.t.sol`](../../../../test/V1/StateChannelDiamondProxy/DisputeVerificationFacet.t.sol) (expiry/atomic-batch tests), [`test/e2e/disputeValidation/`](../../../../test/e2e/disputeValidation)                                                                                                                                                          |
| INV-FP-8 | Design pending | Proof application is idempotent per offender: proofs targeting an already-slashed participant are skipped, and duplicate slashes are never recorded.                                                                                      | skip in [`applyFraudProofs`](../../../../contracts/V1/StateChannelDiamondProxy/FraudProofFacet.sol), dedup in [`addOnChainSlashedParticipant`](../../../../contracts/V1/StateChannelDiamondProxy/StateChannelCommon.sol)                                            | none — gap                                                                                                                                                                                                                                                                                                                                                                    |
| REQ-FP-9 | Design pending | The fraud-proof taxonomy MUST NOT be treated as complete; completeness is gated on the dedicated security review.                                                                                                                         | — (process requirement)                                                                                                                                                                                                                                             | tracked in [../security/open-security-review.md](../security/open-security-review.md)                                                                                                                                                                                                                                                                                         |
