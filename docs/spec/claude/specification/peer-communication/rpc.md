...
7. **Invoke the endpoint.** The endpoint then owns payload decoding, semantic validation,
   authorization, and resource bounding (`REQ-RPC-1`, `REQ-RPC-3`). On the request path, a handler
...
persistence) are tracked in [OQ-34](../open-questions.md).
...
- Correlation identity binds one request to at most one settlement: response, declared remote error,
  timeout, cancellation, or disconnect (`REQ-RPC-2`).
...
part of the service contract and is verified per endpoint (`REQ-RPC-4`).
...
[OQ-34](../open-questions.md), coupled to the signature-domain decision
...
<a id="req-rpc-6"></a>
**REQ-RPC-6 — Ordered ingress verification.** Inbound frames MUST pass the fixed dispatch order of
...
<a id="req-rpc-7"></a>
**REQ-RPC-7 — Guard semantics.** Guards MUST be pure predicates evaluated in declaration order with
...
<a id="req-rpc-8"></a>
**REQ-RPC-8 — Compatibility before protected calls.** Session compatibility (protocol and encoding
...
handshake domain. (Scheme unresolved: [OQ-34](../open-questions.md).)
...
| `REQ-RPC-4`             | Replay and concurrency. Duplicate or reordered delivery MUST be idempotent, rejected, or      |
| `REQ-RPC-5`             | Resource bounds. Payload size, outstanding requests, expensive proof/signaling work, and      |
| `REQ-RPC-6`             | Ordered ingress verification. Inbound frames MUST pass the fixed dispatch order               |
| `REQ-RPC-7`             | Guard semantics. Guards MUST be pure predicates evaluated in declaration order                |
| `REQ-RPC-8`             | Compatibility before protected calls. Session compatibility MUST be established               |
...
- RPC handlers run outside the block-progression execution boundary: an endpoint never assumes
  ...
  ([`REQ-BLOCK-PIPE-5`](../block-progression/block-processing.md)).
...
| --------------------------------------- | ------------------------- | --------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <a id="inv-rpc-1-t1"></a>`INV-RPC-1.T1` | `INV-RPC-1`               | Attempt every service before, during, and after mutual authentication using correct and substituted identities. | Only the bound peer in the correct session state is dispatched.                                                  | <a id="inv-rpc-1-t1-p1"></a>`INV-RPC-1.T1.P1` — mutual success; <a id="inv-rpc-1-t1-p2"></a>`INV-RPC-1.T1.P2` — forged identity; <a id="inv-rpc-1-t1-p3"></a>`INV-RPC-1.T1.P3` — pre-auth; <a id="inv-rpc-1-t1-p4"></a>`INV-RPC-1.T1.P4` — reconnect; <a id="inv-rpc-1-t1-p5"></a>`INV-RPC-1.T1.P5` — stale identity; <a id="inv-rpc-1-t1-p6"></a>`INV-RPC-1.T1.P6` — half-auth.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| <a id="req-rpc-1-t1"></a>`REQ-RPC-1.T1` | `REQ-RPC-1`               | Encode/decode and dispatch every delivery mode and service payload, including invalid envelopes.                | Valid messages preserve meaning; invalid messages cannot reach service logic.                                    | <a id="req-rpc-1-t1-p1"></a>`REQ-RPC-1.T1.P1` — request/response; <a id="req-rpc-1-t1-p2"></a>`REQ-RPC-1.T1.P2` — one-way; <a id="req-rpc-1-t1-p3"></a>`REQ-RPC-1.T1.P3` — malformed message; <a id="req-rpc-1-t1-p4"></a>`REQ-RPC-1.T1.P4` — boundary payloads; <a id="req-rpc-1-t1-p5"></a>`REQ-RPC-1.T1.P5` — unknown message; <a id="req-rpc-1-t1-p6"></a>`REQ-RPC-1.T1.P6` — version mismatch.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| <a id="req-rpc-2-t1"></a>`REQ-RPC-2.T1` | `REQ-RPC-2`               | Race response, remote error, timeout, cancellation, disconnect, and disposal for pending calls.                 | Exactly one outcome wins and all pending resources are released.                                                 | <a id="req-rpc-2-t1-p1"></a>`REQ-RPC-2.T1.P1` — response settles; <a id="req-rpc-2-t1-p2"></a>`REQ-RPC-2.T1.P2` — response vs remote-error race; <a id="req-rpc-2-t1-p3"></a>`REQ-RPC-2.T1.P3` — late reply; <a id="req-rpc-2-t1-p4"></a>`REQ-RPC-2.T1.P4` — reconnect; <a id="req-rpc-2-t1-p5"></a>`REQ-RPC-2.T1.P5` — settlement attempt by a non-addressed peer; <a id="req-rpc-2-t1-p6"></a>`REQ-RPC-2.T1.P6` — remote-error settles; <a id="req-rpc-2-t1-p7"></a>`REQ-RPC-2.T1.P7` — timeout settles; <a id="req-rpc-2-t1-p8"></a>`REQ-RPC-2.T1.P8` — cancellation settles; <a id="req-rpc-2-t1-p9"></a>`REQ-RPC-2.T1.P9` — disconnect settles; <a id="req-rpc-2-t1-p10"></a>`REQ-RPC-2.T1.P10` — response vs timeout race; <a id="req-rpc-2-t1-p11"></a>`REQ-RPC-2.T1.P11` — response vs cancellation race; <a id="req-rpc-2-t1-p12"></a>`REQ-RPC-2.T1.P12` — response vs disconnect race; <a id="req-rpc-2-t1-p13"></a>`REQ-RPC-2.T1.P13` — remote-error vs timeout race; <a id="req-rpc-2-t1-p14"></a>`REQ-RPC-2.T1.P14` — remote-error vs cancellation race; <a id="req-rpc-2-t1-p15"></a>`REQ-RPC-2.T1.P15` — remote-error vs disconnect race; <a id="req-rpc-2-t1-p16"></a>`REQ-RPC-2.T1.P16` — timeout vs cancellation race; <a id="req-rpc-2-t1-p17"></a>`REQ-RPC-2.T1.P17` — timeout vs disconnect race; <a id="req-rpc-2-t1-p18"></a>`REQ-RPC-2.T1.P18` — cancellation vs disconnect race; <a id="req-rpc-2-t1-p19"></a>`REQ-RPC-2.T1.P19` — disposal. |
| <a id="req-rpc-3-t1"></a>`REQ-RPC-3.T1` | `REQ-RPC-3`               | Exercise every service with valid/invalid role, channel, fork, phase, and signature.                            | Only authorized, correctly bound calls affect protocol state.                                                    | <a id="req-rpc-3-t1-p1"></a>`REQ-RPC-3.T1.P1` — negotiation; <a id="req-rpc-3-t1-p2"></a>`REQ-RPC-3.T1.P2` — block; <a id="req-rpc-3-t1-p3"></a>`REQ-RPC-3.T1.P3` — sync; <a id="req-rpc-3-t1-p4"></a>`REQ-RPC-3.T1.P4` — transport upgrade; <a id="req-rpc-3-t1-p5"></a>`REQ-RPC-3.T1.P5` — join; <a id="req-rpc-3-t1-p6"></a>`REQ-RPC-3.T1.P6` — dispute.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| <a id="req-rpc-4-t1"></a>`REQ-RPC-4.T1` | `REQ-RPC-4`               | Duplicate, reorder, replay, and concurrently deliver each state-changing call.                                  | The owning operation's idempotence/merge rule holds without duplicate effects.                                   | <a id="req-rpc-4-t1-p1"></a>`REQ-RPC-4.T1.P1` — duplicate; <a id="req-rpc-4-t1-p2"></a>`REQ-RPC-4.T1.P2` — reorder; <a id="req-rpc-4-t1-p3"></a>`REQ-RPC-4.T1.P3` — concurrent conflicting calls; <a id="req-rpc-4-t1-p4"></a>`REQ-RPC-4.T1.P4` — retry after failure; <a id="req-rpc-4-t1-p5"></a>`REQ-RPC-4.T1.P5` — block-confirmation idempotent-by-merge class; <a id="req-rpc-4-t1-p6"></a>`REQ-RPC-4.T1.P6` — handshake-completion replay-rejecting class; <a id="req-rpc-4-t1-p7"></a>`REQ-RPC-4.T1.P7` — dispute-acknowledgment replay-rejecting class; <a id="req-rpc-4-t1-p8"></a>`REQ-RPC-4.T1.P8` — sync concurrency-limited class.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| <a id="req-rpc-5-t1"></a>`REQ-RPC-5.T1` | `REQ-RPC-5`               | Saturate payload, pending-call, proof/signaling, and rate boundaries from one and many peers.                   | Work is bounded, overload is explicit, and healthy peers/services continue.                                      | <a id="req-rpc-5-t1-p1"></a>`REQ-RPC-5.T1.P1` — payload size at bound; <a id="req-rpc-5-t1-p2"></a>`REQ-RPC-5.T1.P2` — one abusive peer; <a id="req-rpc-5-t1-p3"></a>`REQ-RPC-5.T1.P3` — distributed load; <a id="req-rpc-5-t1-p4"></a>`REQ-RPC-5.T1.P4` — recovery; <a id="req-rpc-5-t1-p5"></a>`REQ-RPC-5.T1.P5` — payload size beyond bound; <a id="req-rpc-5-t1-p6"></a>`REQ-RPC-5.T1.P6` — pending-call count at bound; <a id="req-rpc-5-t1-p7"></a>`REQ-RPC-5.T1.P7` — pending-call count beyond bound; <a id="req-rpc-5-t1-p8"></a>`REQ-RPC-5.T1.P8` — proof/signaling work at bound; <a id="req-rpc-5-t1-p9"></a>`REQ-RPC-5.T1.P9` — proof/signaling work beyond bound; <a id="req-rpc-5-t1-p10"></a>`REQ-RPC-5.T1.P10` — per-peer rate at bound; <a id="req-rpc-5-t1-p11"></a>`REQ-RPC-5.T1.P11` — per-peer rate beyond bound.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| <a id="req-rpc-6-t1"></a>`REQ-RPC-6.T1` | `REQ-RPC-6`               | Deliver frames failing at each dispatch stage, including oversized, response-shaped, and probe frames.          | Each stage fails with its defined consequence; no stage is skippable; probes learn nothing about gated methods.  | <a id="req-rpc-6-t1-p1"></a>`REQ-RPC-6.T1.P1` — oversized before parse; <a id="req-rpc-6-t1-p2"></a>`REQ-RPC-6.T1.P2` — response-shaped frame never dispatched; <a id="req-rpc-6-t1-p3"></a>`REQ-RPC-6.T1.P3` — unknown service; <a id="req-rpc-6-t1-p4"></a>`REQ-RPC-6.T1.P4` — gated-service probe (existent vs nonexistent method indistinguishable); <a id="req-rpc-6-t1-p5"></a>`REQ-RPC-6.T1.P5` — unknown method.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| <a id="req-rpc-7-t1"></a>`REQ-RPC-7.T1` | `REQ-RPC-7`               | Fail each guard in each declared position over network and loopback transports, for one-way and request calls.  | Order and short-circuit respected; request calls settle deterministically; only loopback self-delivery bypasses. | <a id="req-rpc-7-t1-p1"></a>`REQ-RPC-7.T1.P1` — order/short-circuit; <a id="req-rpc-7-t1-p2"></a>`REQ-RPC-7.T1.P2` — request settles on guard failure; <a id="req-rpc-7-t1-p3"></a>`REQ-RPC-7.T1.P3` — loopback self-delivery bypass; <a id="req-rpc-7-t1-p4"></a>`REQ-RPC-7.T1.P4` — guard consequence isolation (other sessions unaffected); <a id="req-rpc-7-t1-p5"></a>`REQ-RPC-7.T1.P5` — network transport non-bypass.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| <a id="req-rpc-8-t1"></a>`REQ-RPC-8.T1` | `REQ-RPC-8`               | Connect peers with matching and mismatched protocol/encoding versions.                                          | Compatible sessions proceed; incompatible peers are refused cleanly before any protected call.                   | <a id="req-rpc-8-t1-p1"></a>`REQ-RPC-8.T1.P1` — compatible; <a id="req-rpc-8-t1-p2"></a>`REQ-RPC-8.T1.P2` — incompatible refused without penalty; <a id="req-rpc-8-t1-p3"></a>`REQ-RPC-8.T1.P3` — protected call attempted before compatibility established.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
and per-service gossip rate limiting remains open ([OQ-6](../open-questions.md)).
