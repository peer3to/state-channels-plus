...
- **Unconditional sync.** The client advances the mirror from every relevant observed on-chain
  event and state read ([`REQ-IX-7`](../interactions.md#req-ix-7)), unconditionally — the mirror
...
<a id="inv-mirror-1"></a>
**INV-MIRROR-1 — Single implementation.** Every protocol predicate evaluated locally MUST be the
...
<a id="req-mirror-1"></a>
**REQ-MIRROR-1 — Constrained equivalence.** Under the [equivalence constraints](#equivalence-constraints)
...
<a id="req-mirror-2"></a>
**REQ-MIRROR-2 — Unconditional replication.** The mirror advances only by replicating observed
...
<a id="req-mirror-3"></a>
**REQ-MIRROR-3 — Cache, never authority.** A local read is an optimization. Absence in the mirror
...
| ----------------------- | ------------------------------------------------------------------------------- |
| `INV-MIRROR-1`          | One implementation: local predicates are the on-chain contract logic.           |
| `REQ-MIRROR-1`          | Local = on-chain under the stated equivalence constraints only.                 |
| `REQ-MIRROR-2`          | Mirror advances only by idempotent replication of observed chain state.         |
| `REQ-MIRROR-3`          | Mirror absence ≠ chain absence; consequential decisions anchor on the RPC view. |
...
- Mirror state is storage-system data ([`REQ-IX-9`](../interactions.md#req-ix-9)): rebuilt from
  chain observation after loss, per [durability.md](../storage/durability.md) `REQ-STOR-3`.

## Security considerations

The mirror's dangers are exactly its conveniences. Treating the cache as truth converts RPC lag
into wrong protocol decisions (`REQ-MIRROR-3` is the defense); letting local work write into the
mirror poisons every later local check (`REQ-MIRROR-2`); evaluating time-dependent predicates with
uncontrolled local time yields plausible-but-wrong window verdicts (`REQ-MIRROR-1`'s context
...
| --------------------------------------------- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <a id="inv-mirror-1-t1"></a>`INV-MIRROR-1.T1` | `INV-MIRROR-1`            | Enumerate every protocol predicate the client evaluates and locate its implementation.                                                                                                                                         | Each locally evaluated predicate resolves to the mirrored contract logic; no client-side reimplementation of an on-chain predicate exists.                    | <a id="inv-mirror-1-t1-p1"></a>`INV-MIRROR-1.T1.P1` — state-proof verification maps to contract logic; <a id="inv-mirror-1-t1-p2"></a>`INV-MIRROR-1.T1.P2` — a needed-but-unexposed predicate is added to the contracts, not beside them; <a id="inv-mirror-1-t1-p3"></a>`INV-MIRROR-1.T1.P3` — dispute reduction maps to contract logic; <a id="inv-mirror-1-t1-p4"></a>`INV-MIRROR-1.T1.P4` — output-state construction maps to contract logic; <a id="inv-mirror-1-t1-p5"></a>`INV-MIRROR-1.T1.P5` — balance-invariant check maps to contract logic; <a id="inv-mirror-1-t1-p6"></a>`INV-MIRROR-1.T1.P6` — replay execution maps to contract logic.                      |
| <a id="req-mirror-1-t1"></a>`REQ-MIRROR-1.T1` | `REQ-MIRROR-1`            | Evaluate every locally used predicate (proof verification, reduction, output construction, balance invariant, replay) locally and on-chain with identical state/inputs, then violate each equivalence constraint individually. | Agreement under the constraints for every predicate; each constraint violation produces a detectably non-equivalent evaluation, never a silently trusted one. | <a id="req-mirror-1-t1-p1"></a>`REQ-MIRROR-1.T1.P1` — proof verification agrees; <a id="req-mirror-1-t1-p2"></a>`REQ-MIRROR-1.T1.P2` — uncontrolled time diverges on window predicates; <a id="req-mirror-1-t1-p3"></a>`REQ-MIRROR-1.T1.P3` — missing replicated state; <a id="req-mirror-1-t1-p4"></a>`REQ-MIRROR-1.T1.P4` — version-skewed logic detected; <a id="req-mirror-1-t1-p5"></a>`REQ-MIRROR-1.T1.P5` — reduction agrees; <a id="req-mirror-1-t1-p6"></a>`REQ-MIRROR-1.T1.P6` — output construction agrees; <a id="req-mirror-1-t1-p7"></a>`REQ-MIRROR-1.T1.P7` — balance invariant agrees; <a id="req-mirror-1-t1-p8"></a>`REQ-MIRROR-1.T1.P8` — replay agrees. |
| <a id="req-mirror-2-t1"></a>`REQ-MIRROR-2.T1` | `REQ-MIRROR-2`            | Replay event streams with duplicates, reordering, and gaps; attempt local hypothetical writes.                                                                                                                                 | Replication converges idempotently; gaps leave explicit absence; no non-replication write path exists into mirror state.                                      | <a id="req-mirror-2-t1-p1"></a>`REQ-MIRROR-2.T1.P1` — duplicate/reordered events converge; <a id="req-mirror-2-t1-p2"></a>`REQ-MIRROR-2.T1.P2` — gap leaves absence, later fill converges; <a id="req-mirror-2-t1-p3"></a>`REQ-MIRROR-2.T1.P3` — local work cannot mutate mirror state.                                                                                                                                                                                                                                                                                                                                                                                     |
| <a id="req-mirror-3-t1"></a>`REQ-MIRROR-3.T1` | `REQ-MIRROR-3`            | Serve reads with the mirror fresh, lagging, and missing records, for informational and consequential decisions.                                                                                                                | Fresh-mirror reads short-circuit; consequential decisions on lagging/missing data anchor to the RPC view; absence never resolves to "does not exist".         | <a id="req-mirror-3-t1-p1"></a>`REQ-MIRROR-3.T1.P1` — fresh cache hit; <a id="req-mirror-3-t1-p2"></a>`REQ-MIRROR-3.T1.P2` — lagging mirror, chain fallback decides; <a id="req-mirror-3-t1-p3"></a>`REQ-MIRROR-3.T1.P3` — missing record treated as unobserved; <a id="req-mirror-3-t1-p4"></a>`REQ-MIRROR-3.T1.P4` — timing predicate near a window edge under observation lag.                                                                                                                                                                                                                                                                                           |
