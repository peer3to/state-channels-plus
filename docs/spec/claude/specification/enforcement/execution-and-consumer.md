...
([`INV-HIST-1`](../protocol-model/history-and-commitments.md)).
...
1. **Restore.** Load the supplied encoded pre-state into the machine (`setState` semantics,
   [`REQ-SM-2`](../protocol-model/state-machines.md)).
2. **Execute.** Run the transition with the protocol-injected execution context — logical author,
   ...
   ([`REQ-SM-1` family](../protocol-model/state-machines.md)); bounded by the configured gas limit.
...
(`REQ-CONTRACT-ARCH-3`): external callers cannot drive the machine directly through the manager.
...
<a id="inv-enfsm-1"></a>
**INV-ENFSM-1 — Replay from supplied state only.** Contract-side execution restores the complete
...
<a id="req-enfsm-1"></a>
**REQ-ENFSM-1 — Injected context, bounded gas.** Execution supplies the protocol's execution
...
([`REQ-MIRROR-1`](./local-mirror.md)).

<a id="req-enfsm-2"></a>
**REQ-ENFSM-2 — Adapter confinement.** The consumer adapter is reachable only through the
...
| ----------------------- | ----------------------------------------------------------------- |
| `INV-ENFSM-1`           | Replay is a pure function of supplied pre-state + transition.     |
| `REQ-ENFSM-1`           | Injected context and gas bound, identical across replay sites.    |
| `REQ-ENFSM-2`           | Adapter reachable only via specified delegation; scoped failures. |
...
transition look fraudulent or a fraudulent one look honest — `INV-ENFSM-1`/`REQ-ENFSM-1` close
...
| ------------------------------------------- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <a id="inv-enfsm-1-t1"></a>`INV-ENFSM-1.T1` | `INV-ENFSM-1`             | Replay identical (pre-state, transition) pairs after unrelated executions, with dirty machine storage, and repeatedly.   | Bit-identical post-state and messages every time; residual storage has no effect.                                                 | <a id="inv-enfsm-1-t1-p1"></a>`INV-ENFSM-1.T1.P1` — clean vs dirty prior state; <a id="inv-enfsm-1-t1-p2"></a>`INV-ENFSM-1.T1.P2` — repeated replay stability; <a id="inv-enfsm-1-t1-p3"></a>`INV-ENFSM-1.T1.P3` — interleaved channels on the shared deployment.                                                                                   |
| <a id="req-enfsm-1-t1"></a>`REQ-ENFSM-1.T1` | `REQ-ENFSM-1`             | Execute transitions that read injected context, attempt ambient context, sit at the gas boundary, and exceed it.         | Injected values govern; gas-limit failures classify as invalid transitions identically on-chain and in the mirror.                | <a id="req-enfsm-1-t1-p1"></a>`REQ-ENFSM-1.T1.P1` — injected context honored; <a id="req-enfsm-1-t1-p2"></a>`REQ-ENFSM-1.T1.P2` — at the gas bound; <a id="req-enfsm-1-t1-p3"></a>`REQ-ENFSM-1.T1.P3` — on-chain vs mirror classification agreement; <a id="req-enfsm-1-t1-p4"></a>`REQ-ENFSM-1.T1.P4` — over the gas bound.                        |
| <a id="req-enfsm-2-t1"></a>`REQ-ENFSM-2.T1` | `REQ-ENFSM-2`             | Call adapter operations through the specified delegation and directly; fail the adapter inside each enclosing operation. | Direct external access is refused; failures propagate exactly per the enclosing atomicity rules; no out-of-scope storage effects. | <a id="req-enfsm-2-t1-p1"></a>`REQ-ENFSM-2.T1.P1` — direct access refused; <a id="req-enfsm-2-t1-p2"></a>`REQ-ENFSM-2.T1.P2` — failure inside deposit composition; <a id="req-enfsm-2-t1-p3"></a>`REQ-ENFSM-2.T1.P3` — failure inside withdrawal processing; <a id="req-enfsm-2-t1-p4"></a>`REQ-ENFSM-2.T1.P4` — adapter storage-scope containment. |
