...
2. **Session authentication**: a signature over a _domain-tagged string_ incorporating a fresh
   challenge ([handshake.md](../peer-communication/handshake.md), `INV-AUTH-2`). The domain tag
...
- Everything signed is either locally produced or fully validated; the signer is never an oracle
  over unvalidated input ([`REQ-AUTH-1`](../peer-communication/handshake.md) is the pattern).

## Requirements and invariants

<a id="inv-id-1"></a>
**INV-ID-1 — Key control is identity.** A valid signature recovering to an address is, for every
...
<a id="req-id-1"></a>
**REQ-ID-1 — Recoverable signatures over canonical targets.** Protocol signatures MUST support
...
<a id="req-id-2"></a>
**REQ-ID-2 — Normalized identity comparison.** Every identity comparison and identity-keyed
...
<a id="req-id-3"></a>
**REQ-ID-3 — Confined signing authority.** One trusted runtime component holds the key and signs;
...
<a id="req-id-4"></a>
**REQ-ID-4 — Domain-separated signing forms.** Session-authentication signatures MUST be
...
chains, and deployments is [OQ-29](../open-questions.md).)

This table is the normative requirement index. Detailed rules and rationale are defined above.

| Requirement / invariant | Statement                                                                     |
| ----------------------- | ----------------------------------------------------------------------------- |
| `INV-ID-1`              | A recovering signature is the participant; compromise is total.               |
| `REQ-ID-1`              | Recoverable signatures over the canonical 32-byte hash target.                |
| `REQ-ID-2`              | Normalized address comparison everywhere.                                     |
| `REQ-ID-3`              | Signing authority confined; validate before any remotely triggered signature. |
| `REQ-ID-4`              | Session and protocol-object signing forms cannot cross-verify.                |
...
`INV-ID-1` — mitigations are operational, plus the economic bound of trust-model A5); signing-oracle
abuse (an endpoint tricked into signing attacker-shaped content — contained by `REQ-ID-3`/
`REQ-ID-4` and per-endpoint validation); identity-comparison bugs (a checksum-variant address
slipping past an exclusion list or splitting attribution — `REQ-ID-2`); cross-context replay of
protocol-object signatures (open, OQ-29); and malleability of the signature encoding (verification
...
| ------------------------------------- | ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <a id="inv-id-1-t1"></a>`INV-ID-1.T1` | `INV-ID-1`                | Perform each protocol role's signed action with the registered key and with a different key.                                                 | Recovery decides everything: the registered key's signatures act as the participant; any other key's do not, regardless of claimed identity fields. | <a id="inv-id-1-t1-p1"></a>`INV-ID-1.T1.P1` — signed block accepted from the key holder; <a id="inv-id-1-t1-p2"></a>`INV-ID-1.T1.P2` — claimed-identity fields cannot override recovery; <a id="inv-id-1-t1-p3"></a>`INV-ID-1.T1.P3` — a "stolen key" signature is indistinguishable and accepted (documenting the compromise model); <a id="inv-id-1-t1-p4"></a>`INV-ID-1.T1.P4` — signed transaction accepted from the key holder; <a id="inv-id-1-t1-p5"></a>`INV-ID-1.T1.P5` — signed open accepted from the key holder; <a id="inv-id-1-t1-p6"></a>`INV-ID-1.T1.P6` — signed join accepted from the key holder; <a id="inv-id-1-t1-p7"></a>`INV-ID-1.T1.P7` — signed dispute accepted from the key holder.                                                                                           |
| <a id="req-id-1-t1"></a>`REQ-ID-1.T1` | `REQ-ID-1`                | Verify identical (message, signature) pairs on-chain and off-chain, including tampered messages and signatures.                              | Same recovered address everywhere; tampering changes or invalidates recovery; targets are the canonical-encoding hash only.                         | <a id="req-id-1-t1-p1"></a>`REQ-ID-1.T1.P1` — on-/off-chain recovery agreement for blocks; <a id="req-id-1-t1-p2"></a>`REQ-ID-1.T1.P2` — tampered message; <a id="req-id-1-t1-p3"></a>`REQ-ID-1.T1.P3` — non-canonical re-encoding fails verification; <a id="req-id-1-t1-p4"></a>`REQ-ID-1.T1.P4` — malleated signature encoding not double-counted; <a id="req-id-1-t1-p5"></a>`REQ-ID-1.T1.P5` — on-/off-chain recovery agreement for transactions; <a id="req-id-1-t1-p6"></a>`REQ-ID-1.T1.P6` — on-/off-chain recovery agreement for opens; <a id="req-id-1-t1-p7"></a>`REQ-ID-1.T1.P7` — on-/off-chain recovery agreement for joins; <a id="req-id-1-t1-p8"></a>`REQ-ID-1.T1.P8` — on-/off-chain recovery agreement for disputes; <a id="req-id-1-t1-p9"></a>`REQ-ID-1.T1.P9` — tampered signature. |
| <a id="req-id-2-t1"></a>`REQ-ID-2.T1` | `REQ-ID-2`                | Exercise identity-keyed structures (participant sets, exclusions, attribution, storage keys) with case/checksum variants of one address.     | All variants resolve to one identity in every structure.                                                                                            | <a id="req-id-2-t1-p1"></a>`REQ-ID-2.T1.P1` — participant sets with variant addresses; <a id="req-id-2-t1-p2"></a>`REQ-ID-2.T1.P2` — variant on the write side; <a id="req-id-2-t1-p3"></a>`REQ-ID-2.T1.P3` — exclusion cannot be evaded by recasing; <a id="req-id-2-t1-p4"></a>`REQ-ID-2.T1.P4` — exclusion lists with variant addresses; <a id="req-id-2-t1-p5"></a>`REQ-ID-2.T1.P5` — attribution records with variant addresses; <a id="req-id-2-t1-p6"></a>`REQ-ID-2.T1.P6` — storage keys with variant addresses; <a id="req-id-2-t1-p7"></a>`REQ-ID-2.T1.P7` — variant on the read side.                                                                                                                                                                                                          |
| <a id="req-id-3-t1"></a>`REQ-ID-3.T1` | `REQ-ID-3`                | Enumerate remotely reachable surfaces and isolated-context boundaries; attempt to obtain signatures outside the specified signing endpoints. | Only the specified endpoints sign, each only after its validation; no key or signing capability crosses a context or peer boundary.                 | <a id="req-id-3-t1-p1"></a>`REQ-ID-3.T1.P1` — surface enumeration finds only specified signers; <a id="req-id-3-t1-p2"></a>`REQ-ID-3.T1.P2` — handshake-response endpoint validates before signing; <a id="req-id-3-t1-p3"></a>`REQ-ID-3.T1.P3` — isolated context cannot request raw signing; <a id="req-id-3-t1-p4"></a>`REQ-ID-3.T1.P4` — join-countersigning endpoint validates before signing; <a id="req-id-3-t1-p5"></a>`REQ-ID-3.T1.P5` — block-confirmation endpoint validates before signing.                                                                                                                                                                                                                                                                                                   |
| <a id="req-id-4-t1"></a>`REQ-ID-4.T1` | `REQ-ID-4`                | Craft session challenges equal to protocol-object hashes and protocol objects shaped like domain-tagged strings.                             | Neither form's signature verifies as the other, in any direction.                                                                                   | <a id="req-id-4-t1-p1"></a>`REQ-ID-4.T1.P1` — challenge = object hash; <a id="req-id-4-t1-p2"></a>`REQ-ID-4.T1.P2` — object mimicking the tagged string; <a id="req-id-4-t1-p3"></a>`REQ-ID-4.T1.P3` — block signature against the session verifier; <a id="req-id-4-t1-p4"></a>`REQ-ID-4.T1.P4` — transaction signature against the session verifier; <a id="req-id-4-t1-p5"></a>`REQ-ID-4.T1.P5` — open signature against the session verifier; <a id="req-id-4-t1-p6"></a>`REQ-ID-4.T1.P6` — join signature against the session verifier; <a id="req-id-4-t1-p7"></a>`REQ-ID-4.T1.P7` — dispute signature against the session verifier.                                                                                                                                                                |

## Future Work

_Non-normative._ Domain separation for protocol objects (chain, deployment, version, object type —
OQ-29) and its proof-migration plan; key rotation or session-key delegation with explicit
