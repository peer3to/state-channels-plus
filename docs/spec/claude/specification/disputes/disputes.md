...
- The on-chain snapshot advances by `updateStateSnapshotFork`
  ...
  [cross-layer-messages.md](../settlement/cross-layer-messages.md)) (`REQ-DIS-9`). Multiple dispute
...
firstBlockGrace + p2pTime + agreementTime + chainFallbackTime`, where `previousTimestamp` is
  ...
  previous block, which forfeits the extra on-chain time. See [time.md](../protocol-model/time.md) for the time
  model.
...
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <a id="req-dis-1"></a>`REQ-DIS-1`   | A dispute MUST state at least one of the four valid inputs (timeout, valid on-chain slashes, self-removal, forced inbound inclusion); fraud proofs are not a dispute input.                                                                 |
| <a id="req-dis-2"></a>`REQ-DIS-2`   | Upload is limited to eligible disputers: `disputer == msg.sender`, dispute-eligible, ≤1 dispute per window per participant, throttled to one upload per `evidenceTime` per sender.                                                          |
| <a id="req-dis-3"></a>`REQ-DIS-3`   | An uploaded dispute records its commitment immediately; while the kill period runs, an invalid committed dispute can be killed via dispute fraud proof, slashing its disputer. (Intended rule: open question §4.2.)                         |
| <a id="req-dis-4"></a>`REQ-DIS-4`   | Reduction runs only after the kill period expires and consumes exactly the committed dispute set.                                                                                                                                           |
| <a id="inv-dis-5"></a>`INV-DIS-5`   | The reduced result is independent of the order in which valid dispute inputs are applied.                                                                                                                                                   |
| <a id="req-dis-6"></a>`REQ-DIS-6`   | Every initiated dispute window MUST end in a canonical successor fork (`forkId = keccak256(output SnapshotData)`, genesis timestamp = kill-period end), whether individual claims are accepted or rejected; valid state is carried forward. |
| <a id="inv-dis-7"></a>`INV-DIS-7`   | In a fork whose reduction contains any on-chain slashes, timeout removal is not applied — slashes take precedence.                                                                                                                          |
| <a id="inv-dis-8"></a>`INV-DIS-8`   | A fork applies at most one timeout, targeting the participant at the lowest timed-out block height; no skipping ahead past an earlier missed slot.                                                                                          |
| <a id="req-dis-9"></a>`REQ-DIS-9`   | The on-chain snapshot advances to a successor fork only along committed `reducedResult` links whose challenge periods have expired, verifying genesis identity/timestamp and processing the proven outbound range incrementally.            |
| <a id="req-dis-10"></a>`REQ-DIS-10` | Timeout claims MUST satisfy the deadline, linkage, schedule, and existence conditions of §6.3; violations are falsifiable via the `Timeout*` dispute fraud proofs and upload race checks.                                                   |
...
| --------------- | ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `REQ-DIS-1.T1`  | `REQ-DIS-1`               | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | A dispute MUST state at least one of the four valid inputs (timeout, valid on-chain slashes, self-removal, forced inbound inclusion); fraud proofs are not a dispute input.                                                                 | <a id="req-dis-1.t1.p1"></a>`REQ-DIS-1.T1.P1` — valid case<br><a id="req-dis-1.t1.p2"></a>`REQ-DIS-1.T1.P2` — before deadline<br><a id="req-dis-1.t1.p3"></a>`REQ-DIS-1.T1.P3` — new participant<br><a id="req-dis-1.t1.p4"></a>`REQ-DIS-1.T1.P4` — malformed input<br><a id="req-dis-1.t1.p5"></a>`REQ-DIS-1.T1.P5` — direct invalid/opposite case<br><a id="req-dis-1.t1.p6"></a>`REQ-DIS-1.T1.P6` — at deadline<br><a id="req-dis-1.t1.p7"></a>`REQ-DIS-1.T1.P7` — after deadline<br><a id="req-dis-1.t1.p8"></a>`REQ-DIS-1.T1.P8` — maximum honest skew<br><a id="req-dis-1.t1.p9"></a>`REQ-DIS-1.T1.P9` — existing participant<br><a id="req-dis-1.t1.p10"></a>`REQ-DIS-1.T1.P10` — removed participant<br><a id="req-dis-1.t1.p11"></a>`REQ-DIS-1.T1.P11` — slashed participant<br><a id="req-dis-1.t1.p12"></a>`REQ-DIS-1.T1.P12` — concurrent membership change<br><a id="req-dis-1.t1.p13"></a>`REQ-DIS-1.T1.P13` — adversarial input<br><a id="req-dis-1.t1.p14"></a>`REQ-DIS-1.T1.P14` — partial failure<br><a id="req-dis-1.t1.p15"></a>`REQ-DIS-1.T1.P15` — retry and recovery                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `REQ-DIS-2.T1`  | `REQ-DIS-2`               | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | Upload is limited to eligible disputers: `disputer == msg.sender`, dispute-eligible, ≤1 dispute per window per participant, throttled to one upload per `evidenceTime` per sender.                                                          | <a id="req-dis-2.t1.p1"></a>`REQ-DIS-2.T1.P1` — valid case<br><a id="req-dis-2.t1.p2"></a>`REQ-DIS-2.T1.P2` — correct identity/signature<br><a id="req-dis-2.t1.p3"></a>`REQ-DIS-2.T1.P3` — before deadline<br><a id="req-dis-2.t1.p4"></a>`REQ-DIS-2.T1.P4` — malformed input<br><a id="req-dis-2.t1.p5"></a>`REQ-DIS-2.T1.P5` — direct invalid/opposite case<br><a id="req-dis-2.t1.p6"></a>`REQ-DIS-2.T1.P6` — wrong identity/signature<br><a id="req-dis-2.t1.p7"></a>`REQ-DIS-2.T1.P7` — missing identity/signature<br><a id="req-dis-2.t1.p8"></a>`REQ-DIS-2.T1.P8` — duplicate identity/signature<br><a id="req-dis-2.t1.p9"></a>`REQ-DIS-2.T1.P9` — forged identity/signature<br><a id="req-dis-2.t1.p10"></a>`REQ-DIS-2.T1.P10` — membership boundary<br><a id="req-dis-2.t1.p11"></a>`REQ-DIS-2.T1.P11` — at deadline<br><a id="req-dis-2.t1.p12"></a>`REQ-DIS-2.T1.P12` — after deadline<br><a id="req-dis-2.t1.p13"></a>`REQ-DIS-2.T1.P13` — maximum honest skew<br><a id="req-dis-2.t1.p14"></a>`REQ-DIS-2.T1.P14` — adversarial input<br><a id="req-dis-2.t1.p15"></a>`REQ-DIS-2.T1.P15` — partial failure<br><a id="req-dis-2.t1.p16"></a>`REQ-DIS-2.T1.P16` — retry and recovery                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `REQ-DIS-3.T1`  | `REQ-DIS-3`               | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | An uploaded dispute records its commitment immediately; while the kill period runs, an invalid committed dispute can be killed via dispute fraud proof, slashing its disputer. (Intended rule: open question §4.2.)                         | <a id="req-dis-3.t1.p1"></a>`REQ-DIS-3.T1.P1` — valid case<br><a id="req-dis-3.t1.p2"></a>`REQ-DIS-3.T1.P2` — matching commitment<br><a id="req-dis-3.t1.p3"></a>`REQ-DIS-3.T1.P3` — before deadline<br><a id="req-dis-3.t1.p4"></a>`REQ-DIS-3.T1.P4` — new participant<br><a id="req-dis-3.t1.p5"></a>`REQ-DIS-3.T1.P5` — malformed input<br><a id="req-dis-3.t1.p6"></a>`REQ-DIS-3.T1.P6` — direct invalid/opposite case<br><a id="req-dis-3.t1.p7"></a>`REQ-DIS-3.T1.P7` — mismatched commitment<br><a id="req-dis-3.t1.p8"></a>`REQ-DIS-3.T1.P8` — predecessor case<br><a id="req-dis-3.t1.p9"></a>`REQ-DIS-3.T1.P9` — genesis case<br><a id="req-dis-3.t1.p10"></a>`REQ-DIS-3.T1.P10` — stale fork<br><a id="req-dis-3.t1.p11"></a>`REQ-DIS-3.T1.P11` — foreign fork<br><a id="req-dis-3.t1.p12"></a>`REQ-DIS-3.T1.P12` — at deadline<br><a id="req-dis-3.t1.p13"></a>`REQ-DIS-3.T1.P13` — after deadline<br><a id="req-dis-3.t1.p14"></a>`REQ-DIS-3.T1.P14` — maximum honest skew<br><a id="req-dis-3.t1.p15"></a>`REQ-DIS-3.T1.P15` — existing participant<br><a id="req-dis-3.t1.p16"></a>`REQ-DIS-3.T1.P16` — removed participant<br><a id="req-dis-3.t1.p17"></a>`REQ-DIS-3.T1.P17` — slashed participant<br><a id="req-dis-3.t1.p18"></a>`REQ-DIS-3.T1.P18` — concurrent membership change<br><a id="req-dis-3.t1.p19"></a>`REQ-DIS-3.T1.P19` — adversarial input<br><a id="req-dis-3.t1.p20"></a>`REQ-DIS-3.T1.P20` — partial failure<br><a id="req-dis-3.t1.p21"></a>`REQ-DIS-3.T1.P21` — retry and recovery                                                                                                                  |
| `REQ-DIS-4.T1`  | `REQ-DIS-4`               | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | Reduction runs only after the kill period expires and consumes exactly the committed dispute set.                                                                                                                                           | <a id="req-dis-4.t1.p1"></a>`REQ-DIS-4.T1.P1` — valid case<br><a id="req-dis-4.t1.p2"></a>`REQ-DIS-4.T1.P2` — matching commitment<br><a id="req-dis-4.t1.p3"></a>`REQ-DIS-4.T1.P3` — before deadline<br><a id="req-dis-4.t1.p4"></a>`REQ-DIS-4.T1.P4` — malformed input<br><a id="req-dis-4.t1.p5"></a>`REQ-DIS-4.T1.P5` — direct invalid/opposite case<br><a id="req-dis-4.t1.p6"></a>`REQ-DIS-4.T1.P6` — mismatched commitment<br><a id="req-dis-4.t1.p7"></a>`REQ-DIS-4.T1.P7` — predecessor case<br><a id="req-dis-4.t1.p8"></a>`REQ-DIS-4.T1.P8` — genesis case<br><a id="req-dis-4.t1.p9"></a>`REQ-DIS-4.T1.P9` — stale fork<br><a id="req-dis-4.t1.p10"></a>`REQ-DIS-4.T1.P10` — foreign fork<br><a id="req-dis-4.t1.p11"></a>`REQ-DIS-4.T1.P11` — at deadline<br><a id="req-dis-4.t1.p12"></a>`REQ-DIS-4.T1.P12` — after deadline<br><a id="req-dis-4.t1.p13"></a>`REQ-DIS-4.T1.P13` — maximum honest skew<br><a id="req-dis-4.t1.p14"></a>`REQ-DIS-4.T1.P14` — adversarial input<br><a id="req-dis-4.t1.p15"></a>`REQ-DIS-4.T1.P15` — partial failure<br><a id="req-dis-4.t1.p16"></a>`REQ-DIS-4.T1.P16` — retry and recovery                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `INV-DIS-5.T1`  | `INV-DIS-5`               | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | The reduced result is independent of the order in which valid dispute inputs are applied.                                                                                                                                                   | <a id="inv-dis-5.t1.p1"></a>`INV-DIS-5.T1.P1` — valid case<br><a id="inv-dis-5.t1.p2"></a>`INV-DIS-5.T1.P2` — matching commitment<br><a id="inv-dis-5.t1.p3"></a>`INV-DIS-5.T1.P3` — new participant<br><a id="inv-dis-5.t1.p4"></a>`INV-DIS-5.T1.P4` — duplicate delivery<br><a id="inv-dis-5.t1.p5"></a>`INV-DIS-5.T1.P5` — malformed input<br><a id="inv-dis-5.t1.p6"></a>`INV-DIS-5.T1.P6` — direct invalid/opposite case<br><a id="inv-dis-5.t1.p7"></a>`INV-DIS-5.T1.P7` — mismatched commitment<br><a id="inv-dis-5.t1.p8"></a>`INV-DIS-5.T1.P8` — predecessor case<br><a id="inv-dis-5.t1.p9"></a>`INV-DIS-5.T1.P9` — genesis case<br><a id="inv-dis-5.t1.p10"></a>`INV-DIS-5.T1.P10` — stale fork<br><a id="inv-dis-5.t1.p11"></a>`INV-DIS-5.T1.P11` — foreign fork<br><a id="inv-dis-5.t1.p12"></a>`INV-DIS-5.T1.P12` — existing participant<br><a id="inv-dis-5.t1.p13"></a>`INV-DIS-5.T1.P13` — removed participant<br><a id="inv-dis-5.t1.p14"></a>`INV-DIS-5.T1.P14` — slashed participant<br><a id="inv-dis-5.t1.p15"></a>`INV-DIS-5.T1.P15` — concurrent membership change<br><a id="inv-dis-5.t1.p16"></a>`INV-DIS-5.T1.P16` — replayed delivery<br><a id="inv-dis-5.t1.p17"></a>`INV-DIS-5.T1.P17` — permuted delivery order<br><a id="inv-dis-5.t1.p18"></a>`INV-DIS-5.T1.P18` — concurrent delivery<br><a id="inv-dis-5.t1.p19"></a>`INV-DIS-5.T1.P19` — adversarial input<br><a id="inv-dis-5.t1.p20"></a>`INV-DIS-5.T1.P20` — partial failure<br><a id="inv-dis-5.t1.p21"></a>`INV-DIS-5.T1.P21` — retry and recovery                                                                                                |
| `REQ-DIS-6.T1`  | `REQ-DIS-6`               | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | Every initiated dispute window MUST end in a canonical successor fork (`forkId = keccak256(output SnapshotData)`, genesis timestamp = kill-period end), whether individual claims are accepted or rejected; valid state is carried forward. | <a id="req-dis-6.t1.p1"></a>`REQ-DIS-6.T1.P1` — valid case<br><a id="req-dis-6.t1.p2"></a>`REQ-DIS-6.T1.P2` — matching commitment<br><a id="req-dis-6.t1.p3"></a>`REQ-DIS-6.T1.P3` — before deadline<br><a id="req-dis-6.t1.p4"></a>`REQ-DIS-6.T1.P4` — malformed input<br><a id="req-dis-6.t1.p5"></a>`REQ-DIS-6.T1.P5` — direct invalid/opposite case<br><a id="req-dis-6.t1.p6"></a>`REQ-DIS-6.T1.P6` — mismatched commitment<br><a id="req-dis-6.t1.p7"></a>`REQ-DIS-6.T1.P7` — predecessor case<br><a id="req-dis-6.t1.p8"></a>`REQ-DIS-6.T1.P8` — genesis case<br><a id="req-dis-6.t1.p9"></a>`REQ-DIS-6.T1.P9` — stale fork<br><a id="req-dis-6.t1.p10"></a>`REQ-DIS-6.T1.P10` — foreign fork<br><a id="req-dis-6.t1.p11"></a>`REQ-DIS-6.T1.P11` — at deadline<br><a id="req-dis-6.t1.p12"></a>`REQ-DIS-6.T1.P12` — after deadline<br><a id="req-dis-6.t1.p13"></a>`REQ-DIS-6.T1.P13` — maximum honest skew<br><a id="req-dis-6.t1.p14"></a>`REQ-DIS-6.T1.P14` — adversarial input<br><a id="req-dis-6.t1.p15"></a>`REQ-DIS-6.T1.P15` — partial failure<br><a id="req-dis-6.t1.p16"></a>`REQ-DIS-6.T1.P16` — retry and recovery                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `INV-DIS-7.T1`  | `INV-DIS-7`               | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | In a fork whose reduction contains any on-chain slashes, timeout removal is not applied — slashes take precedence.                                                                                                                          | <a id="inv-dis-7.t1.p1"></a>`INV-DIS-7.T1.P1` — valid case<br><a id="inv-dis-7.t1.p2"></a>`INV-DIS-7.T1.P2` — matching commitment<br><a id="inv-dis-7.t1.p3"></a>`INV-DIS-7.T1.P3` — before deadline<br><a id="inv-dis-7.t1.p4"></a>`INV-DIS-7.T1.P4` — new participant<br><a id="inv-dis-7.t1.p5"></a>`INV-DIS-7.T1.P5` — direct invalid/opposite case<br><a id="inv-dis-7.t1.p6"></a>`INV-DIS-7.T1.P6` — mismatched commitment<br><a id="inv-dis-7.t1.p7"></a>`INV-DIS-7.T1.P7` — predecessor case<br><a id="inv-dis-7.t1.p8"></a>`INV-DIS-7.T1.P8` — genesis case<br><a id="inv-dis-7.t1.p9"></a>`INV-DIS-7.T1.P9` — stale fork<br><a id="inv-dis-7.t1.p10"></a>`INV-DIS-7.T1.P10` — foreign fork<br><a id="inv-dis-7.t1.p11"></a>`INV-DIS-7.T1.P11` — at deadline<br><a id="inv-dis-7.t1.p12"></a>`INV-DIS-7.T1.P12` — after deadline<br><a id="inv-dis-7.t1.p13"></a>`INV-DIS-7.T1.P13` — maximum honest skew<br><a id="inv-dis-7.t1.p14"></a>`INV-DIS-7.T1.P14` — existing participant<br><a id="inv-dis-7.t1.p15"></a>`INV-DIS-7.T1.P15` — removed participant<br><a id="inv-dis-7.t1.p16"></a>`INV-DIS-7.T1.P16` — slashed participant<br><a id="inv-dis-7.t1.p17"></a>`INV-DIS-7.T1.P17` — concurrent membership change                                                                                                                                                                                                                                                                                                                                                                                                           |
| `INV-DIS-8.T1`  | `INV-DIS-8`               | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | A fork applies at most one timeout, targeting the participant at the lowest timed-out block height; no skipping ahead past an earlier missed slot.                                                                                          | <a id="inv-dis-8.t1.p1"></a>`INV-DIS-8.T1.P1` — valid case<br><a id="inv-dis-8.t1.p2"></a>`INV-DIS-8.T1.P2` — matching commitment<br><a id="inv-dis-8.t1.p3"></a>`INV-DIS-8.T1.P3` — correct identity/signature<br><a id="inv-dis-8.t1.p4"></a>`INV-DIS-8.T1.P4` — before deadline<br><a id="inv-dis-8.t1.p5"></a>`INV-DIS-8.T1.P5` — direct invalid/opposite case<br><a id="inv-dis-8.t1.p6"></a>`INV-DIS-8.T1.P6` — mismatched commitment<br><a id="inv-dis-8.t1.p7"></a>`INV-DIS-8.T1.P7` — predecessor case<br><a id="inv-dis-8.t1.p8"></a>`INV-DIS-8.T1.P8` — genesis case<br><a id="inv-dis-8.t1.p9"></a>`INV-DIS-8.T1.P9` — stale fork<br><a id="inv-dis-8.t1.p10"></a>`INV-DIS-8.T1.P10` — foreign fork<br><a id="inv-dis-8.t1.p11"></a>`INV-DIS-8.T1.P11` — wrong identity/signature<br><a id="inv-dis-8.t1.p12"></a>`INV-DIS-8.T1.P12` — missing identity/signature<br><a id="inv-dis-8.t1.p13"></a>`INV-DIS-8.T1.P13` — duplicate identity/signature<br><a id="inv-dis-8.t1.p14"></a>`INV-DIS-8.T1.P14` — forged identity/signature<br><a id="inv-dis-8.t1.p15"></a>`INV-DIS-8.T1.P15` — membership boundary<br><a id="inv-dis-8.t1.p16"></a>`INV-DIS-8.T1.P16` — at deadline<br><a id="inv-dis-8.t1.p17"></a>`INV-DIS-8.T1.P17` — after deadline<br><a id="inv-dis-8.t1.p18"></a>`INV-DIS-8.T1.P18` — maximum honest skew                                                                                                                                                                                                                                                                                                      |
| `REQ-DIS-9.T1`  | `REQ-DIS-9`               | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | The on-chain snapshot advances to a successor fork only along committed `reducedResult` links whose challenge periods have expired, verifying genesis identity/timestamp and processing the proven outbound range incrementally.            | <a id="req-dis-9.t1.p1"></a>`REQ-DIS-9.T1.P1` — valid case<br><a id="req-dis-9.t1.p2"></a>`REQ-DIS-9.T1.P2` — matching commitment<br><a id="req-dis-9.t1.p3"></a>`REQ-DIS-9.T1.P3` — correct identity/signature<br><a id="req-dis-9.t1.p4"></a>`REQ-DIS-9.T1.P4` — before deadline<br><a id="req-dis-9.t1.p5"></a>`REQ-DIS-9.T1.P5` — duplicate delivery<br><a id="req-dis-9.t1.p6"></a>`REQ-DIS-9.T1.P6` — direct invalid/opposite case<br><a id="req-dis-9.t1.p7"></a>`REQ-DIS-9.T1.P7` — mismatched commitment<br><a id="req-dis-9.t1.p8"></a>`REQ-DIS-9.T1.P8` — predecessor case<br><a id="req-dis-9.t1.p9"></a>`REQ-DIS-9.T1.P9` — genesis case<br><a id="req-dis-9.t1.p10"></a>`REQ-DIS-9.T1.P10` — stale fork<br><a id="req-dis-9.t1.p11"></a>`REQ-DIS-9.T1.P11` — foreign fork<br><a id="req-dis-9.t1.p12"></a>`REQ-DIS-9.T1.P12` — wrong identity/signature<br><a id="req-dis-9.t1.p13"></a>`REQ-DIS-9.T1.P13` — missing identity/signature<br><a id="req-dis-9.t1.p14"></a>`REQ-DIS-9.T1.P14` — duplicate identity/signature<br><a id="req-dis-9.t1.p15"></a>`REQ-DIS-9.T1.P15` — forged identity/signature<br><a id="req-dis-9.t1.p16"></a>`REQ-DIS-9.T1.P16` — membership boundary<br><a id="req-dis-9.t1.p17"></a>`REQ-DIS-9.T1.P17` — at deadline<br><a id="req-dis-9.t1.p18"></a>`REQ-DIS-9.T1.P18` — after deadline<br><a id="req-dis-9.t1.p19"></a>`REQ-DIS-9.T1.P19` — maximum honest skew<br><a id="req-dis-9.t1.p20"></a>`REQ-DIS-9.T1.P20` — replayed delivery<br><a id="req-dis-9.t1.p21"></a>`REQ-DIS-9.T1.P21` — permuted delivery order<br><a id="req-dis-9.t1.p22"></a>`REQ-DIS-9.T1.P22` — concurrent delivery |
| `REQ-DIS-10.T1` | `REQ-DIS-10`              | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | Timeout claims MUST satisfy the deadline, linkage, schedule, and existence conditions of §6.3; violations are falsifiable via the `Timeout*` dispute fraud proofs and upload race checks.                                                   | <a id="req-dis-10.t1.p1"></a>`REQ-DIS-10.T1.P1` — valid case<br><a id="req-dis-10.t1.p2"></a>`REQ-DIS-10.T1.P2` — matching commitment<br><a id="req-dis-10.t1.p3"></a>`REQ-DIS-10.T1.P3` — before deadline<br><a id="req-dis-10.t1.p4"></a>`REQ-DIS-10.T1.P4` — malformed input<br><a id="req-dis-10.t1.p5"></a>`REQ-DIS-10.T1.P5` — direct invalid/opposite case<br><a id="req-dis-10.t1.p6"></a>`REQ-DIS-10.T1.P6` — mismatched commitment<br><a id="req-dis-10.t1.p7"></a>`REQ-DIS-10.T1.P7` — predecessor case<br><a id="req-dis-10.t1.p8"></a>`REQ-DIS-10.T1.P8` — genesis case<br><a id="req-dis-10.t1.p9"></a>`REQ-DIS-10.T1.P9` — stale fork<br><a id="req-dis-10.t1.p10"></a>`REQ-DIS-10.T1.P10` — foreign fork<br><a id="req-dis-10.t1.p11"></a>`REQ-DIS-10.T1.P11` — at deadline<br><a id="req-dis-10.t1.p12"></a>`REQ-DIS-10.T1.P12` — after deadline<br><a id="req-dis-10.t1.p13"></a>`REQ-DIS-10.T1.P13` — maximum honest skew<br><a id="req-dis-10.t1.p14"></a>`REQ-DIS-10.T1.P14` — adversarial input<br><a id="req-dis-10.t1.p15"></a>`REQ-DIS-10.T1.P15` — partial failure<br><a id="req-dis-10.t1.p16"></a>`REQ-DIS-10.T1.P16` — retry and recovery                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
