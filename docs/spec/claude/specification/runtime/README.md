...
- **Trust and availability assumptions.** At least one available, honest RPC endpoint
  ...
  **browser and Node.js** with identical protocol capability
  ([`REQ-RUNTIME-5`](./execution.md)); platform APIs differ between them only beneath the
...
- **Invariants (owned).** `INV-RUNTIME-1`, `REQ-RUNTIME-*` ([execution.md](./execution.md));
  `INV-SDK-ARCH-1`, `REQ-SDK-ARCH-*` ([sdk.md](./sdk.md)); `REQ-CONFIG-*`
...
- **Failure and recovery outcomes.** Context failure is contained at its boundary; restart recovers
  ...
  trusting read-back data ([`REQ-STOR-3`](../storage/durability.md)); stale callbacks cannot mutate
...
- **Verification evidence.** Requirement matrices in the owned documents; execution equivalence and
  the observation edge are proven under [`REQ-IX-8`](../interactions.md#req-ix-8) and
...
Producer of chain observation for every system ([`REQ-IX-7`](../interactions.md#req-ix-7)) and owner
of the execution-equivalence guarantee every system inherits
([`REQ-IX-8`](../interactions.md#req-ix-8)).
