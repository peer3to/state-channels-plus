...
calldata ([`REQ-IX-7`](../interactions.md#req-ix-7)), local authoring (which enters at execution —
...
1. **Authenticate** with the canonical predicate ([`INV-MIRROR-1`](../enforcement/local-mirror.md)):
...
5. **Queue** the block into the pre-execution layer with copy-scoped source attribution
   (`REQ-BLOCK-PIPE-5`, [queue store](../storage/queue.md)), and arm the entry's **fixed lifetime**:
...
current fork, one block at a time (`REQ-BLOCK-PIPE-6`). Competing bodies at one coordinate coexist
...
context's consequence (`REQ-BLOCK-PIPE-3`):
...
6. **author is the scheduled leader**: `getNextToWrite` of the pre-state
   ([`REQ-SM-5`](../protocol-model/state-machines.md)), with the pre-state positioned explicitly in
   replay contexts;
...
boundary releases (`INV-BLOCK-PIPE-1`):
...
2. Execute the transition through the protocol model ([`REQ-IX-2`](../interactions.md#req-ix-2));
...
blocks — [`REQ-IX-3`](../interactions.md#req-ix-3)); persist the snapshot and state, then decide
...
persisting** so echoed copies merge as duplicates (`REQ-BLOCK-PIPE-7`); run the exit path when the
...
<a id="req-block-pipe-7"></a>
**REQ-BLOCK-PIPE-7 — Commit before publish.** A confirmation is persisted locally before it is
...
<a id="req-block-pipe-8"></a>
**REQ-BLOCK-PIPE-8 — Evidence precedes escalation.** Every escalation of an objective fault stores
...
| `REQ-BLOCK-PIPE-5`      | Pre-execution merge layer. Intake, deduplication, and signature merging form a       |
| `REQ-BLOCK-PIPE-6`      | Total-order application. Blocks leave the pre-execution layer in total order by fork |
| `REQ-BLOCK-PIPE-7`      | Commit before publish. A confirmation is persisted locally before it is published    |
| `REQ-BLOCK-PIPE-8`      | Evidence precedes escalation. Every escalation of an objective fault stores the      |
...
| ----------------------------------------------------- | ------------------------- | ----------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <a id="inv-block-pipe-1-t1"></a>`INV-BLOCK-PIPE-1.T1` | `INV-BLOCK-PIPE-1`        | Process valid and failing blocks while injecting failures at each commit boundary.              | Every durable/event effect is all-or-nothing and repeated delivery is idempotent.                                                         | <a id="inv-block-pipe-1-t1-p1"></a>`INV-BLOCK-PIPE-1.T1.P1` — success; <a id="inv-block-pipe-1-t1-p2"></a>`INV-BLOCK-PIPE-1.T1.P2` — validation-stage failure; <a id="inv-block-pipe-1-t1-p3"></a>`INV-BLOCK-PIPE-1.T1.P3` — retry after failure; <a id="inv-block-pipe-1-t1-p4"></a>`INV-BLOCK-PIPE-1.T1.P4` — concurrent forks; <a id="inv-block-pipe-1-t1-p5"></a>`INV-BLOCK-PIPE-1.T1.P5` — execution-stage failure; <a id="inv-block-pipe-1-t1-p6"></a>`INV-BLOCK-PIPE-1.T1.P6` — persist/commit failure; <a id="inv-block-pipe-1-t1-p7"></a>`INV-BLOCK-PIPE-1.T1.P7` — post-commit side-effect failure; <a id="inv-block-pipe-1-t1-p8"></a>`INV-BLOCK-PIPE-1.T1.P8` — duplicate delivery idempotent.                                                                                                                                    |
| <a id="req-block-pipe-1-t1"></a>`REQ-BLOCK-PIPE-1.T1` | `REQ-BLOCK-PIPE-1`        | Deliver complementary, duplicate, forged, and conflicting confirmations from several sources.   | Merge converges without losing attribution; conflicts remain attributable.                                                                | <a id="req-block-pipe-1-t1-p1"></a>`REQ-BLOCK-PIPE-1.T1.P1` — complementary signatures; <a id="req-block-pipe-1-t1-p2"></a>`REQ-BLOCK-PIPE-1.T1.P2` — duplicate; <a id="req-block-pipe-1-t1-p3"></a>`REQ-BLOCK-PIPE-1.T1.P3` — conflicting confirmation; <a id="req-block-pipe-1-t1-p4"></a>`REQ-BLOCK-PIPE-1.T1.P4` — restore/requeue; <a id="req-block-pipe-1-t1-p5"></a>`REQ-BLOCK-PIPE-1.T1.P5` — forged confirmation.                                                                                                                                                                                                                                                                                                                                                                                                                    |
| <a id="req-block-pipe-2-t1"></a>`REQ-BLOCK-PIPE-2.T1` | `REQ-BLOCK-PIPE-2`        | Violate each validation dimension alone and in representative combinations.                     | Rejection occurs before execution with the correct offender/evidence and no partial effect.                                               | <a id="req-block-pipe-2-t1-p1"></a>`REQ-BLOCK-PIPE-2.T1.P1` — identity violation; <a id="req-block-pipe-2-t1-p2"></a>`REQ-BLOCK-PIPE-2.T1.P2` — ordering violation; <a id="req-block-pipe-2-t1-p3"></a>`REQ-BLOCK-PIPE-2.T1.P3` — time violation; <a id="req-block-pipe-2-t1-p4"></a>`REQ-BLOCK-PIPE-2.T1.P4` — combined violations; <a id="req-block-pipe-2-t1-p5"></a>`REQ-BLOCK-PIPE-2.T1.P5` — signature violation; <a id="req-block-pipe-2-t1-p6"></a>`REQ-BLOCK-PIPE-2.T1.P6` — linkage violation; <a id="req-block-pipe-2-t1-p7"></a>`REQ-BLOCK-PIPE-2.T1.P7` — message-input violation; <a id="req-block-pipe-2-t1-p8"></a>`REQ-BLOCK-PIPE-2.T1.P8` — state-proof violation; <a id="req-block-pipe-2-t1-p9"></a>`REQ-BLOCK-PIPE-2.T1.P9` — check-to-execution race.                                                                   |
| <a id="req-block-pipe-3-t1"></a>`REQ-BLOCK-PIPE-3.T1` | `REQ-BLOCK-PIPE-3`        | Apply every deviation in live, stored, spectating, and dispute-replay contexts.                 | Classification agrees; context-specific disconnect, evidence, drop, or retry effects are correct.                                         | <a id="req-block-pipe-3-t1-p1"></a>`REQ-BLOCK-PIPE-3.T1.P1` — objective-fault deviation; <a id="req-block-pipe-3-t1-p2"></a>`REQ-BLOCK-PIPE-3.T1.P2` — live-participant context; <a id="req-block-pipe-3-t1-p3"></a>`REQ-BLOCK-PIPE-3.T1.P3` — impossible-context call; <a id="req-block-pipe-3-t1-p4"></a>`REQ-BLOCK-PIPE-3.T1.P4` — unattributable junk; <a id="req-block-pipe-3-t1-p5"></a>`REQ-BLOCK-PIPE-3.T1.P5` — not-yet-ready deviation; <a id="req-block-pipe-3-t1-p6"></a>`REQ-BLOCK-PIPE-3.T1.P6` — subjective lateness; <a id="req-block-pipe-3-t1-p7"></a>`REQ-BLOCK-PIPE-3.T1.P7` — spectator/pending-joiner context; <a id="req-block-pipe-3-t1-p8"></a>`REQ-BLOCK-PIPE-3.T1.P8` — calldata-committed context; <a id="req-block-pipe-3-t1-p9"></a>`REQ-BLOCK-PIPE-3.T1.P9` — dispute-replay context.                          |
| <a id="req-block-pipe-4-t1"></a>`REQ-BLOCK-PIPE-4.T1` | `REQ-BLOCK-PIPE-4`        | Omit predecessor/input data, then provide valid, invalid, incomplete, or delayed recovery data. | Retry is bounded and uses the full pipeline; invalid recovery never commits.                                                              | <a id="req-block-pipe-4-t1-p1"></a>`REQ-BLOCK-PIPE-4.T1.P1` — valid recovery; <a id="req-block-pipe-4-t1-p2"></a>`REQ-BLOCK-PIPE-4.T1.P2` — invalid recovery data; <a id="req-block-pipe-4-t1-p3"></a>`REQ-BLOCK-PIPE-4.T1.P3` — timeout/disconnect; <a id="req-block-pipe-4-t1-p4"></a>`REQ-BLOCK-PIPE-4.T1.P4` — repeated probe; <a id="req-block-pipe-4-t1-p5"></a>`REQ-BLOCK-PIPE-4.T1.P5` — incomplete recovery data.                                                                                                                                                                                                                                                                                                                                                                                                                    |
| <a id="req-block-pipe-5-t1"></a>`REQ-BLOCK-PIPE-5.T1` | `REQ-BLOCK-PIPE-5`        | Deliver ineligible blocks and late signatures while a transition holds the execution boundary.  | Intake and merge settle without waiting for execution; the merged set is identical for every delivery order and keeps attribution.        | <a id="req-block-pipe-5-t1-p1"></a>`REQ-BLOCK-PIPE-5.T1.P1` — older block; <a id="req-block-pipe-5-t1-p2"></a>`REQ-BLOCK-PIPE-5.T1.P2` — late signatures for a queued block; <a id="req-block-pipe-5-t1-p3"></a>`REQ-BLOCK-PIPE-5.T1.P3` — delivery-order permutations converge; <a id="req-block-pipe-5-t1-p4"></a>`REQ-BLOCK-PIPE-5.T1.P4` — per-entry bound reached; <a id="req-block-pipe-5-t1-p5"></a>`REQ-BLOCK-PIPE-5.T1.P5` — future block; <a id="req-block-pipe-5-t1-p6"></a>`REQ-BLOCK-PIPE-5.T1.P6` — duplicate block; <a id="req-block-pipe-5-t1-p7"></a>`REQ-BLOCK-PIPE-5.T1.P7` — late signatures for an already stored block; <a id="req-block-pipe-5-t1-p8"></a>`REQ-BLOCK-PIPE-5.T1.P8` — never-eligible entry ages out.                                                                                                    |
| <a id="req-block-pipe-6-t1"></a>`REQ-BLOCK-PIPE-6.T1` | `REQ-BLOCK-PIPE-6`        | Make several blocks eligible at once, including two distinct blocks at one fork and height.     | Application is totally ordered by fork and height, one at a time; same-coordinate conflict resolves by evidence rules, not arrival order. | <a id="req-block-pipe-6-t1-p1"></a>`REQ-BLOCK-PIPE-6.T1.P1` — in-order eligibility; <a id="req-block-pipe-6-t1-p2"></a>`REQ-BLOCK-PIPE-6.T1.P2` — concurrent submission of the next block; <a id="req-block-pipe-6-t1-p3"></a>`REQ-BLOCK-PIPE-6.T1.P3` — same fork/height conflict, first arrival order; <a id="req-block-pipe-6-t1-p4"></a>`REQ-BLOCK-PIPE-6.T1.P4` — fork transition with a non-empty pre-execution layer; <a id="req-block-pipe-6-t1-p5"></a>`REQ-BLOCK-PIPE-6.T1.P5` — out-of-order eligibility; <a id="req-block-pipe-6-t1-p6"></a>`REQ-BLOCK-PIPE-6.T1.P6` — same fork/height conflict, reversed arrival order.                                                                                                                                                                                                         |
| <a id="req-block-pipe-7-t1"></a>`REQ-BLOCK-PIPE-7.T1` | `REQ-BLOCK-PIPE-7`        | Commit blocks while observing publication order; echo published confirmations back to the node. | Persistence strictly precedes publication; echoes merge as duplicates without re-validation.                                              | <a id="req-block-pipe-7-t1-p1"></a>`REQ-BLOCK-PIPE-7.T1.P1` — persist-then-publish order; <a id="req-block-pipe-7-t1-p2"></a>`REQ-BLOCK-PIPE-7.T1.P2` — echoed copy is a duplicate merge; <a id="req-block-pipe-7-t1-p3"></a>`REQ-BLOCK-PIPE-7.T1.P3` — publication failure leaves committed state intact.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| <a id="req-block-pipe-8-t1"></a>`REQ-BLOCK-PIPE-8.T1` | `REQ-BLOCK-PIPE-8`        | Drive each objective-fault class and each subjective judgment through the live context.         | Every escalation carries pre-stored evidence for the offender; subjective lateness parks with no evidence, penalty, or escalation.        | <a id="req-block-pipe-8-t1-p1"></a>`REQ-BLOCK-PIPE-8.T1.P1` — double-sign stores evidence before disputing; <a id="req-block-pipe-8-t1-p2"></a>`REQ-BLOCK-PIPE-8.T1.P2` — subjective lateness never escalates; <a id="req-block-pipe-8-t1-p3"></a>`REQ-BLOCK-PIPE-8.T1.P3` — escalation with missing evidence is an internal error, not a silent dispute; <a id="req-block-pipe-8-t1-p4"></a>`REQ-BLOCK-PIPE-8.T1.P4` — invalid transition stores evidence before disputing; <a id="req-block-pipe-8-t1-p5"></a>`REQ-BLOCK-PIPE-8.T1.P5` — wrong genesis stores evidence before disputing; <a id="req-block-pipe-8-t1-p6"></a>`REQ-BLOCK-PIPE-8.T1.P6` — objective timestamp fault stores evidence before disputing; <a id="req-block-pipe-8-t1-p7"></a>`REQ-BLOCK-PIPE-8.T1.P7` — fabricated inbound block stores evidence before disputing. |
