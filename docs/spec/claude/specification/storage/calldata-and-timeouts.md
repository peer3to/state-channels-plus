...
<a id="req-cdstore-1"></a>
**REQ-CDSTORE-1 — Coordinate-keyed calldata with exact matching.** A calldata record is stored and
...
<a id="req-tostore-1"></a>
**REQ-TOSTORE-1 — Lowest-height timeout candidate.** The store keeps at most one candidate per
fork and ignores a stored update whose height is above the retained candidate's — mirroring the
protocol's lowest-timed-out-height precedence (`INV-DIS-8`) so the node never escalates a
...
| ----------------------- | ----------------------------------------------------------------------- |
| `REQ-CDSTORE-1`         | Calldata keyed by (fork, height, author); match requires hash equality. |
| `REQ-TOSTORE-1`         | One timeout candidate per fork; lowest height retained.                 |
...
| ----------------------------------------------- | ------------------------- | ------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <a id="req-cdstore-1-t1"></a>`REQ-CDSTORE-1.T1` | `REQ-CDSTORE-1`           | Store calldata records; query by coordinates and by matching block with equal and unequal hashes. | Coordinate reads return the record; match succeeds only on hash equality.         | <a id="req-cdstore-1-t1-p1"></a>`REQ-CDSTORE-1.T1.P1` — store/read by coordinates; <a id="req-cdstore-1-t1-p2"></a>`REQ-CDSTORE-1.T1.P2` — match equal hash; <a id="req-cdstore-1-t1-p3"></a>`REQ-CDSTORE-1.T1.P3` — same coordinates, different hash → no match; <a id="req-cdstore-1-t1-p4"></a>`REQ-CDSTORE-1.T1.P4` — absent coordinates. |
| <a id="req-tostore-1-t1"></a>`REQ-TOSTORE-1.T1` | `REQ-TOSTORE-1`           | Store timeout candidates at varied heights per fork in varied orders.                             | The lowest height is retained regardless of arrival order; forks are independent. | <a id="req-tostore-1-t1-p1"></a>`REQ-TOSTORE-1.T1.P1` — lower replaces higher; <a id="req-tostore-1-t1-p2"></a>`REQ-TOSTORE-1.T1.P2` — higher ignored; <a id="req-tostore-1-t1-p3"></a>`REQ-TOSTORE-1.T1.P3` — order permutations converge; <a id="req-tostore-1-t1-p4"></a>`REQ-TOSTORE-1.T1.P4` — per-fork isolation.                       |
