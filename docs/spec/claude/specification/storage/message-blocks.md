...
<a id="req-msgstore-1"></a>
**REQ-MSGSTORE-1 — Content-addressed store with tip tracking.** A block is stored under the hash of
...
<a id="req-msgstore-2"></a>
**REQ-MSGSTORE-2 — Linked backward range reads.** A range read walks the chain backward by
...
| ----------------------- | ----------------------------------------------------------------------------- |
| `REQ-MSGSTORE-1`        | Content-addressed storage; tip advances only on higher-height stores.         |
| `REQ-MSGSTORE-2`        | Backward `[upper, lower)` range reads follow linkage only; gaps end the walk. |
...
`REQ-MSGSTORE-2` forbids it. Tip integrity matters for building the _next_ block; it grants no
...
| ------------------------------------------------- | ------------------------- | --------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <a id="req-msgstore-1-t1"></a>`REQ-MSGSTORE-1.T1` | `REQ-MSGSTORE-1`          | Store tip-extending, historical, duplicate, and opt-out blocks in both instances. | Content addressing holds; the tip moves only for higher heights; instances stay independent. | <a id="req-msgstore-1-t1-p1"></a>`REQ-MSGSTORE-1.T1.P1` — tip advance on extension; <a id="req-msgstore-1-t1-p2"></a>`REQ-MSGSTORE-1.T1.P2` — backfill leaves tip; <a id="req-msgstore-1-t1-p3"></a>`REQ-MSGSTORE-1.T1.P3` — inbound/outbound isolation; <a id="req-msgstore-1-t1-p4"></a>`REQ-MSGSTORE-1.T1.P4` — duplicate store idempotent; <a id="req-msgstore-1-t1-p5"></a>`REQ-MSGSTORE-1.T1.P5` — opt-out store leaves tip.                                                                                         |
| <a id="req-msgstore-2-t1"></a>`REQ-MSGSTORE-2.T1` | `REQ-MSGSTORE-2`          | Read ranges over complete, gapped, and unlinked chains with varied bounds.        | Exactly the linked `[upper, lower)` blocks return; gaps end the walk; no substitution.       | <a id="req-msgstore-2-t1-p1"></a>`REQ-MSGSTORE-2.T1.P1` — complete range; <a id="req-msgstore-2-t1-p2"></a>`REQ-MSGSTORE-2.T1.P2` — missing middle block ends walk; <a id="req-msgstore-2-t1-p3"></a>`REQ-MSGSTORE-2.T1.P3` — lower bound at genesis; <a id="req-msgstore-2-t1-p4"></a>`REQ-MSGSTORE-2.T1.P4` — unknown upper bound returns nothing; <a id="req-msgstore-2-t1-p5"></a>`REQ-MSGSTORE-2.T1.P5` — upper bound at tip; <a id="req-msgstore-2-t1-p6"></a>`REQ-MSGSTORE-2.T1.P6` — equal upper and lower bounds. |
