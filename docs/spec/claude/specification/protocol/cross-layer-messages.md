...
5. **Forced inclusion (fallback).** If prompt inclusion fails, the joiner forces it through the
   ...
   (cross-ref [protocol/disputes.md](./disputes.md)). **For this protocol version:** on submitting the join, the
...
| ----------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `INV-MSG-1`             | Each stream is one hash-linked chain per channel; heights +1; identity = `keccak256(abi.encode(block))`                       |
| `INV-MSG-2`             | No replay, no omission: each block processed exactly once, in order                                                           |
| `INV-MSG-3`             | Tip `totalBalance` = cumulative sum of message balances; chain totals match processed prefixes                                |
| `INV-MSG-4`             | `totalWithdrawals ≤ totalDeposits` at every outbound processing step                                                          |
| `INV-MSG-5`             | Processed tips advance only to strictly newer descendants                                                                     |
| `INV-MSG-6`             | Balance invariant: `totalDeposits == totalWithdrawals + getTotalStateBalance(state)` with chain-anchored deposits/withdrawals |
| `REQ-MSG-1`             | Snapshots MUST commit both stream tips + totals; dispute outputs likewise                                                     |
| `REQ-MSG-2`             | A dispute's claimed inbound tip MUST be an ancestor of the chain tip with matching height                                     |
| `REQ-MSG-3`             | Packaged inbound blocks MUST chain from the previous snapshot tip and exist on-chain; fabrication is provable fraud           |
| `REQ-MSG-4`             | Outbound processing MUST verify the linked range and skip the processed prefix before consuming                               |
| `REQ-MSG-5`             | Catch-up MUST be batchable into smaller ranges with identical results                                                         |
| `REQ-MSG-6`             | Snapshot advance MUST require finality (same-fork) or finalized reduction + expired challenge period (successor fork)         |
| `REQ-MSG-7`             | Same-fork advance MUST consume all pending inbound messages                                                                   |
| `REQ-MSG-8`             | Exits MUST be withdrawable only through snapshot advance (no standalone submission)                                           |
| `REQ-MSG-9`             | Spectating MUST be fail-closed: any failure aborts with no funds or obligations at risk                                       |
| `REQ-MSG-10`            | Joining MUST carry the joiner's signature plus the full threshold set's signatures, pinned to an expected snapshot/fork       |
| `REQ-MSG-11`            | A deposited-but-unincluded joiner MUST be able to force inclusion via the dispute game and then be covered by leader election |
...
| --------------- | ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `INV-MSG-1.T1`  | `INV-MSG-1`               | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | Each stream is one hash-linked chain per channel; heights +1; identity = `keccak256(abi.encode(block))`                       | `INV-MSG-1.T1.P1` — valid case and direct invalid/opposite<br>`INV-MSG-1.T1.P2` — matching/mismatched commitment, predecessor/genesis, stale and foreign fork<br>`INV-MSG-1.T1.P3` — correct/wrong/missing/duplicate/forged identity or signature and membership boundary                                                                                                      |
| `INV-MSG-2.T1`  | `INV-MSG-2`               | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | No replay, no omission: each block processed exactly once, in order                                                           | `INV-MSG-2.T1.P1` — valid case and direct invalid/opposite<br>`INV-MSG-2.T1.P2` — duplicate/replay, every relevant permutation and concurrent delivery                                                                                                                                                                                                                         |
| `INV-MSG-3.T1`  | `INV-MSG-3`               | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | Tip `totalBalance` = cumulative sum of message balances; chain totals match processed prefixes                                | `INV-MSG-3.T1.P1` — valid case and direct invalid/opposite<br>`INV-MSG-3.T1.P2` — zero, exact balance/boundary, one beyond, maximum value and conservation                                                                                                                                                                                                                     |
| `INV-MSG-4.T1`  | `INV-MSG-4`               | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | `totalWithdrawals ≤ totalDeposits` at every outbound processing step                                                          | `INV-MSG-4.T1.P1` — valid case and direct invalid/opposite<br>`INV-MSG-4.T1.P2` — zero, exact balance/boundary, one beyond, maximum value and conservation                                                                                                                                                                                                                     |
| `INV-MSG-5.T1`  | `INV-MSG-5`               | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | Processed tips advance only to strictly newer descendants                                                                     | `INV-MSG-5.T1.P1` — valid case and direct invalid/opposite<br>`INV-MSG-5.T1.P2` — matching/mismatched commitment, predecessor/genesis, stale and foreign fork                                                                                                                                                                                                                  |
| `INV-MSG-6.T1`  | `INV-MSG-6`               | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | Balance invariant: `totalDeposits == totalWithdrawals + getTotalStateBalance(state)` with chain-anchored deposits/withdrawals | `INV-MSG-6.T1.P1` — valid case and direct invalid/opposite<br>`INV-MSG-6.T1.P2` — zero, exact balance/boundary, one beyond, maximum value and conservation                                                                                                                                                                                                                     |
| `REQ-MSG-1.T1`  | `REQ-MSG-1`               | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | Snapshots MUST commit both stream tips + totals; dispute outputs likewise                                                     | `REQ-MSG-1.T1.P1` — valid case and direct invalid/opposite<br>`REQ-MSG-1.T1.P2` — matching/mismatched commitment, predecessor/genesis, stale and foreign fork<br>`REQ-MSG-1.T1.P3` — malformed and adversarial input, partial failure, retry and recovery                                                                                                                      |
| `REQ-MSG-2.T1`  | `REQ-MSG-2`               | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | A dispute's claimed inbound tip MUST be an ancestor of the chain tip with matching height                                     | `REQ-MSG-2.T1.P1` — valid case and direct invalid/opposite<br>`REQ-MSG-2.T1.P2` — matching/mismatched commitment, predecessor/genesis, stale and foreign fork<br>`REQ-MSG-2.T1.P3` — malformed and adversarial input, partial failure, retry and recovery                                                                                                                      |
| `REQ-MSG-3.T1`  | `REQ-MSG-3`               | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | Packaged inbound blocks MUST chain from the previous snapshot tip and exist on-chain; fabrication is provable fraud           | `REQ-MSG-3.T1.P1` — valid case and direct invalid/opposite<br>`REQ-MSG-3.T1.P2` — matching/mismatched commitment, predecessor/genesis, stale and foreign fork<br>`REQ-MSG-3.T1.P3` — malformed and adversarial input, partial failure, retry and recovery                                                                                                                      |
| `REQ-MSG-4.T1`  | `REQ-MSG-4`               | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | Outbound processing MUST verify the linked range and skip the processed prefix before consuming                               | `REQ-MSG-4.T1.P1` — valid case and direct invalid/opposite<br>`REQ-MSG-4.T1.P2` — matching/mismatched commitment, predecessor/genesis, stale and foreign fork                                                                                                                                                                                                                  |
| `REQ-MSG-5.T1`  | `REQ-MSG-5`               | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | Catch-up MUST be batchable into smaller ranges with identical results                                                         | `REQ-MSG-5.T1.P1` — valid case and direct invalid/opposite<br>`REQ-MSG-5.T1.P2` — zero/empty/no-op where meaningful, exact boundary, failure/recovery and relevant race                                                                                                                                                                                                        |
| `REQ-MSG-6.T1`  | `REQ-MSG-6`               | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | Snapshot advance MUST require finality (same-fork) or finalized reduction + expired challenge period (successor fork)         | `REQ-MSG-6.T1.P1` — valid case and direct invalid/opposite<br>`REQ-MSG-6.T1.P2` — matching/mismatched commitment, predecessor/genesis, stale and foreign fork<br>`REQ-MSG-6.T1.P3` — before/at/after deadline and maximum honest skew                                                                                                                                          |
| `REQ-MSG-7.T1`  | `REQ-MSG-7`               | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | Same-fork advance MUST consume all pending inbound messages                                                                   | `REQ-MSG-7.T1.P1` — valid case and direct invalid/opposite<br>`REQ-MSG-7.T1.P2` — matching/mismatched commitment, predecessor/genesis, stale and foreign fork                                                                                                                                                                                                                  |
| `REQ-MSG-8.T1`  | `REQ-MSG-8`               | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | Exits MUST be withdrawable only through snapshot advance (no standalone submission)                                           | `REQ-MSG-8.T1.P1` — valid case and direct invalid/opposite<br>`REQ-MSG-8.T1.P2` — matching/mismatched commitment, predecessor/genesis, stale and foreign fork<br>`REQ-MSG-8.T1.P3` — zero, exact balance/boundary, one beyond, maximum value and conservation                                                                                                                  |
| `REQ-MSG-9.T1`  | `REQ-MSG-9`               | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | Spectating MUST be fail-closed: any failure aborts with no funds or obligations at risk                                       | `REQ-MSG-9.T1.P1` — valid case and direct invalid/opposite<br>`REQ-MSG-9.T1.P2` — malformed and adversarial input, partial failure, retry and recovery                                                                                                                                                                                                                         |
| `REQ-MSG-10.T1` | `REQ-MSG-10`              | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | Joining MUST carry the joiner's signature plus the full threshold set's signatures, pinned to an expected snapshot/fork       | `REQ-MSG-10.T1.P1` — valid case and direct invalid/opposite<br>`REQ-MSG-10.T1.P2` — matching/mismatched commitment, predecessor/genesis, stale and foreign fork<br>`REQ-MSG-10.T1.P3` — correct/wrong/missing/duplicate/forged identity or signature and membership boundary<br>`REQ-MSG-10.T1.P4` — new/existing/removed/slashed participant and concurrent membership change |
| `REQ-MSG-11.T1` | `REQ-MSG-11`              | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | A deposited-but-unincluded joiner MUST be able to force inclusion via the dispute game and then be covered by leader election | `REQ-MSG-11.T1.P1` — valid case and direct invalid/opposite<br>`REQ-MSG-11.T1.P2` — zero, exact balance/boundary, one beyond, maximum value and conservation<br>`REQ-MSG-11.T1.P3` — new/existing/removed/slashed participant and concurrent membership change<br>`REQ-MSG-11.T1.P4` — malformed and adversarial input, partial failure, retry and recovery                    |
| `REQ-MSG-12.T1` | `REQ-MSG-12`              | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | Anyone MUST be able to verify the balance invariant trustlessly for a claimed snapshot                                        | `REQ-MSG-12.T1.P1` — valid case and direct invalid/opposite<br>`REQ-MSG-12.T1.P2` — matching/mismatched commitment, predecessor/genesis, stale and foreign fork<br>`REQ-MSG-12.T1.P3` — zero, exact balance/boundary, one beyond, maximum value and conservation                                                                                                               |
