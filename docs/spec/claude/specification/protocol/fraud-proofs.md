...
| ----------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `REQ-FP-1`              | Fraud-proof enforcement is separate from the dispute game: an observed objective violation can be proven immediately, with a binary prove-and-slash / no-slash outcome; disputes consume recorded slashes instead of re-executing proofs. |
| `REQ-FP-2`              | Every block fraud-proof handler is sound: a passing proof implies a real violation by the returned address (types: `BlockDoubleSign`, `BlockInvalidStateTransition`, `WrongGenesis`, `InvalidTimestamp`, `ForgedInboundMessageBlock`).    |
| `REQ-FP-3`              | Slashes are recorded only via `addOnChainSlashedParticipant`: per-channel, append-only, deduplicated, timestamped, evented.                                                                                                               |
| `REQ-FP-4`              | A recorded slash disqualifies the participant from dispute participation and from the on-chain threshold set.                                                                                                                             |
| `REQ-FP-5`              | A dispute may list any subset of recorded slashes; listing an unrecorded slash is disprovable (`DisputeOnChainSlashesNotSubset`), and the reduction independently consumes all slashes recorded up to window expiry.                      |
| `REQ-FP-6`              | `For this protocol version:` an invalid fraud proof slashes its dispute-eligible submitter (self-slashing guard); the intended penalty rule for invalid submissions is an **open question** awaiting engineer decision.                   |
| `REQ-FP-7`              | A valid dispute fraud proof applied within the kill period kills the committed dispute and slashes its disputer; uncommitted targets are no-ops; expired kill periods revert.                                                             |
| `INV-FP-8`              | Proof application is idempotent per offender: proofs targeting an already-slashed participant are skipped, and duplicate slashes are never recorded.                                                                                      |
| `REQ-FP-9`              | The fraud-proof taxonomy MUST NOT be treated as complete; completeness is gated on the dedicated security review.                                                                                                                         |
...
| ------------- | ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `REQ-FP-1.T1` | `REQ-FP-1`                | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | Fraud-proof enforcement is separate from the dispute game: an observed objective violation can be proven immediately, with a binary prove-and-slash / no-slash outcome; disputes consume recorded slashes instead of re-executing proofs. | `REQ-FP-1.T1.P1` — valid case and direct invalid/opposite<br>`REQ-FP-1.T1.P2` — new/existing/removed/slashed participant and concurrent membership change<br>`REQ-FP-1.T1.P3` — malformed and adversarial input, partial failure, retry and recovery                                                                                                                                                                                     |
| `REQ-FP-2.T1` | `REQ-FP-2`                | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | Every block fraud-proof handler is sound: a passing proof implies a real violation by the returned address (types: `BlockDoubleSign`, `BlockInvalidStateTransition`, `WrongGenesis`, `InvalidTimestamp`, `ForgedInboundMessageBlock`).    | `REQ-FP-2.T1.P1` — valid case and direct invalid/opposite<br>`REQ-FP-2.T1.P2` — matching/mismatched commitment, predecessor/genesis, stale and foreign fork<br>`REQ-FP-2.T1.P3` — correct/wrong/missing/duplicate/forged identity or signature and membership boundary<br>`REQ-FP-2.T1.P4` — before/at/after deadline and maximum honest skew<br>`REQ-FP-2.T1.P5` — malformed and adversarial input, partial failure, retry and recovery |
| `REQ-FP-3.T1` | `REQ-FP-3`                | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | Slashes are recorded only via `addOnChainSlashedParticipant`: per-channel, append-only, deduplicated, timestamped, evented.                                                                                                               | `REQ-FP-3.T1.P1` — valid case and direct invalid/opposite<br>`REQ-FP-3.T1.P2` — correct/wrong/missing/duplicate/forged identity or signature and membership boundary<br>`REQ-FP-3.T1.P3` — before/at/after deadline and maximum honest skew<br>`REQ-FP-3.T1.P4` — new/existing/removed/slashed participant and concurrent membership change<br>`REQ-FP-3.T1.P5` — duplicate/replay, every relevant permutation and concurrent delivery   |
| `REQ-FP-4.T1` | `REQ-FP-4`                | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | A recorded slash disqualifies the participant from dispute participation and from the on-chain threshold set.                                                                                                                             | `REQ-FP-4.T1.P1` — valid case and direct invalid/opposite<br>`REQ-FP-4.T1.P2` — correct/wrong/missing/duplicate/forged identity or signature and membership boundary<br>`REQ-FP-4.T1.P3` — new/existing/removed/slashed participant and concurrent membership change<br>`REQ-FP-4.T1.P4` — malformed and adversarial input, partial failure, retry and recovery                                                                          |
| `REQ-FP-5.T1` | `REQ-FP-5`                | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | A dispute may list any subset of recorded slashes; listing an unrecorded slash is disprovable (`DisputeOnChainSlashesNotSubset`), and the reduction independently consumes all slashes recorded up to window expiry.                      | `REQ-FP-5.T1.P1` — valid case and direct invalid/opposite<br>`REQ-FP-5.T1.P2` — before/at/after deadline and maximum honest skew<br>`REQ-FP-5.T1.P3` — new/existing/removed/slashed participant and concurrent membership change<br>`REQ-FP-5.T1.P4` — malformed and adversarial input, partial failure, retry and recovery                                                                                                              |
| `REQ-FP-6.T1` | `REQ-FP-6`                | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | `For this protocol version:` an invalid fraud proof slashes its dispute-eligible submitter (self-slashing guard); the intended penalty rule for invalid submissions is an **open question** awaiting engineer decision.                   | `REQ-FP-6.T1.P1` — valid case and direct invalid/opposite<br>`REQ-FP-6.T1.P2` — new/existing/removed/slashed participant and concurrent membership change<br>`REQ-FP-6.T1.P3` — malformed and adversarial input, partial failure, retry and recovery                                                                                                                                                                                     |
| `REQ-FP-7.T1` | `REQ-FP-7`                | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | A valid dispute fraud proof applied within the kill period kills the committed dispute and slashes its disputer; uncommitted targets are no-ops; expired kill periods revert.                                                             | `REQ-FP-7.T1.P1` — valid case and direct invalid/opposite<br>`REQ-FP-7.T1.P2` — matching/mismatched commitment, predecessor/genesis, stale and foreign fork<br>`REQ-FP-7.T1.P3` — before/at/after deadline and maximum honest skew<br>`REQ-FP-7.T1.P4` — new/existing/removed/slashed participant and concurrent membership change<br>`REQ-FP-7.T1.P5` — malformed and adversarial input, partial failure, retry and recovery            |
| `INV-FP-8.T1` | `INV-FP-8`                | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | Proof application is idempotent per offender: proofs targeting an already-slashed participant are skipped, and duplicate slashes are never recorded.                                                                                      | `INV-FP-8.T1.P1` — valid case and direct invalid/opposite<br>`INV-FP-8.T1.P2` — correct/wrong/missing/duplicate/forged identity or signature and membership boundary<br>`INV-FP-8.T1.P3` — new/existing/removed/slashed participant and concurrent membership change<br>`INV-FP-8.T1.P4` — duplicate/replay, every relevant permutation and concurrent delivery                                                                          |
| `REQ-FP-9.T1` | `REQ-FP-9`                | Use the applicable black-box method in the verification strategy above; exercise the behavior through public inputs without implementation internals. | The fraud-proof taxonomy MUST NOT be treated as complete; completeness is gated on the dedicated security review.                                                                                                                         | `REQ-FP-9.T1.P1` — valid case and direct invalid/opposite<br>`REQ-FP-9.T1.P2` — malformed and adversarial input, partial failure, retry and recovery<br>`REQ-FP-9.T1.P3` — static review of every named alternative, omitted category and changed assumption                                                                                                                                                                             |
