...
drives the runtime only through the returned [`P2pInstance`](../../../../src/evm/P2pInstance.ts)
...
Implemented by [`EvmDiamondStateMachine.p2pSetup`](../../../../src/evm/EvmDiamondStateMachine.ts)
...
| `options.config`                       | `Partial<Config>?`               | Runtime config overrides; precedence in [`createConfig`](../../../../src/utils/config.ts) is overrides > `process.env` > `peer3.config.ts` > defaults. See [../reference/configuration.md](../reference/configuration.md). |
| `options.signerSecret`                 | `string?`                        | Private key (`0x` + 64 hex) or mnemonic. **A random private key is generated when omitted.**                                                                                                                               |
| `options.customRpcManifest`            | `CustomRpcManifest?`             | Integrator RPC root, resolved on the host via [`resolveCustomRpcManifest`](../../../../src/rpc/resolveCustomRpcManifest.ts) and typed through [`registry.ts`](../../../../src/rpc/registry.ts).                            |
...
and wraps it in [`HostNonceManager`](../../../../src/evm/signer/HostNonceManager.ts)
...
2. **Runtime client.** [`P2pRuntimeClient`](../../../../src/evm/p2pRuntime/P2pRuntimeClient.ts)
   speaks a request/response protocol over a `RuntimePort` and mirrors bus
   events into the client-realm `EventBus`.
3. **Runtime host.** [`startP2pRuntimeHost`](../../../../src/evm/p2pRuntime/P2pRuntimeHost.ts)
...
Client → host request types (verified in [`P2pRuntimeHost.handleRequest`](../../../../src/evm/p2pRuntime/P2pRuntimeHost.ts)):
...
- The integrator state machine obeys the deterministic execution-context rules
  ([../concepts/state-machines.md](../concepts/state-machines.md)); otherwise
...
1. **Live instance** — drives the replicated channel state. All happy-path
   ...
   [`EvmDiamondStateMachine`](../../../../src/evm/EvmDiamondStateMachine.ts).
2. **Diamond instance** — embedded in the locally deployed
   [`LocalDiamond`](../../../../src/evm/EvmDiamondStateMachine.ts) (see
   ...
   [`EventHandler`](../../../../src/eventHandlers/EventHandler.ts) replaying
...
[`EventBus`](../../../../src/events/EventBus.ts) is the one event class on both
...
| --------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| REQ-SDK-1 | Design pending | The runtime owns its signer; `p2pSetup` accepts only `signerSecret` (random when omitted), never an injected `Signer`.         | [src/evm/EvmDiamondStateMachine.ts](../../../../src/evm/EvmDiamondStateMachine.ts), [src/evm/p2pRuntime/RuntimeChainContext.ts](../../../../src/evm/p2pRuntime/RuntimeChainContext.ts) | e2e suites construct peers exclusively via `signerSecret` (e.g. [test/harness](../../../../test/harness)) — implicit; no direct negative test (`none — gap` for rejection path) |
| REQ-SDK-2 | Design pending | The SDK requires at least one available honest RPC endpoint; current implementation uses exactly one WebSocket `PROVIDER_URL`. | [src/evm/p2pRuntime/RuntimeChainContext.ts](../../../../src/evm/p2pRuntime/RuntimeChainContext.ts), [src/utils/config.ts](../../../../src/utils/config.ts)                             | none — gap (no partitioned/dishonest-RPC test)                                                                                                                                  |
| INV-SDK-1 | Design pending | All app↔runtime interaction crosses the runtime port; `getStateManager()` throws.                                             | [src/evm/P2pInstance.ts](../../../../src/evm/P2pInstance.ts)                                                                                                                           | [test/e2e/E2E-RuntimeTransportModes.test.ts](../../../../test/e2e/E2E-RuntimeTransportModes.test.ts)                                                                            |
| INV-SDK-2 | Design pending | On-chain sends draw nonces from the host-owned nonce manager.                                                                  | [src/evm/p2pRuntime/P2pRuntimeHost.ts](../../../../src/evm/p2pRuntime/P2pRuntimeHost.ts), [src/evm/signer/HostNonceManager.ts](../../../../src/evm/signer/HostNonceManager.ts)         | concurrency e2e suites (dispute/reduction races) exercise it indirectly; none — gap (no dedicated nonce-race test)                                                              |
| INV-SDK-3 | Design pending | Dispute execution uses a dedicated state-machine instance, never the live one.                                                 | [src/evm/EvmDiamondStateMachine.ts](../../../../src/evm/EvmDiamondStateMachine.ts) (`createStandaloneFromLocalStateMachineWithExecutor`, `p2pSetup` double deploy)                     | [test/e2e/disputeValidation](../../../../test/e2e/disputeValidation) suites replay disputes while live execution continues                                                      |
