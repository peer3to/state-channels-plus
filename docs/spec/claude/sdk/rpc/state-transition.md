...
Implementation:
[`StateTransitionService`](../../../../../src/rpc/services/stateTransition/StateTransitionService.ts),
[`StateTransitionRpcMethods`](../../../../../src/rpc/services/stateTransition/StateTransitionRpcMethods.ts).
Primary consumer: [`BlockQueueManager.ingestBlockConfirmation`](../../../../../src/stateManager/BlockQueueManager.ts)
via [`StateManager.ingestBlockConfirmation`](../../../../../src/stateManager/StateManager.ts).
...
- **Sending side** (local, typed proxy — never through this service's handler): the success path
  ...
  ([`StateManager.success`](../../../../../src/stateManager/StateManager.ts) step 7, only when
  `PARTICIPATING` and not dispute replay), the stored-merge path re-broadcasts grown signature
  sets ([`tryMergeStoredBlockConfirmation`](../../../../../src/stateManager/StateManager.ts) →
  `BROADCAST`), and the strategies re-broadcast on `goodNewSignaturesOnExistingBlock`
  ([`BlockValidationStrategy`](../../../../../src/stateManager/validationStrategy/BlockValidationStrategy.ts)).
...
| ------------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------ | ------------------------------------------------------------------------------- |
| Queued entries, signature sets, source attribution, 128-source cap | [`QueueStorage`](../../../../../src/storage/QueueStorage.ts) | pipeline intake                | [../block-confirmation-pipeline.md](../block-confirmation-pipeline.md) §3.1, §4 |
| Stored blocks / merged signatures                                  | [`BlockStorage`](../../../../../src/storage/BlockStorage.ts) | pipeline merge/success         | ibid. §4.1, §8                                                                  |
| Peer profiles, blacklist                                           | [`ProfileManager`](../../../../../src/ProfileManager.ts)     | this service's penalty mapping | [./README.md](./README.md) §8                                                   |

Statelessness is load-bearing: the handler runs without the `StateManager` mutex
([./README.md](./README.md) §6.6, INV-RPC-5) and can be dispatched concurrently for many frames;
all merge atomicity is the queue's responsibility (REQ-BCP-3).
...
2. **Sender attribution** _(RPC layer,
   [`StateTransitionRpcMethods`](../../../../../src/rpc/services/stateTransition/StateTransitionRpcMethods.ts))_:
   ...
   disconnect-only ([./README.md](./README.md) §8 open question, [OQ-34](../../open-questions.md)).
3. **Delegation** _(pipeline)_:
   ...
   REQ-RPC-2 demands is discharged inside intake
...
([../../security/trust-model.md](../../security/trust-model.md)) requires separating local
...
engineer decision on the error partition. Same family as DEF-5
([../../open-questions.md](../../open-questions.md)).

### 4.2 Flooding

**Unhandled — the canonical OQ-6 surface.** This is the highest-volume method on the node and
...
- **Authentic junk at scale.** Authenticity only binds the author signature to the _declared_
  ...
  [OQ-6](../../open-questions.md)). Until then, memory and task-queue growth during one
  `agreementTime` window is bounded only by link bandwidth. The per-entry caps (128 tracked
  sources, INV-BCP-4) bound _each entry_, not the entry count. Mitigations that do exist: the
...
- **Duplicate replay of a valid confirmation** — penalty-free by design (§4.3); infinite
  replays are a pure flooding vector with zero per-frame penalty, again deferred to OQ-6.

Verdict: **accepted-missing** with a decided direction — the single central limiter at
`P2PManager.onRpc` (engineer direction 2026-08-10, [OQ-6](../../open-questions.md)); required
before production. No flood test exists (gap, §7).

### 4.3 Replay / duplicate delivery

**Handled — idempotent-by-merge** (REQ-RPC-6 pattern 1, [./README.md](./README.md) §6.7).
...
(INV-BCP-4). Persist-before-gossip (INV-BCP-5) makes the node's own echoes merge as duplicates
...
same-coordinate rule, §6 predicate 4; REQ-BCP-4). What the RPC layer must (and does) preserve
...
protocol-level property ([../../protocol/finality.md](../../protocol/finality.md)), not fixable
at this ingress.
...
- **Disputed fork:** tolerated-ignore at intake (fork queue cleared, connection kept) — the
  ...
  are cut ([`BlockValidationStrategy.blockForkIsDisputed`](../../../../../src/stateManager/validationStrategy/BlockValidationStrategy.ts));
...
- **Unknown fork:** queued; the queue timeout is the sole sync probe; a failed sync punishes the
  ...
  the sync failure path inherits DEF-5's availability/Byzantine conflation).

All three: **handled**, with the DEF-5 caveat on the sync probe.
...
signatures. A participant-authorization guard is future work under REQ-RPC-5
...
| ID        | Invariant                                                                                                                                                                                                                                                                                                |
| --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| INV-STS-1 | `onBlockConfirmation` never mutates live channel state inline and never takes the `StateManager` mutex; its only state effects are pipeline delegation and the penalty mapping (specializes INV-RPC-5 / REQ-BCP-3).                                                                                      |
| INV-STS-2 | Every confirmation entering the pipeline through this service carries the handshake-verified sender address; the peer-RPC path never produces a sourceless entry (sourceless ingest is reserved for replay adapters — [../block-confirmation-pipeline.md](../block-confirmation-pipeline.md) §2 path 4). |
| INV-STS-3 | A `false` ingest verdict always maps to disconnect + blacklist by EVM address; a `true` verdict produces no RPC-layer side effect.                                                                                                                                                                       |
...
- **Gaps:** no unit test exercises `StateTransitionRpcMethods` directly (guard bypass attempt,
  ...
  test (nothing to enforce yet, [OQ-6](../../open-questions.md)); no test pinning the §4.1
...
- Central RPC rate limiter (OQ-6) — this endpoint is its primary beneficiary and the block
  queue's boundedness argument depends on it.
- Participant/membership guard (REQ-RPC-5) to shrink the stranger-traffic surface of §4.7.
...
| --------- | -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| REQ-STS-1 | Design pending | `onBlockConfirmation` delegates all payload judgment to `ingestBlockConfirmation`, passing the handshake-verified sender address for attribution; the RPC layer performs no payload validation of its own. | [src/rpc/services/stateTransition/StateTransitionRpcMethods.ts](../../../../../src/rpc/services/stateTransition/StateTransitionRpcMethods.ts), [src/stateManager/BlockQueueManager.ts](../../../../../src/stateManager/BlockQueueManager.ts) | [test/e2e/E2E-BlockQueueManager.test.ts](../../../../../test/e2e/E2E-BlockQueueManager.test.ts)                                           |
| REQ-STS-2 | Design pending | A `false` keep-connection verdict disconnects and blacklists the sender by EVM address; `true` has no RPC-layer side effect.                                                                               | [src/rpc/services/stateTransition/StateTransitionRpcMethods.ts](../../../../../src/rpc/services/stateTransition/StateTransitionRpcMethods.ts)                                                                                                | [test/e2e/E2E-BlockQueueManager.test.ts](../../../../../test/e2e/E2E-BlockQueueManager.test.ts) (wrong-channel and forged-signature cuts) |
| INV-STS-1 | Design pending | Handler holds no mutex and mutates no live state inline.                                                                                                                                                   | [src/rpc/services/stateTransition](../../../../../src/rpc/services/stateTransition), pipeline mutex sites in [src/stateManager/StateManager.ts](../../../../../src/stateManager/StateManager.ts)                                             | via REQ-BCP-3/INV-RPC-5 evidence ([../block-confirmation-pipeline.md](../block-confirmation-pipeline.md) §12); dedicated test: none — gap |
| INV-STS-2 | Design pending | Peer-RPC ingest is always source-attributed.                                                                                                                                                               | [src/rpc/services/stateTransition/StateTransitionRpcMethods.ts](../../../../../src/rpc/services/stateTransition/StateTransitionRpcMethods.ts)                                                                                                | [test/e2e/E2E-ByzantineErrorAttribution.test.ts](../../../../../test/e2e/E2E-ByzantineErrorAttribution.test.ts)                           |
| INV-STS-3 | Design pending | Verdict-to-penalty mapping as stated.                                                                                                                                                                      | [src/rpc/services/stateTransition/StateTransitionRpcMethods.ts](../../../../../src/rpc/services/stateTransition/StateTransitionRpcMethods.ts)                                                                                                | [test/e2e/E2E-BlockQueueManager.test.ts](../../../../../test/e2e/E2E-BlockQueueManager.test.ts)                                           |
