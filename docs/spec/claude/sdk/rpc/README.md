...
- **What it guarantees.** A frame reaching a handler has passed the frame-size cap, envelope-shape
  ...
  ([`P2PManager.onRpc`](../../../../../src/P2PManager.ts),
  [`ARpcService.runRPC`](../../../../../src/rpc/ARpcService.ts)). Request/response replies are
...
A service (a subclass of [`ARpcService`](../../../../../src/rpc/ARpcService.ts)) owns shared state,
internal helpers, and business logic — e.g.
[`IsForkDisputedService`](../../../../../src/rpc/services/isForkDisputedService/IsForkDisputedService.ts)
owns the per-peer acknowledgment maps, and
[`SpectateService`](../../../../../src/rpc/services/spectate/SpectateService.ts) owns the in-flight
sync set and the whole payload-generation/verification machinery. It pairs with an **RpcMethods**
class (a subclass of [`ARpcMethods`](../../../../../src/rpc/ARpcMethods.ts)) that exposes *only* the
...
([`hasMethod`](../../../../../src/utils/ObjectChecks.ts)) uses `prop in obj` plus a `typeof
...
pending; observed in [`ARpcService.runRPC`](../../../../../src/rpc/ARpcService.ts) +
[`ObjectChecks.ts`](../../../../../src/utils/ObjectChecks.ts)).

### 2.2 MainRpcService — the root

[`MainRpcService`](../../../../../src/rpc/MainRpcService.ts) is the dispatch root. Its constructor
...
Registration is purely structural: [`P2PManager.onRpc`](../../../../../src/P2PManager.ts) resolves
`rpc.service` with [`isInstanceOfRpcService`](../../../../../src/utils/ObjectChecks.ts), which
...
[`OpenChannelNegotiationService`](../../../../../src/rpc/services/openChannelNegotiation/OpenChannelNegotiationService.ts)
...
[`RemoteRpcProxy.createProxy(localRpcRoot)`](../../../../../src/rpc/RemoteRpcProxy.ts) derives the
...
([`RpcHandleProxy.ts`](../../../../../src/rpc/RpcHandleProxy.ts)). At runtime the proxy fabricates,
for `remoteRpc.<service>.<method>(...params)`, an envelope `{service, method, params}` wrapped in
an [`RpcHandler`](../../../../../src/rpc/RpcHandler.ts); no code generation and no per-service
...
([`RpcHandleProxy.ts`](../../../../../src/rpc/RpcHandleProxy.ts)):
...
- **Broadcast** ([`P2PManager.broadcastRpc`](../../../../../src/P2PManager.ts)) sends the envelope to
  every open connection. No `requestId` — no replies.
- **One-way send** (`sendOne`/`sendMultiple`, [`RpcHandler`](../../../../../src/rpc/RpcHandler.ts))
...
- **Loopback:** omitting the target on `sendOne`/`request` delivers to self through
  [`LoopbackTransport`](../../../../../src/transport/LoopbackTransport.ts) — the node invokes its own
...
[`CustomRpcManifest`](../../../../../src/rpc/registry.ts) (`{module, exportName?, options?}`) via
`p2pSetup(options.customRpcManifest)` ([architecture.md](../architecture.md) §1.1). The host side
resolves the manifest with
[`resolveCustomRpcConstructor`](../../../../../src/rpc/resolveCustomRpcManifest.ts) (dynamic module
load; throws unless the export is a constructor) and passes the constructor into
[`P2PManager`](../../../../../src/P2PManager.ts), which instantiates it in place of the base root and
...
([`ClientHostRpc.ts`](../../../../../src/evm/p2pRuntime/ClientHostRpc.ts)): a client-realm proxy that
...
([`dispatchHostRpc`](../../../../../src/evm/p2pRuntime/P2pRuntimeHost.ts)); for `request` the host
...
- **Wire data is untrusted JSON.** A Byzantine peer does not use the proxy; it sends arbitrary
  ...
  `method` strings, `params` an array — [`deserializeRpc`](../../../../../src/rpc/Rpc.ts)), service
...
(encoded protocol structs through [`Codec`](../../../../../src/utils/Codec.ts), §6.3) treating decode
...
`Codec.encode` strings, never raw JSON. [`serializeRpc`](../../../../../src/rpc/Rpc.ts) enforces this
...
[`InitHandshakeRpcMethods.onInitHandshakeRequest`](../../../../../src/rpc/services/initHandshake/InitHandshakeRpcMethods.ts)
...
[`JoinChannelService.signJoinRequest`](../../../../../src/rpc/services/joinChannel/JoinChannelService.ts)
...
A guard subclasses [`AGuard`](../../../../../src/rpc/guards/AGuard.ts): `check(rpc, transport)`
...
[`runGuards`](../../../../../src/rpc/guards/runGuards.ts) evaluates them **sequentially in
...
Placement in the dispatch path ([`ARpcService.runRPC`](../../../../../src/rpc/ARpcService.ts)):
...
only for [`LoopbackTransport`](../../../../../src/transport/LoopbackTransport.ts) (self-delivery,
§2.4/§3); every network transport reports `false`
([`ATransport.isTrusted`](../../../../../src/transport/ATransport.ts)).
...
[`HandshakeCompletedGuard`](../../../../../src/rpc/guards/HandshakeCompletedGuard.ts) is the one
...
[`handleRpcResponse`](../../../../../src/P2PManager.ts). The retry queue therefore only benefits
...
[`ARpcService.runRPC`](../../../../../src/rpc/ARpcService.ts) +
[`HandshakeCompletedGuard`](../../../../../src/rpc/guards/HandshakeCompletedGuard.ts).)
...
One JSON object per frame ([`Rpc.ts`](../../../../../src/rpc/Rpc.ts)):
...
[`ATransport.send`](../../../../../src/transport/ATransport.ts) serializes with `serializeRpc` and
...
canonical ethers type strings ([`Codec`](../../../../../src/utils/Codec.ts), `Type` enum covering
...
[`P2PManager.onRpc(serializedRpc, transport)`](../../../../../src/P2PManager.ts) is the single entry
...
1. **Frame-size cap.** `serializedRpc.length > MAX_RPC_FRAME_BYTES` (16 MiB,
   [`Rpc.ts`](../../../../../src/rpc/Rpc.ts)) → log, disconnect, stop — *before* any `JSON.parse`, so
...
[`P2PManager.sendRpcRequest`](../../../../../src/P2PManager.ts):
...
- **Addressed-peer rule (INV-RPC-2).** Only the peer the request was sent to may settle it.
  [`handleRpcResponse`](../../../../../src/P2PManager.ts) compares peer *identity*
  ([`ATransport.isSamePeer`](../../../../../src/transport/ATransport.ts), checksum-address based) —
...
- **Replay-as-violation.** A duplicate handshake ack and a duplicate dispute-acknowledgment
  ...
  ([`InitHandshakeRpcMethods.onInitHandshakeAck`](../../../../../src/rpc/services/initHandshake/InitHandshakeRpcMethods.ts),
  [`IsForkDisputedRpcMethods`](../../../../../src/rpc/services/isForkDisputedService/IsForkDisputedRpcMethods.ts)).
...
- **Transport replacement.** Peer identity is the EVM address; profiles (and blacklist state)
  survive transport churn ([`ProfileManager`](../../../../../src/ProfileManager.ts), INV-SDK-6). The
...
[OQ-29](../../open-questions.md) (which must bind protocol version into signed domains). Divergence
...
[`disconnectAndBlacklistPeer`](../../../../../src/P2PManager.ts) is a no-op) — pre-handshake
...
**Intended (engineer direction, 2026-08-10 — [OQ-6](../../open-questions.md)):** enforcement lives in
...
- Timeouts are denominated in `timeConfig.agreementTime`, read from the chain
  ([../protocol/time.md](../../protocol/time.md)); clock quality per
  [`Clock`](../../../../../src/Clock.ts).
- Trust context: peers are Byzantine; the transport provides no authentication or delivery
  guarantees ([../security/trust-model.md](../../security/trust-model.md), RPC observation
  assumptions in [../security/open-security-review.md](../../security/open-security-review.md)).
...
| --- | --- | --- | --- | --- |
| REQ-RPC-1 | Design pending | Only deliberately public methods live on `RpcMethods` classes; private helpers and mutable service state are never exposed through them. | [src/rpc/ARpcMethods.ts](../../../../../src/rpc/ARpcMethods.ts), [src/rpc/services](../../../../../src/rpc/services) | none — gap (structural convention; no lint/test enforces it) |
| REQ-RPC-2 | Design pending | Every endpoint decodes and semantically validates its payload before any state effect; decode failure is a handled protocol failure. | [src/rpc/services](../../../../../src/rpc/services) (per-endpoint) | [test/rpc](../../../../../test/rpc), [test/unit/SpectateService.test.ts](../../../../../test/unit/SpectateService.test.ts); none — gap (no adversarial fuzz suite) |
| REQ-RPC-3 | Design pending | Custom roots extend the boundary only through the service/RpcMethods pattern via manifest + registry. | [src/rpc/registry.ts](../../../../../src/rpc/registry.ts), [src/rpc/resolveCustomRpcManifest.ts](../../../../../src/rpc/resolveCustomRpcManifest.ts), [src/P2PManager.ts](../../../../../src/P2PManager.ts) | [test/e2e/E2E-CustomRpcRequestResponse.test.ts](../../../../../test/e2e/E2E-CustomRpcRequestResponse.test.ts) |
| REQ-RPC-4 | Design pending | Bigints cross the boundary only as canonical Codec encodings; raw `BigInt` throws at the sender. | [src/rpc/Rpc.ts](../../../../../src/rpc/Rpc.ts) (`serializeRpc`), [src/utils/Codec.ts](../../../../../src/utils/Codec.ts) | [test/rpc/Rpc.test.ts](../../../../../test/rpc/Rpc.test.ts) |
| INV-RPC-4 | Design pending | Same mechanism, stated as an invariant of the wire: no raw `BigInt` ever appears in an envelope. | [src/rpc/Rpc.ts](../../../../../src/rpc/Rpc.ts) | [test/rpc/Rpc.test.ts](../../../../../test/rpc/Rpc.test.ts) |
| REQ-RPC-5 | Design pending | Caller-scoped admission preconditions are expressed as explicit guards with documented ordering, trusted-transport behavior, and rejection consequence. | [src/rpc/guards](../../../../../src/rpc/guards) | [test/e2e/E2E-InitHandshake.test.ts](../../../../../test/e2e/E2E-InitHandshake.test.ts); none — gap (no per-service guard-bypass matrix) |
| REQ-RPC-6 | Design pending | Every endpoint is idempotent under re-delivery or replay-rejecting with a defined consequence. | [src/rpc/services](../../../../../src/rpc/services), [src/stateManager/BlockQueueManager.ts](../../../../../src/stateManager/BlockQueueManager.ts) | [test/e2e/E2E-IsForkDisputed.test.ts](../../../../../test/e2e/E2E-IsForkDisputed.test.ts); none — gap (no systematic replay matrix) |
| INV-RPC-1 | Design pending | Dispatch order: size cap → envelope → service → guards → method, before any handler runs. | [src/P2PManager.ts](../../../../../src/P2PManager.ts) (`onRpc`), [src/rpc/ARpcService.ts](../../../../../src/rpc/ARpcService.ts) (`runRPC`) | [test/rpc/Rpc.test.ts](../../../../../test/rpc/Rpc.test.ts) |
| INV-RPC-2 | Design pending | Only the addressed peer settles a request; other responders are blacklisted. | [src/P2PManager.ts](../../../../../src/P2PManager.ts) (`handleRpcResponse`) | [test/rpc/Rpc.test.ts](../../../../../test/rpc/Rpc.test.ts), [test/e2e/E2E-CustomRpcRequestResponse.test.ts](../../../../../test/e2e/E2E-CustomRpcRequestResponse.test.ts) |
| INV-RPC-3 | Design pending | Guard bypass only for the trusted in-process loopback. | [src/transport/ATransport.ts](../../../../../src/transport/ATransport.ts), [src/transport/LoopbackTransport.ts](../../../../../src/transport/LoopbackTransport.ts), [src/rpc/ARpcService.ts](../../../../../src/rpc/ARpcService.ts) | none — gap (no test asserts network transports cannot claim trust) |
| INV-RPC-5 | Design pending | RPC handlers never hold the state mutex; mutation goes through the pipeline's entry points. | [src/rpc](../../../../../src/rpc), [src/stateManager/StateManager.ts](../../../../../src/stateManager/StateManager.ts) | [block-confirmation-pipeline.md](../block-confirmation-pipeline.md) §12 evidence (REQ-BCP-3/4) |
| REQ-RPC-7 | Design pending | A single central RPC-level rate limiter (shared across services, possibly per-peer) bounds peer-driven resource use. | none — not implemented ([OQ-6](../../open-questions.md)) | none — gap |
