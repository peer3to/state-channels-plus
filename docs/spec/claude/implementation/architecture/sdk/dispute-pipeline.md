...
> **Specification subject:** [specification/protocol/dispute-processing.md](../../../specification/protocol/dispute-processing.md)
...
> [../protocol/disputes.md](../../../specification/protocol/disputes.md),
> [../protocol/fraud-proofs.md](../../../specification/protocol/fraud-proofs.md),
> [../protocol/state-proofs.md](../../../specification/protocol/state-proofs.md); contract side:
...
1. **State proof.** [`AgreementManager.getStateProof`](../../../../../../src/agreementManager/AgreementManager.ts#L67)
   ...
   ([../protocol/state-proofs.md](../../../specification/protocol/state-proofs.md)).
...
| 9   | Balance invariant on the latest snapshot                                 | `SCM.verifyBalanceInvariantCheckSnapshot` ([../protocol/cross-layer-messages.md](../../../specification/protocol/cross-layer-messages.md))                                                                                                                     | `DisputeInvalidBalanceInvariant`                |
...
| 16  | Dispute states a reason (timeout, slashes, self-removal, forced inbound) | `LocalDiamond.hasDisputeReason` ([../protocol/disputes.md](../../../specification/protocol/disputes.md) §3)                                                                                                                                                    | `InvalidDisputeReason`                          |
...
4. **Compute.** `SCM.reduce.staticCall(disputes)` (order-independent
   deterministic reduction, [../protocol/disputes.md](../../../specification/protocol/disputes.md))
...
[../protocol/cross-layer-messages.md](../../../specification/protocol/cross-layer-messages.md) §2),
...
| -------------- | ----------------------- | ----------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `INV-DVP-1.T1` | `INV-DVP-1`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | Per-fork dispute idempotence with rollback on failed upload.                                                | `INV-DVP-1.T1.P1` — valid case and direct invalid/opposite<br>`INV-DVP-1.T1.P2` — matching/mismatched commitment, predecessor/genesis, stale and foreign fork<br>`INV-DVP-1.T1.P3` — duplicate/replay, every relevant permutation and concurrent delivery<br>`INV-DVP-1.T1.P4` — malformed and adversarial input, partial failure, retry and recovery |
| `INV-DVP-2.T1` | `INV-DVP-2`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | Kill decisions are grounded in canonical Solidity predicates; self-slashing proofs are preflighted.         | `INV-DVP-2.T1.P1` — valid case and direct invalid/opposite<br>`INV-DVP-2.T1.P2` — new/existing/removed/slashed participant and concurrent membership change                                                                                                                                                                                           |
| `INV-DVP-3.T1` | `INV-DVP-3`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | Invalid audit ⇔ exactly one stored dispute fraud proof.                                                     | `INV-DVP-3.T1.P1` — valid case and direct invalid/opposite<br>`INV-DVP-3.T1.P2` — malformed and adversarial input, partial failure, retry and recovery                                                                                                                                                                                                |
| `INV-DVP-4.T1` | `INV-DVP-4`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | Deterministic, order-independent reduction; races classified as convergence.                                | `INV-DVP-4.T1.P1` — valid case and direct invalid/opposite<br>`INV-DVP-4.T1.P2` — duplicate/replay, every relevant permutation and concurrent delivery                                                                                                                                                                                                |
| `INV-DVP-5.T1` | `INV-DVP-5`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | Every dispute path installs a successor fork via `unsafeSetGenesisState`.                                   | `INV-DVP-5.T1.P1` — valid case and direct invalid/opposite<br>`INV-DVP-5.T1.P2` — matching/mismatched commitment, predecessor/genesis, stale and foreign fork<br>`INV-DVP-5.T1.P3` — malformed and adversarial input, partial failure, retry and recovery                                                                                             |
| `INV-DVP-6.T1` | `INV-DVP-6`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | Fraud proofs slash before/independently of the dispute that consumes the slash set.                         | `INV-DVP-6.T1.P1` — valid case and direct invalid/opposite<br>`INV-DVP-6.T1.P2` — new/existing/removed/slashed participant and concurrent membership change<br>`INV-DVP-6.T1.P3` — malformed and adversarial input, partial failure, retry and recovery                                                                                               |
| `REQ-DVP-1.T1` | `REQ-DVP-1`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | Timeout submission respects precedence/race guards (existing window age, calldata grants, forced timeouts). | `REQ-DVP-1.T1.P1` — valid case and direct invalid/opposite<br>`REQ-DVP-1.T1.P2` — before/at/after deadline and maximum honest skew                                                                                                                                                                                                                    |
| `REQ-DVP-2.T1` | `REQ-DVP-2`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | The reducer reads the dispute window through event-synchronized storage (never a window it cannot back).    | `REQ-DVP-2.T1.P1` — valid case and direct invalid/opposite<br>`REQ-DVP-2.T1.P2` — before/at/after deadline and maximum honest skew<br>`REQ-DVP-2.T1.P3` — malformed and adversarial input, partial failure, retry and recovery                                                                                                                        |
| `REQ-DVP-3.T1` | `REQ-DVP-3`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | An incorrect committed reduction is challenged within the challenge period.                                 | `REQ-DVP-3.T1.P1` — valid case and direct invalid/opposite<br>`REQ-DVP-3.T1.P2` — matching/mismatched commitment, predecessor/genesis, stale and foreign fork<br>`REQ-DVP-3.T1.P3` — before/at/after deadline and maximum honest skew                                                                                                                 |
...
| ----------------------- | ----------------------------------------------------------------------------------------------------------- | ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| `INV-DVP-1`             | Per-fork dispute idempotence with rollback on failed upload.                                                | Claimed; audit pending | [src/disputeManager/DisputeManager.ts](../../../../../../src/disputeManager/DisputeManager.ts#L1) (`dispute`)                                                                                                                                            | Engineer audit pending; any divergence named in the evidence remains open. |
| `INV-DVP-2`             | Kill decisions are grounded in canonical Solidity predicates; self-slashing proofs are preflighted.         | Claimed; audit pending | [src/stateManager/DisputeValidationService.ts](../../../../../../src/stateManager/DisputeValidationService.ts#L33) (staticCalls, `validateTimeoutCalldataPostedProof`)                                                                                   | Engineer audit pending; any divergence named in the evidence remains open. |
| `INV-DVP-3`             | Invalid audit ⇔ exactly one stored dispute fraud proof.                                                     | Claimed; audit pending | `validateDispute` + `hasStoredDisputeFraudProof` throw paths                                                                                                                                                                                             | Engineer audit pending; any divergence named in the evidence remains open. |
| `INV-DVP-4`             | Deterministic, order-independent reduction; races classified as convergence.                                | Claimed; audit pending | [src/stateManager/reduction](../../../../../../src/stateManager/reduction) (`classifyReductionRace`, `compute`)                                                                                                                                          | Engineer audit pending; any divergence named in the evidence remains open. |
| `INV-DVP-5`             | Every dispute path installs a successor fork via `unsafeSetGenesisState`.                                   | Claimed; audit pending | [src/stateManager/reduction/ReductionManager.ts](../../../../../../src/stateManager/reduction/ReductionManager.ts#L52) (`completeWithGenesis`), [src/eventHandlers/EventHandler.ts](../../../../../../src/eventHandlers/EventHandler.ts#L1) (final path) | Engineer audit pending; any divergence named in the evidence remains open. |
| `INV-DVP-6`             | Fraud proofs slash before/independently of the dispute that consumes the slash set.                         | Claimed; audit pending | `constructDispute` multicall ordering; `killDispute`                                                                                                                                                                                                     | Engineer audit pending; any divergence named in the evidence remains open. |
| `REQ-DVP-1`             | Timeout submission respects precedence/race guards (existing window age, calldata grants, forced timeouts). | Claimed; audit pending | [src/stateManager/StateManager.ts](../../../../../../src/stateManager/StateManager.ts#L1) (`tryTimeoutParticipant`)                                                                                                                                      | Engineer audit pending; any divergence named in the evidence remains open. |
| `REQ-DVP-2`             | The reducer reads the dispute window through event-synchronized storage (never a window it cannot back).    | Claimed; audit pending | [src/stateManager/EventSyncService.ts](../../../../../../src/stateManager/EventSyncService.ts#L1) (`loadSynchronizedWindowCommitments`, `ensureDisputesProcessed`)                                                                                       | Engineer audit pending; any divergence named in the evidence remains open. |
| `REQ-DVP-3`             | An incorrect committed reduction is challenged within the challenge period.                                 | Claimed; audit pending | [src/eventHandlers/EventHandler.ts](../../../../../../src/eventHandlers/EventHandler.ts#L1) (`validateDisputeReductionAndChallenge`)                                                                                                                     | Engineer audit pending; any divergence named in the evidence remains open. |
