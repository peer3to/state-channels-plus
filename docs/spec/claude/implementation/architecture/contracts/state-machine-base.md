...
> **Specification subject:** [specification/concepts/state-machines.md](../../../specification/concepts/state-machines.md)
...
> [../concepts/state-machines.md](../../../specification/concepts/state-machines.md); this document is the binding
...
| `getTotalStateBalance`                                                        | `function getTotalStateBalance() public view virtual returns (Balance memory)`             | Total value accounted inside the current state (feeds the channel-balance invariant — [../protocol/cross-layer-messages.md](../../../specification/protocol/cross-layer-messages.md)).         |
...
protocol code. See [../protocol/finality.md](../../../specification/protocol/finality.md) for the leader schedule
built on this hook.
...
[OQ-26](../../../specification/open-questions.md) for the resolution options (generic on-chain author check vs.
...
**Decided (2026-08-10, [OQ-18](../../../specification/open-questions.md)):** the wrappers MUST be symmetric — both
...
[../concepts/state-machines.md](../../../specification/concepts/state-machines.md)); the table above documents
...
| `_tx.header.timestamp`                     | The protocol's logical time for this transition (chain-time model: [../protocol/time.md](../../../specification/protocol/time.md)). |
...
[../concepts/history-and-commitments.md](../../../specification/concepts/history-and-commitments.md)).
...
| --------------- | ----------------------- | ----------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `INV-CON-5.T1`  | `INV-CON-5`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | `_setState(getState())` leaves state unchanged; the pair is an exact inverse for all protocol-reachable state.                                                                                                              | `INV-CON-5.T1.P1` — valid case and direct invalid/opposite<br>`INV-CON-5.T1.P2` — zero/empty/no-op where meaningful, exact boundary, failure/recovery and relevant race                                                                                                                                                                                                     |
| `REQ-CON-6.T1`  | `REQ-CON-6`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | Transitions use only the injected context (`_tx.header.*`, own state, calldata args, `gasLimit`); ambient EVM context (`msg.sender`, `block.timestamp`, `msg.data`, …) is prohibited.                                       | `REQ-CON-6.T1.P1` — valid case and direct invalid/opposite<br>`REQ-CON-6.T1.P2` — before/at/after deadline and maximum honest skew                                                                                                                                                                                                                                          |
| `REQ-CON-7.T1`  | `REQ-CON-7`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | Turn authorization is protocol-enforced (SDK pre-execution leader check); in-contract checks are optional defense in depth. Authorization is block-author-level (one tx per block today makes it look tx-level).            | `REQ-CON-7.T1.P1` — valid case and direct invalid/opposite<br>`REQ-CON-7.T1.P2` — correct/wrong/missing/duplicate/forged identity or signature and membership boundary                                                                                                                                                                                                      |
| `REQ-CON-8.T1`  | `REQ-CON-8`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | `subtractBalance` MUST revert on underflow; the balance algebra is deterministic and pure.                                                                                                                                  | `REQ-CON-8.T1.P1` — valid case and direct invalid/opposite<br>`REQ-CON-8.T1.P2` — zero, exact balance/boundary, one beyond, maximum value and conservation                                                                                                                                                                                                                  |
| `REQ-CON-9.T1`  | `REQ-CON-9`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | State serialization is canonical, lossless, deterministic; mappings only with a complete deterministic key-enumeration scheme; commitment is indirect via `block → stateSnapshotHash → snapshotData.stateMachineStateHash`. | `REQ-CON-9.T1.P1` — valid case and direct invalid/opposite<br>`REQ-CON-9.T1.P2` — matching/mismatched commitment, predecessor/genesis, stale and foreign fork                                                                                                                                                                                                               |
| `REQ-CON-10.T1` | `REQ-CON-10`            | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | `_joinChannel` admits a new participant with its initial balance, or tops up an existing participant's balance without changing membership.                                                                                 | `REQ-CON-10.T1.P1` — valid case and direct invalid/opposite<br>`REQ-CON-10.T1.P2` — correct/wrong/missing/duplicate/forged identity or signature and membership boundary<br>`REQ-CON-10.T1.P3` — zero, exact balance/boundary, one beyond, maximum value and conservation<br>`REQ-CON-10.T1.P4` — new/existing/removed/slashed participant and concurrent membership change |
...
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| `INV-CON-5`             | `_setState(getState())` leaves state unchanged; the pair is an exact inverse for all protocol-reachable state.                                                                                                              | Claimed; audit pending | [AStateMachine.sol](../../../../../../contracts/V1/AStateMachine.sol#L3) (abstract pair); [MathStateMachine.sol](../../../../../../contracts/V1/examples/MathStateMachine/MathStateMachine.sol#L4) (`abi.encode`/`abi.decode` of one struct)                          | Engineer audit pending; any divergence named in the evidence remains open. |
| `REQ-CON-6`             | Transitions use only the injected context (`_tx.header.*`, own state, calldata args, `gasLimit`); ambient EVM context (`msg.sender`, `block.timestamp`, `msg.data`, …) is prohibited.                                       | Claimed; audit pending | [AStateMachine.sol](../../../../../../contracts/V1/AStateMachine.sol#L3) (`stateTransition` sets `_tx.header`); [MathStateMachine.sol](../../../../../../contracts/V1/examples/MathStateMachine/MathStateMachine.sol#L4) (author checks via `_tx.header.participant`) | Engineer audit pending; any divergence named in the evidence remains open. |
| `REQ-CON-7`             | Turn authorization is protocol-enforced (SDK pre-execution leader check); in-contract checks are optional defense in depth. Authorization is block-author-level (one tx per block today makes it look tx-level).            | Claimed; audit pending | [ValidationService.ts](../../../../../../src/stateManager/ValidationService.ts#L1); optional in-contract guards: [MathStateMachine.sol](../../../../../../contracts/V1/examples/MathStateMachine/MathStateMachine.sol#L4) (`add`, `leaveChannel` requires)            | Engineer audit pending; any divergence named in the evidence remains open. |
| `REQ-CON-8`             | `subtractBalance` MUST revert on underflow; the balance algebra is deterministic and pure.                                                                                                                                  | Claimed; audit pending | [AStateMachine.sol](../../../../../../contracts/V1/AStateMachine.sol#L3) (contract comment); [MathStateMachine.sol](../../../../../../contracts/V1/examples/MathStateMachine/MathStateMachine.sol#L4) (`require(balance1.amount >= balance2.amount)`)                 | Engineer audit pending; any divergence named in the evidence remains open. |
| `REQ-CON-9`             | State serialization is canonical, lossless, deterministic; mappings only with a complete deterministic key-enumeration scheme; commitment is indirect via `block → stateSnapshotHash → snapshotData.stateMachineStateHash`. | Claimed; audit pending | [AStateMachine.sol](../../../../../../contracts/V1/AStateMachine.sol#L3); [DataTypes.sol](../../../../../../contracts/V1/types/DataTypes.sol#L3) (`Block`, `StateSnapshot`, `SnapshotData`)                                                                           | Engineer audit pending; any divergence named in the evidence remains open. |
| `REQ-CON-10`            | `_joinChannel` admits a new participant with its initial balance, or tops up an existing participant's balance without changing membership.                                                                                 | Claimed; audit pending | [AStateMachine.sol](../../../../../../contracts/V1/AStateMachine.sol#L3) (hook contract comment); [MathStateMachine.sol](../../../../../../contracts/V1/examples/MathStateMachine/MathStateMachine.sol#L4) (`_joinChannel` both branches)                             | Engineer audit pending; any divergence named in the evidence remains open. |
