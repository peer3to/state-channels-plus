...
> **Specification subject:** [specification/architecture/contracts.md](../../../specification/architecture/contracts.md)
...
> here; the binding semantics live in [../protocol/disputes.md](../../../specification/protocol/disputes.md),
> [../protocol/fraud-proofs.md](../../../specification/protocol/fraud-proofs.md),
> [../protocol/state-proofs.md](../../../specification/protocol/state-proofs.md), and
...
> [../reference/data-types.md](../../../specification/reference/data-types.md).
...
Behavior: [../protocol/disputes.md](../../../specification/protocol/disputes.md),
[../protocol/fraud-proofs.md](../../../specification/protocol/fraud-proofs.md).
...
Behavior: [../protocol/state-proofs.md](../../../specification/protocol/state-proofs.md),
[../protocol/cross-layer-messages.md](../../../specification/protocol/cross-layer-messages.md).
...
| `evidenceTime`                           | 30                          | Length of the dispute evidence window, the kill period (measured from the last evidence submission), and the reduction challenge period; also the first-block grace term. [../protocol/disputes.md](../../../specification/protocol/disputes.md), [../protocol/time.md](../../../specification/protocol/time.md). |
...
- Effect on-chain is an appended inbound `JOIN` message block; the channel applies it off-chain via
  the inbound stream ([../protocol/cross-layer-messages.md](../../../specification/protocol/cross-layer-messages.md)),
...
([../protocol/state-proofs.md](../../../specification/protocol/state-proofs.md)): `verifyStateProof` (full check
...
reduction engine ([../protocol/disputes.md](../../../specification/protocol/disputes.md)): `reduce` (deterministic,
...
[../protocol/cross-layer-messages.md](../../../specification/protocol/cross-layer-messages.md)). Contains live
...
([../protocol/fraud-proofs.md](../../../specification/protocol/fraud-proofs.md)). `applyFraudProofs(proofs, context)`:
...
| --------------- | ----------------------- | ----------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `REQ-CON-11.T1` | `REQ-CON-11`            | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | `open` MUST reject duplicate participants and already-open channels, verify a unanimous threshold signature over `encodedOpenChannel`, and require ≥ 2 successful deposits before storing the genesis snapshot.                          | `REQ-CON-11.T1.P1` — valid case and direct invalid/opposite<br>`REQ-CON-11.T1.P2` — matching/mismatched commitment, predecessor/genesis, stale and foreign fork<br>`REQ-CON-11.T1.P3` — correct/wrong/missing/duplicate/forged identity or signature and membership boundary<br>`REQ-CON-11.T1.P4` — zero, exact balance/boundary, one beyond, maximum value and conservation<br>`REQ-CON-11.T1.P5` — duplicate/replay, every relevant permutation and concurrent delivery<br>`REQ-CON-11.T1.P6` — malformed and adversarial input, partial failure, retry and recovery |
| `INV-CON-12.T1` | `INV-CON-12`            | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | Processed withdrawals never exceed resolved deposits for a channel: every outbound message application re-checks `totalWithdrawals ≤ totalDeposits` (`CantWithdrawMoreThanDeposits`).                                                    | `INV-CON-12.T1.P1` — valid case and direct invalid/opposite<br>`INV-CON-12.T1.P2` — zero, exact balance/boundary, one beyond, maximum value and conservation                                                                                                                                                                                                                                                                                                                                                                                                            |
| `REQ-CON-13.T1` | `REQ-CON-13`            | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | Block-calldata commitments are append-only and author-bound: no overwrite, `msg.sender` must be the block's author, and posting must beat `maxTimestamp`.                                                                                | `REQ-CON-13.T1.P1` — valid case and direct invalid/opposite<br>`REQ-CON-13.T1.P2` — matching/mismatched commitment, predecessor/genesis, stale and foreign fork<br>`REQ-CON-13.T1.P3` — correct/wrong/missing/duplicate/forged identity or signature and membership boundary<br>`REQ-CON-13.T1.P4` — before/at/after deadline and maximum honest skew                                                                                                                                                                                                                   |
| `REQ-CON-14.T1` | `REQ-CON-14`            | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | Dispute uploads are participant-gated and throttled: disputer == `msg.sender`, disputer eligible (snapshot ∪ pending − slashed), one window-opening upload per `evidenceTime` per address, one evidence post per participant per window. | `REQ-CON-14.T1.P1` — valid case and direct invalid/opposite<br>`REQ-CON-14.T1.P2` — matching/mismatched commitment, predecessor/genesis, stale and foreign fork<br>`REQ-CON-14.T1.P3` — correct/wrong/missing/duplicate/forged identity or signature and membership boundary<br>`REQ-CON-14.T1.P4` — before/at/after deadline and maximum honest skew<br>`REQ-CON-14.T1.P5` — new/existing/removed/slashed participant and concurrent membership change<br>`REQ-CON-14.T1.P6` — malformed and adversarial input, partial failure, retry and recovery                    |
| `REQ-CON-15.T1` | `REQ-CON-15`            | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | A committed dispute proven fraudulent during the kill period MUST be killed and its disputer slashed; an invalid dispute-fraud-proof submission slashes the submitter instead.                                                           | `REQ-CON-15.T1.P1` — valid case and direct invalid/opposite<br>`REQ-CON-15.T1.P2` — matching/mismatched commitment, predecessor/genesis, stale and foreign fork<br>`REQ-CON-15.T1.P3` — before/at/after deadline and maximum honest skew<br>`REQ-CON-15.T1.P4` — new/existing/removed/slashed participant and concurrent membership change<br>`REQ-CON-15.T1.P5` — malformed and adversarial input, partial failure, retry and recovery                                                                                                                                 |
...
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| `REQ-CON-11`            | `open` MUST reject duplicate participants and already-open channels, verify a unanimous threshold signature over `encodedOpenChannel`, and require ≥ 2 successful deposits before storing the genesis snapshot.                          | Claimed; audit pending | [StateChannelManagerProxy.sol](../../../../../../contracts/V1/StateChannelDiamondProxy/StateChannelManagerProxy.sol#L3) (`open`)                                                                                                                                                         | Engineer audit pending; any divergence named in the evidence remains open. |
| `INV-CON-12`            | Processed withdrawals never exceed resolved deposits for a channel: every outbound message application re-checks `totalWithdrawals ≤ totalDeposits` (`CantWithdrawMoreThanDeposits`).                                                    | Claimed; audit pending | [StateSnapshotFacet.sol](../../../../../../contracts/V1/StateChannelDiamondProxy/StateSnapshotFacet.sol#L3) (`_applyOutboundMessageBlocks`)                                                                                                                                              | Engineer audit pending; any divergence named in the evidence remains open. |
| `REQ-CON-13`            | Block-calldata commitments are append-only and author-bound: no overwrite, `msg.sender` must be the block's author, and posting must beat `maxTimestamp`.                                                                                | Claimed; audit pending | [StateChannelManagerProxy.sol](../../../../../../contracts/V1/StateChannelDiamondProxy/StateChannelManagerProxy.sol#L3) (`postBlockCalldata`)                                                                                                                                            | Engineer audit pending; any divergence named in the evidence remains open. |
| `REQ-CON-14`            | Dispute uploads are participant-gated and throttled: disputer == `msg.sender`, disputer eligible (snapshot ∪ pending − slashed), one window-opening upload per `evidenceTime` per address, one evidence post per participant per window. | Claimed; audit pending | [DisputeManagerFacet.sol](../../../../../../contracts/V1/StateChannelDiamondProxy/DisputeManagerFacet.sol#L3) (`_uploadDispute`); [StateChannelManagerStorage.sol](../../../../../../contracts/V1/StateChannelDiamondProxy/StateChannelManagerStorage.sol#L3) (`disputerThrottle`)       | Engineer audit pending; any divergence named in the evidence remains open. |
| `REQ-CON-15`            | A committed dispute proven fraudulent during the kill period MUST be killed and its disputer slashed; an invalid dispute-fraud-proof submission slashes the submitter instead.                                                           | Claimed; audit pending | [DisputeFraudProofFacet.sol](../../../../../../contracts/V1/StateChannelDiamondProxy/DisputeFraudProofFacet.sol#L3) (`applyDisputeFraudProofs`); [DisputeVerificationFacet.sol](../../../../../../contracts/V1/StateChannelDiamondProxy/DisputeVerificationFacet.sol#L3) (`killDispute`) | Engineer audit pending; any divergence named in the evidence remains open. |
