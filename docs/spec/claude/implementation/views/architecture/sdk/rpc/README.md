...
| [`INV-RPC-2-6DYF4E`](README.md#inv-rpc-2-6dyf4e) | A pending request is settled only by the addressed peer (checksum identity, transport upgrades tolerated); any other responder is disconnected and blacklisted. |
| [`INV-RPC-3-Y6PE7S`](README.md#inv-rpc-3-y6pe7s) | Guards are bypassed only for `isTrusted` transports, and the only trusted transport is the in-process loopback (never a network peer). |
| [`INV-RPC-4-RFDNH3`](README.md#inv-rpc-4-rfdnh3) | Raw `BigInt` values never cross the wire: serialization throws at the sender; protocol structs cross as canonical Codec encodings. |
| [`INV-RPC-5-BCEZVC`](README.md#inv-rpc-5-bcezvc) | RPC dispatch and handlers never hold the `StateManager` mutex; live-state mutation happens only through the pipeline's mutex-acquiring entry points. |
