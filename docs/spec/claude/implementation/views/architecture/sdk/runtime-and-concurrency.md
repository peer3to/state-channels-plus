...
| --------------- | ----------------------- | ----------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `REQ-RUN-1.T1`  | `REQ-RUN-1`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | Communication is serialized messages over paired ports; the protocol and observable behavior are identical inline or worker, so callers keep no separate implementations and a component can move to a worker without changing its contract.                                                                                                | `REQ-RUN-1.T1.P1` — valid case and direct invalid/opposite<br>`REQ-RUN-1.T1.P2` — zero/empty/no-op where meaningful, exact boundary, failure/recovery and relevant race                                                                                                    |
| `REQ-RUN-2.T1`  | `REQ-RUN-2`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | No shared mutable memory between main thread, host, and executor; all inter-context interaction is a serialized message.                                                                                                                                                                                                                    | `REQ-RUN-2.T1.P1` — valid case and direct invalid/opposite<br>`REQ-RUN-2.T1.P2` — zero/empty/no-op where meaningful, exact boundary, failure/recovery and relevant race                                                                                                    |
| `REQ-RUN-3.T1`  | `REQ-RUN-3`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | Internal state is owned by the receiving component; the app cannot reach node state, the host cannot reach EVM state, except by a request.                                                                                                                                                                                                  | `REQ-RUN-3.T1.P1` — valid case and direct invalid/opposite<br>`REQ-RUN-3.T1.P2` — zero/empty/no-op where meaningful, exact boundary, failure/recovery and relevant race                                                                                                    |
| `REQ-RUN-4.T1`  | `REQ-RUN-4`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | Ownership, ordering, lifecycle, error, disposal, and serialization rules for a cross-boundary message are explicit before a component moves to a worker.                                                                                                                                                                                    | `REQ-RUN-4.T1.P1` — valid case and direct invalid/opposite<br>`REQ-RUN-4.T1.P2` — duplicate/replay, every relevant permutation and concurrent delivery                                                                                                                     |
| `REQ-RUN-5.T1`  | `REQ-RUN-5`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | Every client→host request carries a client-local `requestId`; a single handler per port correlates the one matching response; only the paired context can settle it.                                                                                                                                                                        | `REQ-RUN-5.T1.P1` — valid case and direct invalid/opposite<br>`REQ-RUN-5.T1.P2` — zero/empty/no-op where meaningful, exact boundary, failure/recovery and relevant race                                                                                                    |
| `REQ-RUN-6.T1`  | `REQ-RUN-6`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | Cross-boundary values obey structured-clone limits: bigints as quantity/Codec strings, manifests not class instances, only ports transferred; `customData`/`kzg` rejected; non-serializable bus args drop.                                                                                                                                  | `REQ-RUN-6.T1.P1` — valid case and direct invalid/opposite<br>`REQ-RUN-6.T1.P2` — zero, exact balance/boundary, one beyond, maximum value and conservation<br>`REQ-RUN-6.T1.P3` — malformed and adversarial input, partial failure, retry and recovery                     |
| `REQ-RUN-7.T1`  | `REQ-RUN-7`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | Worker isolation is a client performance strategy (not server scaling), applied incrementally where measurement justifies it; each boundary should be measurement-justified.                                                                                                                                                                | `REQ-RUN-7.T1.P1` — valid case and direct invalid/opposite<br>`REQ-RUN-7.T1.P2` — duplicate/replay, every relevant permutation and concurrent delivery                                                                                                                     |
| `INV-RUN-1.T1`  | `INV-RUN-1`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | Ports parallelize across contexts; the host mutex serializes live-state mutation, preserved across the executor worker because executor calls are awaited under the mutex.                                                                                                                                                                  | `INV-RUN-1.T1.P1` — valid case and direct invalid/opposite<br>`INV-RUN-1.T1.P2` — zero/empty/no-op where meaningful, exact boundary, failure/recovery and relevant race                                                                                                    |
| `INV-RUN-2.T1`  | `INV-RUN-2`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | 1:1 port pair; one transfer owner; one handler per port; correlation trusts the paired writer.                                                                                                                                                                                                                                              | `INV-RUN-2.T1.P1` — valid case and direct invalid/opposite<br>`INV-RUN-2.T1.P2` — zero/empty/no-op where meaningful, exact boundary, failure/recovery and relevant race                                                                                                    |
| `INV-RUN-3.T1`  | `INV-RUN-3`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | Host construction failure posts `hostError` + closes the port (client `ready` rejects, pending reject); dead client port self-disposes the host.                                                                                                                                                                                            | `INV-RUN-3.T1.P1` — valid case and direct invalid/opposite<br>`INV-RUN-3.T1.P2` — malformed and adversarial input, partial failure, retry and recovery                                                                                                                     |
| `INV-RUN-4.T1`  | `INV-RUN-4`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | The signing key and nonce never cross a boundary; only the host signs; the client holds proxy signers.                                                                                                                                                                                                                                      | `INV-RUN-4.T1.P1` — valid case and direct invalid/opposite<br>`INV-RUN-4.T1.P2` — correct/wrong/missing/duplicate/forged identity or signature and membership boundary                                                                                                     |
| `REQ-RUN-8.T1`  | `REQ-RUN-8`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | Harness control is a custom-RPC root over the same serialized surfaces; a controller drives a peer only through `hostRpc` (loopback to that peer's host, or a typed relay to a peer address), never by in-process reference.                                                                                                                | `REQ-RUN-8.T1.P1` — valid case and direct invalid/opposite<br>`REQ-RUN-8.T1.P2` — correct/wrong/missing/duplicate/forged identity or signature and membership boundary                                                                                                     |
| `REQ-RUN-9.T1`  | `REQ-RUN-9`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | Every harness-control operation returns structured-clone-serializable projections; bigint-bearing structs cross `Codec.encode`d and are decoded in the host endpoint.                                                                                                                                                                       | `REQ-RUN-9.T1.P1` — valid case and direct invalid/opposite<br>`REQ-RUN-9.T1.P2` — zero/empty/no-op where meaningful, exact boundary, failure/recovery and relevant race                                                                                                    |
| `REQ-RUN-10.T1` | `REQ-RUN-10`            | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | The harness-control root is not reachable by a network peer and is not present in a production artifact.                                                                                                                                                                                                                                    | `REQ-RUN-10.T1.P1` — valid case and direct invalid/opposite<br>`REQ-RUN-10.T1.P2` — correct/wrong/missing/duplicate/forged identity or signature and membership boundary                                                                                                   |
| `REQ-RUN-11.T1` | `REQ-RUN-11`            | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | Concurrently executing peers and test processes are isolated: per-peer host realm and service state, disjoint account partitions per process, per-test channel id, per-slot chain/discovery/cache.                                                                                                                                          | `REQ-RUN-11.T1.P1` — valid case and direct invalid/opposite<br>`REQ-RUN-11.T1.P2` — correct/wrong/missing/duplicate/forged identity or signature and membership boundary<br>`REQ-RUN-11.T1.P3` — new/existing/removed/slashed participant and concurrent membership change |
| `REQ-RUN-12.T1` | `REQ-RUN-12`            | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | The inline-vs-worker equivalence criterion states which observations must be identical across `RUN_SDK_IN_THREAD × VM_DEDICATED_THREAD`, and the matrix test is that criterion's oracle.                                                                                                                                                    | `REQ-RUN-12.T1.P1` — valid case and direct invalid/opposite<br>`REQ-RUN-12.T1.P2` — zero/empty/no-op where meaningful, exact boundary, failure/recovery and relevant race                                                                                                  |
| `REQ-RUN-13.T1` | `REQ-RUN-13`            | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | Both worker boundaries (runtime host, EVM executor) are the intended defaults, uniformly — no per-environment profile branching; today's off-by-default state is a maturity gap. Engineer decision 2026-08-10 (§6.1).                                                                                                                       | `REQ-RUN-13.T1.P1` — valid case and direct invalid/opposite<br>`REQ-RUN-13.T1.P2` — before/at/after deadline and maximum honest skew                                                                                                                                       |
| `REQ-RUN-14.T1` | `REQ-RUN-14`            | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | A ~6-participant channel must run within a mid-range mobile browser envelope (~4 GB device RAM, few-hundred-MB usable heap, mobile CPU). The envelope is a hard implementation budget: three contexts must be made to fit it. The 1024 MB worker cap is CI containment and is too high for this floor. Engineer decision 2026-08-10 (§6.1). | `REQ-RUN-14.T1.P1` — valid case and direct invalid/opposite<br>`REQ-RUN-14.T1.P2` — correct/wrong/missing/duplicate/forged identity or signature and membership boundary                                                                                                   |
| `REQ-RUN-15.T1` | `REQ-RUN-15`            | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | Inline and worker mode are equivalent when observable protocol behavior matches: blocks/signatures/state roots, on-chain actions, emitted events and payloads, and error identity. Timing and internal interleaving are out of scope.                                                                                                       | `REQ-RUN-15.T1.P1` — valid case and direct invalid/opposite<br>`REQ-RUN-15.T1.P2` — correct/wrong/missing/duplicate/forged identity or signature and membership boundary                                                                                                   |
| `INV-RUN-5.T1`  | `INV-RUN-5`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | A harness-control operation executes in the host realm on live objects and returns only serializable projections; no live `Block`, transport, profile, or `StateManager` reference crosses the port.                                                                                                                                        | `INV-RUN-5.T1.P1` — valid case and direct invalid/opposite<br>`INV-RUN-5.T1.P2` — zero/empty/no-op where meaningful, exact boundary, failure/recovery and relevant race                                                                                                    |
| `INV-RUN-6.T1`  | `INV-RUN-6`             | Exercise the real public component or contract boundary, including rejection and failure paths without partial effects. | Harness stub state (installed doubles, saved originals, registered peer keys) is scoped to one peer's host service instance, so co-located inline hosts do not leak faults between peers.                                                                                                                                                   | `INV-RUN-6.T1.P1` — valid case and direct invalid/opposite<br>`INV-RUN-6.T1.P2` — correct/wrong/missing/duplicate/forged identity or signature and membership boundary                                                                                                     |
