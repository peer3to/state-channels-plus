...
| ID                                                     | Invariant                                                                                                                                                                                                           |
| ------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`INV-JCS-1-SRJKVK`](join-channel.md#inv-jcs-1-srjkvk) | A responder signs a join only if the ECDSA signer, the declared `participant`, and the authenticated transport peer are the same address (`signJoinRequest` stage 3).                                               |
| [`INV-JCS-2-95QM5S`](join-channel.md#inv-jcs-2-95qm5s) | A responder signs only a join pinned to its own current on-chain snapshot and fork (`expectedForkId`/`expectedSnapshotHash` equality, stages 6–7) and only when the local signer is in the threshold set (stage 8). |
| [`INV-JCS-3-XJ8KGX`](join-channel.md#inv-jcs-3-xj8kgx) | The collector accepts a returned signature only if it recovers to the addressed threshold participant; one mismatched or missing signature fails the entire collection (unanimity).                                 |
| [`INV-JCS-4-1FJ7CH`](join-channel.md#inv-jcs-4-1fj7ch) | The collector only ever collects for the local node's own participant identity (stage 1).                                                                                                                           |
| [`INV-JCS-5-16V4VG`](join-channel.md#inv-jcs-5-16v4vg) | `joinChannelService` holds no state between calls; every decision is derived from arguments plus live chain/state-manager reads at call time.                                                                       |
