...
Trigger: [`EventHandler.handleDisputeCommitted`](../../../../../../../src/eventHandlers/EventHandler.ts#L364).
Evidence consumer:
...
([`handleDisputeCommitted`](../../../../../../../src/eventHandlers/EventHandler.ts#L364)); late non-final
...
- **Can an adversary _time_ the round to hit a known-offline victim?** Partially. Round timing
  ...
  (throttled to one per `evidenceTime` per channel, [`REQ-DIS-2-PKVZ7E` (Upload is limited to eligible disputers)](../../../../../specification/disputes/disputes.md#req-dis-2-pkvz7e) —
...
<a id="inv-ifd-1-hbjr2p"></a>

### INV-IFD-1-HBJR2P — One recorded acknowledgment per peer and fork

Per direction and per `(peerAddress, forkId)`, at most one acknowledgment is ever recorded; a second incoming request for a recorded pair is a violation with a defined consequence (disconnect + blacklist).

- [x] `INV-IFD-1-HBJR2P.T1.P4` — duplicate delivery

<a id="inv-ifd-2-6n9g29"></a>

### INV-IFD-2-6N9G29 — Acknowledge only a verified disputed fork

A `true` acknowledgment is returned only after the fork was verified disputed against the local mirror or the chain; the record is written before the reply is sent.

<a id="inv-ifd-3-dz83bb"></a>

### INV-IFD-3-DZ83BB — One outgoing round per fork

The outgoing round runs at most once per fork per process, against the peer set snapshot taken at round start; peers connecting later are neither asked nor punished for that fork.

- [x] `INV-IFD-3-DZ83BB.T1.P1` — valid case

<a id="inv-ifd-4-5j5w3t"></a>

### INV-IFD-4-5J5W3T — Acknowledgments keyed by EVM address

Acknowledgment state is keyed by EVM address, never by transport, and survives transport replacement.

<a id="req-ifd-1-x2vcjw"></a>

### REQ-IFD-1-X2VCJW — Request acknowledgment once per fork

On a relevant `DisputeCommitted` event, the node requests acknowledgment from every connected peer exactly once per fork, with a `2 × agreementTime` reply window.

<a id="req-ifd-2-13862z"></a>

### REQ-IFD-2-13862Z — Acknowledge only verified disputes

A responder acknowledges only a fork verified disputed (local mirror, then chain); a false claim, duplicate request, or missing sender identity disconnects and blacklists the requester.

<a id="req-ifd-3-qxncn9"></a>

### REQ-IFD-3-QXNCN9 — Non-acknowledging peers are disconnected

A peer that rejects, errors, replies `false`, or exceeds the window is disconnected and blacklisted by the requester.

<a id="req-ifd-4-26fwyz"></a>

### REQ-IFD-4-26FWYZ — Acknowledgments gate dead-fork punishment

Recorded peer acknowledgments gate the dead-fork punishment: acknowledged suppliers of disputed-fork blocks are cut; unacknowledged suppliers retain straggler tolerance.

- [x] `REQ-IFD-4-26FWYZ.T1.P1` — valid case
