...
| `applySyncResponse`            | runtime left the channel the request was for (channel generation changed)                                              | Return false before the first write, before persistence, and before each replayed block; no abort, no peer cut                 |
