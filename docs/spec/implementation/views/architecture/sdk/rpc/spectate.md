...
obligation ([`REQ-MSG-9-BFN9P5` (Spectating MUST be fail-closed)](../../../../../specification/settlement/cross-layer-messages.md#req-msg-9-bfn9p5)). It is the mandatory first step before joining
...
11. **Channel-balance invariant.** `verifyBalanceInvariantCheckSnapshot.staticCall(...)` on the
    latest finalized snapshot ([`INV-MSG-6-1C22RD` (Balance invariant)](../../../../../specification/settlement/cross-layer-messages.md#inv-msg-6-1c22rd) / §6 of the protocol doc); abort on failure. This is the
...
- A snapshot whose in-channel balances exceed deposits minus withdrawals fails
  `verifyBalanceInvariantCheckSnapshot` (step 11, [`INV-MSG-6-1C22RD` (Balance invariant)](../../../../../specification/settlement/cross-layer-messages.md#inv-msg-6-1c22rd)) — this is the specific defense against
...
A returned `SyncPayload` is validated against the requester's own on-chain reads and contract logic (dispute walk, milestones, outbound ranges, balance invariant, simulated advance) before any state effect; nothing in the payload is trusted on receipt.
...
Spectating is fail-closed: any verification failure aborts (fresh spectator → full stop; participant → cut peer) with no partial local commitment ([`REQ-MSG-9-BFN9P5` (Spectating MUST be fail-closed)](../../../../../specification/settlement/cross-layer-messages.md#req-msg-9-bfn9p5)).
...
- [`REQ-SPC-2-45C3CT`](spectate.md#req-spc-2-45c3ct)
  Missing: [`DEF-5-E8TP9N`](../../../../../audit/open-findings.md#def-5-e8tp9n) (over-broad blacklist)
