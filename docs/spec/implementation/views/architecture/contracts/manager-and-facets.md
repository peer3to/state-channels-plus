...
- Everything else reaches
  ...
  which delegatecalls the facet that
...
- Routing-table entries are written `Facet.fn.selector`, so the compiler derives each selector from
  the facet's own function type; changing a facet signature changes the routing with it.
...
- **Unknown selectors** resolve to `consumerFacetAddress`
