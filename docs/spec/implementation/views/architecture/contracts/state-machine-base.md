...
<a id="inv-con-5-t1b2eg"></a>

#### INV-CON-5-T1B2EG — State encoding round-trips exactly

`_setState(getState())` MUST leave the state unchanged, and
...
<a id="req-con-7-dxvw98"></a>

#### REQ-CON-7-DXVW98 — Turn authorization is protocol-enforced
...
<a id="req-con-10-cn3fcd"></a>

#### REQ-CON-10-CN3FCD — Join admits or tops up a participant

`_joinChannel(JoinChannel)` MUST implement two cases:
...
<a id="req-con-8-pghk0w"></a>

#### REQ-CON-8-PGHK0W — Balance subtraction rejects underflow

`subtractBalance(a, b)` MUST revert when `b` exceeds `a` under the application's
...
<a id="req-con-6-35xmja"></a>

### REQ-CON-6-35XMJA — Transitions use only the injected context

During a transition, a state machine MUST derive its behavior only from:
...
<a id="req-con-9-1ajc5b"></a>

### REQ-CON-9-1AJC5B — Canonical state serialization

The `getState`/`_setState` encoding MUST be canonical, lossless, and deterministic:
...
Ordinary application transitions may change participants as well as lifecycle joins/removals. MathStateMachine demonstrates a balance-funded insertion under configured N, sharing its append helper with joins. Valid at-capacity insertion advances only the turn; it neither creates funds nor promotes a new member. This does not close generic genesis/snapshot maximum-enforcement gaps. Local promotion/finality can precede on-chain dispute standing; prompt publication after final promotion remains future work. See [MathStateMachine.sol](../../../source/contracts/V1/examples/MathStateMachine/MathStateMachine.sol.md).
