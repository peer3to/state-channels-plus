...
**[`INV-CON-5-T1B2EG`](state-machine-base.md#inv-con-5-t1b2eg).** `_setState(getState())` MUST leave the state unchanged, and
...
**[`REQ-CON-7-DXVW98`](state-machine-base.md#req-con-7-dxvw98).** Turn authorization is enforced by the protocol layer, not by the state machine:
...
**[`REQ-CON-10-CN3FCD`](state-machine-base.md#req-con-10-cn3fcd).** `_joinChannel(JoinChannel)` MUST implement two cases:
...
**[`REQ-CON-8-PGHK0W`](state-machine-base.md#req-con-8-pghk0w).** `subtractBalance(a, b)` MUST revert when `b` exceeds `a` under the application's
...
**[`REQ-CON-6-35XMJA`](state-machine-base.md#req-con-6-35xmja).** During a transition, a state machine MUST derive its behavior only from:
...
**[`REQ-CON-9-1AJC5B`](state-machine-base.md#req-con-9-1ajc5b).** The `getState`/`_setState` encoding MUST be canonical, lossless, and deterministic:
