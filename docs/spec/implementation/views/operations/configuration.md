...
| [`REQ-CONFIG-3-J4H12F`](../../../specification/runtime/configuration.md#req-config-3-j4h12f) | Missing               | [`config.ts`](../../../../../src/utils/config.ts) supplies defaults and accepts any override without a declared range; only local constructor checks exist, such as `QueueStorage`'s participant bound. | No declared range per key, no startup rejection naming the offending key, and no guarantee that rejection precedes signing or durable writes. |
