...
- **The budget is measured on the encoded line.** What counts against the bound is what would go on
  the wire, so a store never holds more than roughly its bound unpacked ([`REQ-LOG-3-T9FM2K` (Writing a log line does not disturb the session)](../../../../../specification/runtime/log-collection.md#req-log-3-t9fm2k)).
...
- **A store id separates runs.** Sequence numbers restart per store, so without the id a second run
  would overwrite the first's chunks at the receiver ([`REQ-LOG-6-Q8KY4N` (One run's logs never overwrite another's)](../../../../../specification/runtime/log-collection.md#req-log-6-q8ky4n)).
- **A disabled store keeps nothing.** With collection off there is nothing to bound.

- **The store id is 64 random bits from the platform CSPRNG.** Two runs of one participant file under different store directories, so a second run's sequence numbers can not overwrite or merge into the first's ([`REQ-LOG-6-Q8KY4N` (One run's logs never overwrite another's)](../../../../../specification/runtime/log-collection.md#req-log-6-q8ky4n)). `getRandomValues`, not `randomUUID`: the file is browser-compiled and the latter needs a secure context.

## Inputs, outputs, state, and side effects

The store size limit must be finite and non-negative; invalid limits reject construction. Zero capacity retains no entries. Pruning evicts old entries to keep the stored encoded-size estimate within that limit.
...
- The oldest lines go first and the delta shows where they went ([`REQ-LOG-3-T9FM2K` (Writing a log line does not disturb the session)](../../../../../specification/runtime/log-collection.md#req-log-3-t9fm2k)).
- The sequence stays monotonic across eviction ([`REQ-LOG-5-ST6S0G` (Sending twice does not store twice)](../../../../../specification/runtime/log-collection.md#req-log-5-st6s0g)).
- Each store names itself so runs stay apart at the receiver ([`REQ-LOG-6-Q8KY4N` (One run's logs never overwrite another's)](../../../../../specification/runtime/log-collection.md#req-log-6-q8ky4n)).
...
| ----------------------------------------------------------------------- | ------------------------------------------ | -------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <a id="unit-test-log-store-1-279z99"></a>`UNIT-TEST-LOG-STORE-1-279Z99` | The buffer's sequence and delta semantics. | Construct a store with a small bound; store past it; read deltas past several cursors. | Sequence numbers monotonic; delta range and entries consistent; nothing kept past the bound. | <a id="unit-test-log-store-1-279z99.p1"></a>`UNIT-TEST-LOG-STORE-1-279Z99.P1` — sequence numbers stay monotonic across eviction; <a id="unit-test-log-store-1-279z99.p2"></a>`UNIT-TEST-LOG-STORE-1-279Z99.P2` — a delta holds only entries past the cursor; <a id="unit-test-log-store-1-279z99.p3"></a>`UNIT-TEST-LOG-STORE-1-279Z99.P3` — an empty delta leaves the cursor where it was; <a id="unit-test-log-store-1-279z99.p4"></a>`UNIT-TEST-LOG-STORE-1-279Z99.P4` — a delta whose start jumped past the cursor shows the gap; <a id="unit-test-log-store-1-279z99.p5"></a>`UNIT-TEST-LOG-STORE-1-279Z99.P5` — the store id is 64 random bits no two stores share; <a id="unit-test-log-store-1-279z99.p6"></a>`UNIT-TEST-LOG-STORE-1-279Z99.P6` — An infinite store limit rejects.; <a id="unit-test-log-store-1-279z99.p7"></a>`UNIT-TEST-LOG-STORE-1-279Z99.P7` — An oversized entry is evicted.; <a id="unit-test-log-store-1-279z99.p8"></a>`UNIT-TEST-LOG-STORE-1-279Z99.P8` — A zero limit retains no entries.; <a id="unit-test-log-store-1-279z99.p9"></a>`UNIT-TEST-LOG-STORE-1-279Z99.P9` — A NaN store limit rejects.; <a id="unit-test-log-store-1-279z99.p10"></a>`UNIT-TEST-LOG-STORE-1-279Z99.P10` — A negative store limit rejects. |
