...
Unhandled capture calls Logger.error once; that path schedules local uploading and optional gossip. Destroy removes crash hooks and its diagnostic logger reference. An already-awaited upload can complete or retry without logging through a disposed instance. The delta queue still owns at most one active upload and one follow-up.
...
- **A depth-one queue.** A second `uploadLogs` while one is in flight returns one queued follow-up
  ...
  the initiating logger returns that local result; the optional service gossips separately.
- **The empty-delta check sits above the jitter sleep.** A collection fanning out onto an idle realm
  costs neither HTTP nor wall time ([`REQ-LOG-3-T9FM2K` (Writing a log line does not disturb the session)](../../../../../specification/runtime/log-collection.md#req-log-3-t9fm2k)).
- **The watermark moves only on a confirmed POST, and one retry is built in.** A failed send leaves
  ...
  a 2xx ([`REQ-LOG-5-ST6S0G` (Sending twice does not store twice)](../../../../../specification/runtime/log-collection.md#req-log-5-st6s0g)).
- **A changed identity resets the watermark.** Lines stored under the placeholder identity are sent
  again under the real one ([`REQ-LOG-4-W5XR7Q` (Every line says where it came from)](../../../../../specification/runtime/log-collection.md#req-log-4-w5xr7q)).
- **The body names the store and the sequence range.** The receiver can tell runs apart and
  de-duplicate an overlapping resend without remembering anything ([`REQ-LOG-6-Q8KY4N` (One run's logs never overwrite another's)](../../../../../specification/runtime/log-collection.md#req-log-6-q8ky4n)).

- **Identity and delta are read together, after the jitter sleep.** A channel or participant set during the sleep files that batch; the cursor is only read before the sleep (to skip an idle realm without paying it) and reset for a changed key next to the delta, so nothing is filed under the old bucket and sent again under the new one ([`REQ-LOG-4-W5XR7Q` (Every line says where it came from)](../../../../../specification/runtime/log-collection.md#req-log-4-w5xr7q)).
...
- The outcome separates ok from failed and carries the count ([`REQ-LOG-2-N6BJ3D` (The caller receives its local upload outcome)](../../../../../specification/runtime/log-collection.md#req-log-2-n6bj3d)).
- Nothing new means no POST and no sleep ([`REQ-LOG-3-T9FM2K` (Writing a log line does not disturb the session)](../../../../../specification/runtime/log-collection.md#req-log-3-t9fm2k)).
- Lines confirmed under a placeholder identity are sent again under the real one ([`REQ-LOG-4-W5XR7Q` (Every line says where it came from)](../../../../../specification/runtime/log-collection.md#req-log-4-w5xr7q)).
- An unconfirmed send is sent again and never counted as stored ([`REQ-LOG-5-ST6S0G` (Sending twice does not store twice)](../../../../../specification/runtime/log-collection.md#req-log-5-st6s0g)).
- Every body names the thread, the store and the sequence range ([`REQ-LOG-6-Q8KY4N` (One run's logs never overwrite another's)](../../../../../specification/runtime/log-collection.md#req-log-6-q8ky4n)).
...
| --------------------------------------------------------------------------------------------- | --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- |
| [`REQ-LOG-2-N6BJ3D`](../../../../../specification/runtime/log-collection.md#req-log-2-n6bj3d) | Covered               | **Here:** `postDelta` resolves ok with the count on a 2xx and failed otherwise. **Other files:** [LoggerService.ts.md](../../rpc/internal/services/logger/LoggerService.ts.md) sums outcomes and adds never-answered. | None.            |
...
| [`REQ-LOG-4-W5XR7Q`](../../../../../specification/runtime/log-collection.md#req-log-4-w5xr7q) | Covered               | **Here:** the watermark resets when the identity key changes. **Other files:** [Logger.ts.md](./Logger.ts.md) holds the context by reference.                                                                         | None.            |
