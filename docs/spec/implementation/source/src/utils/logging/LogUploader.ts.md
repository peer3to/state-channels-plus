...
- **Identity and delta are read together, after the jitter sleep.** A channel or participant set during the sleep files that batch; the cursor is only read before the sleep (to skip an idle realm without paying it) and reset for a changed key next to the delta, so nothing is filed under the old bucket and sent again under the new one ([`REQ-LOG-4-W5XR7Q`](../../../../../specification/runtime/log-collection.md#req-log-4-w5xr7q)).
