...
- **A collection floods outward and answers back.** A thread that receives a request forwards it on
  ...
  seen-sets are needed for that. The answer is the reply of the `flush` call, correlated by the
  RPC core; the bus keeps no ids of its own.
...
- **Every request is its own round; only this realm's upload is shared.** A request forwards on
  every port but its own source and awaits this realm's upload, which the uploader's depth-one
  queue coalesces: a request arriving during an upload is answered by one that starts after it,
  never by the one already running. No handler waits on another realm's _answer_, so two realms
  originating at once cannot deadlock ([`INV-LOG-2-C7KZ9M`](../../../../../specification/runtime/log-collection.md#inv-log-2-c7kz9m)); no round is
  shared between askers, so each asker's count covers the whole tree
...
| Aspect       | Contents                                                                                                                                                                                       |
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Inputs       | Root loggers registered by `createLogger`; links registered on the loggers by the owners of worker ports; `flush` and `contextUpdate` calls arriving from neighbouring threads.                |
...
| Owned state  | This thread's root loggers; its connections and which logger each belongs to; which loggers follow another's session. No round state: a request is answered by the promise of its own fan-out. |
...
| ----------------------------------------------------------------------------------------------- | --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| [`INV-LOG-1-P4WT6R`](../../../../../specification/runtime/log-collection.md#inv-log-1-p4wt6r)   | Covered               | **Here:** forwards on every connection but the one a request arrived on, and sums its own upload with the answers. **Other files:** the port owners register the links; [rpc/logControl/LogControlService.ts.md](./rpc/logControl/LogControlService.ts.md) hands an inbound call to the bus that holds the link; the transports add the connections. | None.                                                                                |
...
| [`REQ-LOG-2-N6BJ3D`](../../../../../specification/runtime/log-collection.md#req-log-2-n6bj3d)   | Covered               | **Here:** per-thread outcomes are summed into the totals returned to the caller; every request runs its own fan-out that skips only its source, so each asker's count covers the whole tree. **Other files:** the uploader classifies its own result.                                                                                                | None.                                                                                |
...
| [`REQ-LOG-8-B7VN3J`](../../../../../specification/runtime/log-collection.md#req-log-8-b7vn3j)   | Covered               | **Here:** imports nothing platform-specific; a same-thread pair is joined without a link. **Other files:** the port owners supply the links.                                                                                                                                                                                                         | Browser paths are written but not yet executed.                                      |
...
| <a id="unit-test-logbus-2-m0271x"></a>`UNIT-TEST-LOGBUS-2-M0271X` | Two collections at once both finish.                                | Two joined threads start one simultaneously.                                                  | Neither reports a thread as never answering; both finish under the limit.                       | <a id="unit-test-logbus-2-m0271x.p1"></a>`UNIT-TEST-LOGBUS-2-M0271X.P1` — two realms starting at once both finish; <a id="unit-test-logbus-2-m0271x.p2"></a>`UNIT-TEST-LOGBUS-2-M0271X.P2` — concurrent requests share one in-flight upload and one behind it; <a id="unit-test-logbus-2-m0271x.p3"></a>`UNIT-TEST-LOGBUS-2-M0271X.P3` — a request arriving mid-upload is answered after a later upload; <a id="unit-test-logbus-2-m0271x.p5"></a>`UNIT-TEST-LOGBUS-2-M0271X.P5` — two children asking during a hub round are each told the whole tree                                                                                                                                                                                                                                 |
