...
1. **One reporting loop over a sample source.** The real source wraps the perf_hooks delay histogram and event-loop utilization, and on Linux the calling thread's scheduler counters read through [kernelCounters.ts](./kernelCounters.ts.md) (CPU time and run-queue wait per interval, so a stalled interval shows whether the thread worked, waited for a CPU, or blocked); the main thread additionally reads the host-wide counters once per interval (`/proc/stat` busy and steal share, `/proc/pressure/cpu` stall time, the cgroup's `cpu.stat` throttling), so a run-queue wait can be attributed to a burst, hypervisor steal or a quota. A test injects a scripted source. The loop is unchanged: warn or verbose per sample, the `##E2E_TIMING##` peak marker under the test-only config, and past the threshold one throw with the unchanged message text and a typed `eventLoopDelay` (`EventLoopDelayDetails`). The monitor stops itself before it throws, so a later tick reports nothing. `onStarted` fires once the interval is installed.
