...
1. **All protocol timers in one registry** so disposal can settle every pending timer exactly once. A
   task body that throws is reported through the injected logger rather than the console ([#L54](../../../../../../src/utils/TimeoutManager.ts#L54)), so the
   failure reaches the collected log pipeline like every other runtime diagnostic.
...
| ------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [TimeoutManager.ts](../../../../../../src/utils/TimeoutManager.ts) | [`REQ-RUNTIME-3-VQXW59`](../../../../specification/runtime/execution.md#req-runtime-3-vqxw59), [`REQ-SDK-ARCH-2-QBZAT8`](../../../../specification/runtime/sdk.md#req-sdk-arch-2-qbzat8) , [`REQ-LIF-10-QR8NQ9`](../../../../specification/settlement/lifecycle.md#req-lif-10-qr8nq9) |
...
- `cancelAllTasks()` cancels pending timeouts and drains running tasks without marking the manager disposed ([#L91](../../../../../../src/utils/TimeoutManager.ts#L91)), and `dispose()` is that same body behind the terminal flag ([#L81](../../../../../../src/utils/TimeoutManager.ts#L81)) — one implementation, two lifecycles. The drain is bounded by `TASK_DRAIN_TIMEOUT_MS` ([#L4](../../../../../../src/utils/TimeoutManager.ts#L4)); the race's deadline timer is cleared once the drain settles ([#L112](../../../../../../src/utils/TimeoutManager.ts#L112)), so a completed drain leaves no stray timer behind. A timed-out drain no longer just logs and continues: `cancelAllTasks()` returns whether it finished — `false` when a task outlived the bound ([#L132](../../../../../../src/utils/TimeoutManager.ts#L132)), `true` otherwise ([#L136](../../../../../../src/utils/TimeoutManager.ts#L136)) — and the still-running task is deliberately left in `runningTasks` so a later disposal can wait for it. The channel-reset caller turns a `false` into a failed release, the same policy the chain-log drain already had, because a task still running for the channel left can write into the next one ([`REQ-LIF-10-QR8NQ9` (Runtime departure and channel reuse)](../../../../specification/settlement/lifecycle.md#req-lif-10-qr8nq9)). Callers must stop their own producers first, because a task still running here may schedule another; the channel-reset caller does exactly that. Contributes to [`REQ-SDK-ARCH-2-QBZAT8` (Ordered lifecycle)](../../../../specification/runtime/sdk.md#req-sdk-arch-2-qbzat8).
...
| [`REQ-SDK-ARCH-2-QBZAT8`](../../../../specification/runtime/sdk.md#req-sdk-arch-2-qbzat8)     | Covered               | **Here:** one body for the non-terminal `cancelAllTasks()` and terminal `dispose()` ([#L91](../../../../../../src/utils/TimeoutManager.ts#L91), [#L81](../../../../../../src/utils/TimeoutManager.ts#L81)), each pending task's `onCancel` run exactly once and isolated by `try`/`catch` ([#L97](../../../../../../src/utils/TimeoutManager.ts#L97)), an owner's explicit `cancelTask` excluded ([#L78](../../../../../../src/utils/TimeoutManager.ts#L78)), and a drain outcome reported to the caller rather than only logged ([#L132](../../../../../../src/utils/TimeoutManager.ts#L132), [#L136](../../../../../../src/utils/TimeoutManager.ts#L136)). **Other files:** [StateManager](../stateManager/StateManager.ts.md) calls `cancelAllTasks()` at the scheduled-work step of the reset and throws on a `false`; [JoinChannelService](../rpc/network/services/joinChannel/JoinChannelService.ts.md) supplies the handler that fails a pending join wait. | This file settles the timers only; whether a given waiter registers a handler is the scheduler's decision. |
| [`REQ-LIF-10-QR8NQ9`](../../../../specification/settlement/lifecycle.md#req-lif-10-qr8nq9)    | Covered               | **Here:** the channel-reset drain reports whether it finished, so a scheduled task that outlived its bound is visible to the caller ([#L132](../../../../../../src/utils/TimeoutManager.ts#L132)) instead of being logged and forgotten while the runtime is handed on; the task is left registered ([#L132](../../../../../../src/utils/TimeoutManager.ts#L132)) so a later disposal can still wait for it. **Other files:** [StateManager](../stateManager/StateManager.ts.md) turns the `false` into a failed release, and [LeaveChannelService](../stateManager/membership/LeaveChannelService.ts.md) turns that into a retired runtime and a rejected leave.                                                                                                                                                                                                                                                                                                  | None; this row covers the drain-outcome contribution only.                                                 |
