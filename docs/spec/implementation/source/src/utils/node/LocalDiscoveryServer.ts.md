...
6. **Topic membership owns replacement dialing.** One active dial and one bounded-backoff retry chain are
   ...
   transport leaves the pending set and remains under channel ownership. The secondary-dial takeover of
   decision 10 is session-owned in the same way: it is scheduled through the session's timer set, so `leave`
   cancels it and a replaced session never runs it.
...
9. **The local backend mirrors the production swarm's ban behavior, or tests prove the wrong thing.**
   ...
   dial when the peer is blacklisted or reconnect-banned ([#L1067](../../../../../../../src/utils/node/LocalDiscoveryServer.ts#L1067)) and logs the skip.
...
10. **The non-primary side takes the dial over when the primary's loop stops.** Only the primary
    dialer of a pair runs a retry loop here, so when its own local policy stops that loop — the
    session reconnect suspension it placed, which is one-sided and which the suspended peer is never
    told about — the pair is left with no dialer at all and connectivity vanishes. Hyperswarm never
    does that: a ban stops the banner's own dials while the banned peer keeps dialing and is refused
    at admission. Every inbound-accepted `LocalTransport` therefore registers
    [`scheduleSecondaryDial`](../../../../../../../src/utils/node/LocalDiscoveryServer.ts#L1038) on
    `onClosed` ([#L696](../../../../../../../src/utils/node/LocalDiscoveryServer.ts#L696)), which
    dials from this side after
    [`SECONDARY_DIAL_TAKEOVER_MS`](../../../../../../../src/utils/node/LocalDiscoveryServer.ts#L22).
    The window is longer than the primary's whole backoff ramp, so a primary that is still dialing
    has reconnected by then and the takeover skips as `peer-connected`; a peer this side excluded
    skips as `blacklisted`. The port comes from
    [`announcedPeerPorts`](../../../../../../../src/utils/node/LocalDiscoveryServer.ts#L45), which
    [`handlePeerAnnouncement`](../../../../../../../src/utils/node/LocalDiscoveryServer.ts#L975)
    records for every matching announcer, primary or not, because the non-primary side has no other
    source for it
...
| Owned state  | Registry and peer servers, active sockets, topic sessions, pending dials, active-dial keys, retry counters/timers, per-session announced peer ports, and the cleanup gate. |
...
- Recreation is pair-level, not one-sided: when the canonical dialer stops its loop because of a
  suspension it placed, the side that accepted the connection takes the dial over after a fixed
  delay, so a one-sided suspension never leaves the pair with no side dialing. The suspending side
  still refuses the resulting attempt at the handshake rather than by silence.
...
| [`REQ-LOBBY-9-N894C0`](../../../../../specification/peer-communication/lobby-matching.md#req-lobby-9-n894c0)                                                                                                  | Covered               | **Here:** an active topic redials an eligible closed peer through one canonical bounded-backoff chain, rejects blacklisted endpoints, and cancels session timers before leave closes registry/listener ownership. The [secondary-dial takeover](../../../../../../../src/utils/node/LocalDiscoveryServer.ts#L1038), armed on every inbound-accepted transport ([#L696](../../../../../../../src/utils/node/LocalDiscoveryServer.ts#L696)), keeps that obligation pair-level when the canonical dialer's own suspension stops its loop. Accepted channel sockets remain separate. **Other files:** [ProfileManager](../../ProfileManager.ts.md) holds the suspension the takeover's attempt is refused under, and [InitHandshakeService](../../rpc/services/initHandshake/InitHandshakeService.ts.md) performs that refusal without an exclusion. | None.                                                                                         |
| [`REQ-RUNTIME-4-B0N70Y`](../../../../../specification/runtime/execution.md#req-runtime-4-b0n70y)                                                                                                              | Partial               | **Here:** the dial gate refuses a blacklisted or reconnect-banned peer ([#L1067](../../../../../../../src/utils/node/LocalDiscoveryServer.ts#L1067)), matching the production swarm's refusal to dial banned peer info, so ban semantics are observationally equal on both backends. The [secondary-dial takeover](../../../../../../../src/utils/node/LocalDiscoveryServer.ts#L1038) completes that equivalence on the pair: in the swarm a ban stops only the banner's dials while the banned peer keeps dialing, and here the accepting side picks the dial up when the banning side's loop stops. **Other files:** [ProfileManager](../../ProfileManager.ts.md) owns both ban states.                                                                                                                                                        | Only the dial side is mirrored; the swarm's internal ban bookkeeping has no local equivalent. |
