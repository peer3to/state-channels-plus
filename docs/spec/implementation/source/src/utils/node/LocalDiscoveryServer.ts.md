...
The process-wide discovery logger owns an independent store attached to the provided logger’s service, outside an individual state-manager subtree. Rebinding disposes the previous discovery logger. This preserves later discovery cleanup after an inline state manager is disposed.
...
3. **Cleanup closes admission immediately.** Once `_cleanupRequested` is set, timers stop retrying. An inbound client-ready
   frame is also rejected when its own manager is disposed, even while other runtimes
   keep process-wide discovery active. The socket is closed without acknowledgement, transport construction, or
...
Concurrent cleanup callers share one active promise, including socket settlement and logger disposal. The promise is cleared after settlement so a later server lifetime can be cleaned up independently.
...
- An eligible peer transport can be recreated while the topic remains observed. Session identity,
  canonical dial ownership, one-active-dial state, a one-second floor under capped exponential backoff, and the existing blacklist
  check bound that lifecycle. Topic leave cancels every session-owned retry.
- Reconnect spacing belongs to the network emulation, not the SDK. The client-ready frame carries the
  dialer's address, and the accepting peer server holds an inbound socket from a peer whose previous
  accepted socket closed less than one second ago until that cooldown has passed, then acknowledges it
  and starts the handshake as usual. The SDK admission layer never sees the delay, just as a real
  network adds its own reconnect latency (routing, hole punching). Nothing is refused or blacklisted;
  session leave or cleanup cancels the hold and closes the socket.
...
Concurrent cleanup callers share the active socket-and-logger cleanup promise. The accepted-socket lifetime case checks this identity before awaiting settlement.
