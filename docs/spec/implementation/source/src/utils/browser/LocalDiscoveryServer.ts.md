...
4. **`redialPeer` exists for surface parity and does nothing here.**
   [`redialPeer`](../../../../../../../src/utils/browser/LocalDiscoveryServer.ts#L112) is the
   browser half of the platform-aliased entry point `P2PManager.allowReconnect` calls when it lifts
   a reconnect suspension. The relay hub owns pairing on this platform, so a suspension never stops
   a local dial loop and there is nothing to resume; the no-op keeps both builds compiling against
   one signature instead of putting a platform check at the call site
...
| [`REQ-LOBBY-9-N894C0`](../../../../../specification/peer-communication/lobby-matching.md#req-lobby-9-n894c0) | Covered               | **Here:** leave removes the owning manager's topic metadata but keeps the paired relay transport alive for channel traffic; runtime cleanup closes it. [`redialPeer`](../../../../../../../src/utils/browser/LocalDiscoveryServer.ts#L112) is a no-op because the relay hub owns pairing here, so no suspension can stop a local dial loop. **Other files:** [P2PManager](../../P2PManager.ts.md) makes the platform-aliased call on a lift; the [node adapter](../node/LocalDiscoveryServer.ts.md) is the one that dials. | None.            |
