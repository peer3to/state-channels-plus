...
2. **Removal is transport-specific.** Disconnect closes and removes the connection while the
   identity profile retains its last transport reference; retiring an older transport closes only
   that object and keeps a newer replacement live.
...
4. **Current transport controls fallback release.** A stale WebRTC close is inert, and explicit
   blacklist remains banned.
...
| [`REQ-UPG-4-M2XDBA`](../../../specification/peer-communication/transport-upgrade.md#req-upg-4-m2xdba) | Covered               | **Here:** immediate profile registration, final authenticated fallback admission, current-transport guard, fallback release, and explicit-blacklist precedence. **Other files:** [PeerProfile](./PeerProfile.ts.md) stores the handle before and after authentication; [HolepunchTransport](./transport/HolepunchTransport.ts.md) supplies it. | Hyperswarm's internal ban enforcement is outside the repository harness; application admission no longer relies on it alone. |
