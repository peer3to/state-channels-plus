...
the recorded blacklist on the profile, the session suspension set, and the session retry counters.
...
9. **Suspension is manager state, the blacklist is profile state.** A suspension is a set of
   checksummed addresses on the manager and writes nothing to the profile, so it dies with the
   session and a fresh manager holding the same profile does not know it; the blacklist is a verdict
   recorded on the profile and travels with it. Because the set is independent of any profile, an
   address barred on a handle that never authenticated is still refused when it dials back on a
   fresh one. Suspension has no release path of its own; it only keeps `unblacklistPeer` and the
   upgrade-ban release from lifting a ban it did not place.
10. **The retry counter is per peer, not per check.** One counter keyed by normalized address serves
    every call site that states the bounded tier, so a peer cannot spread its allowance over
    different checks. Two call sites naming different bounds therefore still draw on the same
    counter, and the counter is never reset inside a session.
...
| Owned state  | Transport/profile indexes, identity blacklist state, the session suspension set, and the session retry counters. |
...
- Normalized-address keying; churn-surviving exclusion ([`REQ-AUTH-4-JWCF71` (Penalty requires proof and an attributable fault)](../../../specification/peer-communication/handshake.md#req-auth-4-jwcf71) consequence store). The store records only what a caller's stated policy asked for; it takes no view of its own on whether a failure was attributable.
