...
and excluded with a recorded verdict; a waiter that simply expires counts one close against the peer
instead, and the third such expiry suspends it for the session. A frame dispatched after its
authenticated transport closes is dropped silently.
...
3. **Waiters cannot outlive their transport or owner.** Success requires exact-transport
   ...
   so a stale waiter cannot punish a healthy replacement. An expired waiter is the node's own
   deadline rather than proven misconduct, so it states `allowRetry()`: one strike on the peer's
   shared counter, suspension for the session only at the bound, never a
   verdict, while traffic from a non-negotiating unauthenticated transport still records one
   ([DisconnectPolicy](../../../DisconnectPolicy.ts.md)).
...
| Side effects | Disconnect, counted close, or blacklist; delayed replays via `service.runRPC`. |
