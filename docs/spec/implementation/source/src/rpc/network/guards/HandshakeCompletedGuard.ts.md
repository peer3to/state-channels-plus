...
and excluded with a recorded verdict; a waiter that simply expires bars the peer for the session
instead. A frame dispatched after its authenticated transport closes is dropped silently.
...
3. **Waiters cannot outlive their transport or owner.** Success requires exact-transport
   ...
   deadline rather than proven misconduct, so it bars the peer for the session without recording a
...
| Side effects | Disconnect, session bar, or blacklist; delayed replays via `service.runRPC`. |
