...
5. **An expired deferral suspends, it does not blacklist.** The handshake simply did not finish in
   time, which is a load or clock symptom rather than proof of fault, so the transport closes and
   the peer is shut out for the runtime with no penalty recorded; an unauthenticated transport that
   was never negotiating is still blacklisted ([`REQ-AUTH-4-JWCF71` (Penalty requires proof; timing failures withdraw instead)](../../../../../../specification/peer-communication/handshake.md#req-auth-4-jwcf71)).
...
| Side effects | Disconnect with blacklist (never negotiating) or suspension (expired deferral); delayed replays via `service.runRPC`. |
...
| [`REQ-AUTH-4-JWCF71`](../../../../../../specification/peer-communication/handshake.md#req-auth-4-jwcf71) | Covered               | **Here:** the expired-deferral branch closes and suspends the transport instead of blacklisting the identity, while the never-negotiating branch keeps the blacklist. **Other files:** [P2PManager](../../../P2PManager.ts.md) owns the suspend-and-close entry point. | None. |
