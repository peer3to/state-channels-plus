...
2. **And answer nothing at all once the channel is gone.** The endpoint's dispute reads are awaits a
   ...
   `stateManager.channelGeneration` before them ([#L75](../../../../../../../../../src/rpc/network/services/isForkDisputedService/IsForkDisputedRpcMethods.ts#L75)) and throws afterwards when the generation moved
   ([#L95](../../../../../../../../../src/rpc/network/services/isForkDisputedService/IsForkDisputedRpcMethods.ts#L95)). Both effects that follow the reads belong to no channel then: recording the acknowledgement
...
3. **And answer only for the channel it serves now.** The generation fence covers a request already
   ...
   `stateManager.channelId` ([#L46](../../../../../../../../../src/rpc/network/services/isForkDisputedService/IsForkDisputedRpcMethods.ts#L46)), records no acknowledgement, closes the
   asker's connection with `DisconnectPolicy.ALLOW` ([#L54](../../../../../../../../../src/rpc/network/services/isForkDisputedService/IsForkDisputedRpcMethods.ts#L54)) and throws. The local diamond
   still holds the state of a channel the runtime left, so an answer read from it would record an
   acknowledgement, or blacklist the asker, for a channel this runtime no longer serves
   ([`REQ-LIF-10-QR8NQ9` (Runtime departure and channel reuse)](../../../../../../../specification/settlement/lifecycle.md#req-lif-10-qr8nq9)). _Engineer decision (2026-09-26):_ the mismatch
   costs a plain close and no verdict. An honest peer still on the channel left can reconnect, and a
   peer repeating such requests pays a reconnect for each. The rejected alternative, blacklisting as
   for a false dispute claim, would exclude that honest peer by identity into every later channel.
   The dispatcher still logs the endpoint's throw at error level, as for every other refusal here.
...
- No answer and no verdict for a channel the runtime has left: the generation captured before the
  dispute reads ([#L75](../../../../../../../../../src/rpc/network/services/isForkDisputedService/IsForkDisputedRpcMethods.ts#L75)) is re-read after them and the endpoint throws instead of recording or
  blacklisting ([#L95](../../../../../../../../../src/rpc/network/services/isForkDisputedService/IsForkDisputedRpcMethods.ts#L95)), so an inbound request that outlived its channel refills nothing and costs
...
- No answer and no verdict for a request naming any channel but the current one, including one that
  arrives after the runtime left the channel it asks about ([#L45](../../../../../../../../../src/rpc/network/services/isForkDisputedService/IsForkDisputedRpcMethods.ts#L45)).
...
| [`REQ-LIF-10-QR8NQ9`](../../../../../../../specification/settlement/lifecycle.md#req-lif-10-qr8nq9)                      | Covered               | **Here:** the generation is captured before the local-then-chain dispute reads ([#L75](../../../../../../../../../src/rpc/network/services/isForkDisputedService/IsForkDisputedRpcMethods.ts#L75)) and re-read after them; a moved generation throws ([#L95](../../../../../../../../../src/rpc/network/services/isForkDisputedService/IsForkDisputedRpcMethods.ts#L95)) rather than recording the acknowledgement or blacklisting the asker; a request naming any channel but the current one is refused before the reads ([#L45](../../../../../../../../../src/rpc/network/services/isForkDisputedService/IsForkDisputedRpcMethods.ts#L45)). **Other files:** [StateManager](../../../../stateManager/StateManager.ts.md) owns the generation and the `isStaleChannelWork` predicate, [IsForkDisputedService](IsForkDisputedService.ts.md) owns the acknowledgement records the reset clears and the requester-side half of the same rule, [P2PManager](../../../../P2PManager.ts.md) owns the exclusion this endpoint declines to request. | None; this row covers the responder contribution only. |
