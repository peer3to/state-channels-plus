...
> **Source:** [src/rpc/network/services/WebRTCSetup/WebRTCSetupService.ts](../../../../../../../../../src/rpc/network/services/WebRTCSetup/WebRTCSetupService.ts) > **Status:** Authored — engineer verification pending.
...
The runtime host may set its own connection factory before application use. Otherwise the service loads the native factory. Negotiation and peer interfaces remain unchanged.

## Key design decisions

1. **Identity from the session, connections keyed by address.** Signaling state binds to the authenticated peer, so payload-claimed identities are inert ([`REQ-UPG-1-MFBTZ1` (Identity-bound signaling)](../../../../../../../specification/peer-communication/transport-upgrade.md#req-upg-1-mfbtz1)).
2. **Best-effort, protocol-inert.** Every failure path is a logged ignore — nothing depends on signaling truthfulness, so silent-ignore is the correct consequence class ([`INV-UPG-1-KW2A02` (Best-effort with no protocol effect)](../../../../../../../specification/peer-communication/transport-upgrade.md#inv-upg-1-kw2a02)).
3. **A successful channel becomes an ordinary transport** that runs the full handshake before cutover ([`REQ-UPG-2-WH7BC7` (Re-authentication before cutover)](../../../../../../../specification/peer-communication/transport-upgrade.md#req-upg-2-wh7bc7)).
...
- Deterministic single initiator via the handshake tie-break ([`REQ-UPG-3-T1SRMS` (Single deterministic initiator)](../../../../../../../specification/peer-communication/transport-upgrade.md#req-upg-3-t1srms)); replacement closes the prior attempt ([`REQ-UPG-1-MFBTZ1` (Identity-bound signaling)](../../../../../../../specification/peer-communication/transport-upgrade.md#req-upg-1-mfbtz1)).
...
**[`DEF-11-JN8N6H`](../../../../../../../audit/open-findings.md#def-11-jn8n6h):** attacker-supplied ICE candidates reach the stack unfiltered — an authenticated peer can induce STUN/connectivity traffic toward arbitrary third-party hosts (reflection primitive). Fix or accept under the future rate limiter ([`OQ-6-4JPNE5` (P2P gossip rate limiting)](../../../../../../../specification/open-questions.md#oq-6-4jpne5)); tracked in [open-findings](../../../../../../../audit/open-findings.md). Connection-map entries leak if `close` is never reached (bounded by replacement, unbounded across many peers).
