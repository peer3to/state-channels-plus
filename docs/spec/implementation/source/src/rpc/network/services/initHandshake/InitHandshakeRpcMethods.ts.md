...
1. **Validate before signing, structurally.** Non-32-byte challenge or non-finite time is rejected
   before any signature exists and blacklists the sender, while an out-of-window timestamp only
   suspends it. The NaN check is load-bearing because NaN defeats window
...
4. **The skew refusal reaches the wire before the close.** A request whose timestamp is outside the
   window is answered with an error response and only then closed and suspended, so the caller reads
   the refusal and suspends symmetrically instead of redialling ([`REQ-AUTH-4-JWCF71` (Penalty requires proof; timing failures withdraw instead)](../../../../../../../specification/peer-communication/handshake.md#req-auth-4-jwcf71)).
...
| Side effects | Signing; blacklist on malformed input, suspension after an out-of-window refusal; timeout arming. |
...
| -------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [InitHandshakeRpcMethods.ts](../../../../../../../../../src/rpc/network/services/initHandshake/InitHandshakeRpcMethods.ts) | [`INV-AUTH-2-VQ6D54`](../../../../../../../specification/peer-communication/handshake.md#inv-auth-2-vq6d54), [`REQ-AUTH-1-RF901K`](../../../../../../../specification/peer-communication/handshake.md#req-auth-1-rf901k), [`REQ-AUTH-4-JWCF71`](../../../../../../../specification/peer-communication/handshake.md#req-auth-4-jwcf71), [`REQ-RPC-4-9VX0B9`](../../../../../../../specification/peer-communication/rpc.md#req-rpc-4-9vx0b9) |
...
- Clock skew carries no penalty and the refusal is delivered before the close ([`REQ-AUTH-4-JWCF71` (Penalty requires proof; timing failures withdraw instead)](../../../../../../../specification/peer-communication/handshake.md#req-auth-4-jwcf71)).
...
| [`REQ-AUTH-4-JWCF71`](../../../../../../../specification/peer-communication/handshake.md#req-auth-4-jwcf71) | Covered               | **Here:** the out-of-window branch throws so the error response is written, then a zero-delay task closes and suspends ([#L74](../../../../../../../../../src/rpc/network/services/initHandshake/InitHandshakeRpcMethods.ts#L74)); malformed input still blacklists. **Other files:** [InitHandshakeService](InitHandshakeService.ts.md) defers the close, [P2PManager](../../../../P2PManager.ts.md) performs it. | None. |
...
must be rejected before signing and blacklist the sender. An out-of-window timestamp is a timing
failure instead: the error response must be observable on the wire before the transport closes, and
the peer is suspended rather than blacklisted.
