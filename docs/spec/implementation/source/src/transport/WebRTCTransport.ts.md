...
- `onMessage` converts Uint8Array/Buffer input to UTF-8 text, preserves string input and receipt logging, then calls `router.onRpc`.

1. **Re-authentication is built into the wrapper** — signaling produced connectivity, the transport proves identity ([`REQ-UPG-2-WH7BC7` (Re-authentication before cutover)](../../../../specification/peer-communication/transport-upgrade.md#req-upg-2-wh7bc7)).
