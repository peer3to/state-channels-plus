...
2. **A disputed-fork refusal resolves the wait `false`.** `RaceConditionSnapshotUpdateDisputedFork`, raised by either adoption call, is logged once as a warning and resolves the submission `false` instead of rejecting ([#L161](../../../../../../../src/stateManager/snapshotUpdate/SnapshotUpdateService.ts#L161)). `postStateSnapshotWait` returns that value; it resolves `true` when the post landed, lost a handled race, or had nothing to post, and every other failure still rejects. The leave exit falls back to its self-removal dispute on `false` ([`REQ-ENFSNAP-4-ESP98F` (Disputed-fork freeze)](../../../../../specification/enforcement/snapshot-adoption.md#req-enfsnap-4-esp98f)). The refusal is not retried: preparation already skips posting onto a disputed fork, so it only meets a post that raced a new dispute, and the next post targets the reduced fork.
