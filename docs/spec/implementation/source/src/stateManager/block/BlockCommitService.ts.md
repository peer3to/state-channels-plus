...
6. **The authored block's calldata timer re-checks the channel generation when it fires** ([#L175](../../../../../../../src/stateManager/block/BlockCommitService.ts#L175)). Step 11 arms a
   ...
   still run for the channel the runtime just left and send a transaction for it. It captures
   `sm.channelGeneration` when it arms the timer and returns when the timer fires under a moved
   generation ([#L178](../../../../../../../src/stateManager/block/BlockCommitService.ts#L178)); the reset bumps the generation before its first await, so the
   guard is already stale by the time any of the release has run. It is fenced by the channel
   generation rather than by `isActiveFork`: a fork change inside the same channel (a reduction or a
   new-fork snapshot) still posts the author's block, as it did before channel reuse, so the guard
   changes nothing on the live path ([`REQ-LIF-10-QR8NQ9` (Runtime departure and channel reuse)](../../../../../specification/settlement/lifecycle.md#req-lif-10-qr8nq9)).
...
The authored block's calldata timer produces no chain submission for a channel the runtime has left:
the scheduled closure compares the channel generation captured when it was armed before posting ([#L178](../../../../../../../src/stateManager/block/BlockCommitService.ts#L178)), so a leave that lands between
...
| [`REQ-LIF-10-QR8NQ9`](../../../../../specification/settlement/lifecycle.md#req-lif-10-qr8nq9)                        | Covered               | **Here:** the step-11 calldata timer compares the channel generation captured when it was armed ([#L178](../../../../../../../src/stateManager/block/BlockCommitService.ts#L178)), so a posting armed while the channel was held sends nothing once the runtime has left it. **Other files:** [StateManager](../StateManager.ts.md) bumps the generation before the release's first await, [TimeoutManager](../../utils/TimeoutManager.ts.md) owns the timer registry the release drains, [CalldataPostingService](../chainFallback/CalldataPostingService.ts.md) owns the submission this guard declines to start.                                                            | None; this row covers the scheduled-posting contribution only.                                       |
...
| <a id="unit-test-block-commit-service-2-apafnv"></a>`UNIT-TEST-BLOCK-COMMIT-SERVICE-2-APAFNV` | Authored-block calldata timer | Capture the step-11 closure as the commit path schedules it for a block this runtime authored, release the channel, then fire the captured closure | The closure makes no posting attempt once the channel is left, counted at `maybePostBlockOnChain` rather than at the chain so a submission that is attempted and reverts still fails the case; the live side of the boundary is the ordinary posting path, exercised by every run that reaches a calldata post ([CalldataPostingService](../chainFallback/CalldataPostingService.ts.md)), so it is not restated here as a permutation of this timer | <a id="unit-test-block-commit-service-2-apafnv.p1"></a>`UNIT-TEST-BLOCK-COMMIT-SERVICE-2-APAFNV.P1` — channel released: the captured timer fired after `resetChannel()` makes no posting attempt                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
