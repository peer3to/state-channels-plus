...
2. **A losing local race is identified by committed evidence.** The candidate must name the current
   fork and an older height whose stored block author is the local signer. A current-height call by
   a non-writer still throws `NOT MY TURN`.
