...
- A replacement ends a wait only when the observed response scans for one. Every response the
  owning signer hands over does, including one it recovered from the node after a failed
  broadcast; a response that did not would stay in flight until disposal, and every bounded read
  would wait its full bound.
- Known limit: a transient RPC error when an observation's wait starts drops that transaction. The
  wait makes its first receipt and block-number reads at once, and
  [resolveReceipt](../../../../../../../src/evm/gasUsage/GasUsageRecorder.ts#L120) treats any error
  other than `CALL_EXCEPTION` as "not mined", so the transaction is not counted even if it mines.
  Later per-block checks retry on their own. One known trigger: a broadcast whose own block-number
  read failed while the node accepted the transaction; the provider's request cache returns that
  failure for up to 250 ms, and the observation starts inside that window. Only the gas usage
  report is affected; the send's result and nonce are not. Accepted for now and tracked as
  [`FIND-GAS-1-M26TMB`](../../../../../audit/open-findings.md#find-gas-1-m26tmb); retrying the wait on a
  transient error would remove it.
