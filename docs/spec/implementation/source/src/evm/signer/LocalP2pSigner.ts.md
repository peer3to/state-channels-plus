...
2. **Both join paths re-read the channel generation after the signature round trip.** Collecting the
   join confirmation is a network round trip over the peer set, and a leave can settle inside it. Each
   path captures `stateManager.channelGeneration` before `prepareJoinChannelConfirmation` and calls
   `stateManager.isStaleChannelWork()` before submitting: the top-up path at [#L237](../../../../../../../src/evm/signer/LocalP2pSigner.ts#L237) and [#L242](../../../../../../../src/evm/signer/LocalP2pSigner.ts#L242), the first-join path at [#L250](../../../../../../../src/evm/signer/LocalP2pSigner.ts#L250) and [#L257](../../../../../../../src/evm/signer/LocalP2pSigner.ts#L257). A stale
   round returns `false` rather than submitting, because the submission is an on-chain write that would
   put the departed signer back into the channel it just left — the one consequence of late work that no
   later local cleanup can undo. `false` is the existing operational-failure answer of `connectToChannel`,
...
[`REQ-TJOIN-2-MFWADG` (Separated matching and handoff)](../../../../../specification/peer-communication/targeted-channel-join.md#req-tjoin-2-mfwadg): [connectToChannel](../../../../../../../src/evm/signer/LocalP2pSigner.ts#L186) uses the separated matching topic, completes the winning lobby handoff or releases the observed-open loser, then joins the selected raw discovery key.
...
- Neither join path submits for a channel the runtime has left: the generation captured before the
  signature round trip is re-checked before `membershipService.topUpBalance` ([#L242](../../../../../../../src/evm/signer/LocalP2pSigner.ts#L242)) and before `membershipService.joinChannel` ([#L257](../../../../../../../src/evm/signer/LocalP2pSigner.ts#L257)), and a
...
| [`REQ-LIF-10-QR8NQ9`](../../../../../specification/settlement/lifecycle.md#req-lif-10-qr8nq9)                       | Covered               | **Here:** both join paths capture the channel generation before the signature round trip and abandon the submission when it moved ([#L237](../../../../../../../src/evm/signer/LocalP2pSigner.ts#L237), [#L242](../../../../../../../src/evm/signer/LocalP2pSigner.ts#L242), [#L250](../../../../../../../src/evm/signer/LocalP2pSigner.ts#L250), [#L257](../../../../../../../src/evm/signer/LocalP2pSigner.ts#L257)), returning `false`, so a departed signer is not put back on chain. **Other files:** [StateManager](../../stateManager/StateManager.ts.md) owns the generation and the `isStaleChannelWork` predicate; [JoinChannelService](../../rpc/network/services/joinChannel/JoinChannelService.ts.md) owns the collection round; [MembershipService](../../stateManager/membership/MembershipService.ts.md) owns the submission this path declines to make. | None; this row covers the membership contribution only. |
