...
3. **The log link is registered before init is awaited, and a worker that fails init is ended** — the init request is posted first and the link registered second, so the identity cast on registration lands on a link the worker created at the top of init, and a crash while the EVM is still being built already has a way up ([`INV-LOG-1-P4WT6R`](../../../../../specification/runtime/log-collection.md#inv-log-1-p4wt6r)). A rejected init disposes the executor before the error propagates, so no worker outlives its owner (`create`).
...
| ----------------------------------------------------------------------------------------------- | --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- |
| [`INV-LOG-1-P4WT6R`](../../../../../specification/runtime/log-collection.md#inv-log-1-p4wt6r)   | Covered               | **Here:** `create` posts init, registers the link on the owner logger, then awaits init; log control is the `logControl` service on both roots. **Other files:** [rpc/contractExecutor/ContractExecutorRpcMethods.ts.md](./rpc/contractExecutor/ContractExecutorRpcMethods.ts.md) registers the worker's end at the top of init; [LogFlushBus.ts.md](../../utils/logging/LogFlushBus.ts.md) runs the collection. | None.            |
| [`REQ-LOG-4-W5XR7Q`](../../../../../specification/runtime/log-collection.md#req-log-4-w5xr7q)   | Covered               | **Here:** the link is owned by the host logger, so its identity is cast on registration and on every later change. **Other files:** [LogFlushBus.ts.md](../../utils/logging/LogFlushBus.ts.md) decides how much of it the worker applies.                                                                                                                                                                        | None.            |
