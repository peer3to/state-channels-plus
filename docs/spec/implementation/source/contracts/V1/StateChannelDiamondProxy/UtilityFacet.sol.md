...
2. **Proxy-storage views, reached by delegatecall through the proxy's selector routing**
   ...
   verification/pruning. Each body is a thin wrapper over the corresponding `internal` on
...
views remain reachable at the manager address after moving off the proxy.
