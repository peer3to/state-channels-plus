...
1. **Selector routing replaced the typed forwarders, to fit EIP-170.** The proxy previously
   ...
   deployed bytes — over the 24,576-byte budget. Routing by selector costs one comparison chain
   instead of ~28 forwarder bodies and brings the proxy to 13,779 bytes
...
2. **The routing table is written as `Facet.fn.selector`, never as a literal hash.** The compiler
   derives every entry from the facet's own function type, so a signature change either updates the
   routing with it or fails the build — the table cannot silently drift from the facets
   ([#L286](../../../../../../../contracts/V1/StateChannelDiamondProxy/StateChannelManagerProxy.sol#L286)).
3. **State-changing facets are compared before the utility views.** On-chain callers pay the fewest
   comparisons; the utility views are predominantly read off-chain, where the extra comparisons cost
   nothing
   ([#L280](../../../../../../../contracts/V1/StateChannelDiamondProxy/StateChannelManagerProxy.sol#L280)).
...
required module and the deployable now fits the platform size limit;
...
- Constructor wiring is trusted: the deployer chooses every facet address and nothing verifies that
  the code at that address is the expected module (or is code at all) — a deployment commitment, not
  a runtime check.
...
- `StateChannelManagerInterface` is not compiler-checked against the routing table. Nothing
  ...
  is caught only by review and by the routing test
  ([`UNIT-TEST-MANAGER-PROXY-2-KJRMB8.P21`](StateChannelManagerProxy.sol.md#unit-test-manager-proxy-2-kjrmb8.p21)).
...
- **No rejection of an unowned selector.** [`REQ-CONTRACT-ARCH-5-QT17P1`](../../../../../specification/enforcement/contracts.md#req-contract-arch-5-qt17p1) requires that an
  ...
  ([#L355](../../../../../../../contracts/V1/StateChannelDiamondProxy/StateChannelManagerProxy.sol#L355)).
- **No deployment-time module verification.** The constructor accepts any address, including the
  zero address, for every facet ([#L33](../../../../../../../contracts/V1/StateChannelDiamondProxy/StateChannelManagerProxy.sol#L33)).
...
| [`REQ-CONTRACT-ARCH-4-FZ3CJE`](../../../../../specification/enforcement/contracts.md#req-contract-arch-4-fz3cje) | Partial               | **Here:** the constructor names every required module ([#L33](../../../../../../../contracts/V1/StateChannelDiamondProxy/StateChannelManagerProxy.sol#L33)), the addresses are fixed for the deployment's life so an open channel's semantics cannot change under it, and the deployable measures 13,779 bytes against the 24,576-byte limit. **Other files:** [architecture view](../../../../views/architecture/contracts/architecture.md) §3 holds the per-contract measurements; [UtilityFacetInterface](./UtilityFacetInterface.sol.md) gives the helper module a compile-time type. | Missing/incompatible modules are not rejected: any address, including zero, is accepted and never code-checked. No build or deployment gate enforces the size limit either (architecture view [`REQ-CON-2-CBVFV9`](../../../../views/architecture/contracts/architecture.md#req-con-2-cbvfv9)). |
