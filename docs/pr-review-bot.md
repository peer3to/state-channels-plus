...
- **Claude:** `claude` must be on `PATH` and logged in with a claude.ai subscription (`claude auth status` shows `authMethod: claude.ai`). The worker passes only `PATH`, `HOME` and optional `CLAUDE_CONFIG_DIR`, never an API key. Each review runs `claude -p` with its built-in Bash, Read, Edit and Write tools plus the worker source tools (served in-band over the CLI's stream-JSON protocol), and no settings files, skills, plugins or user MCP servers. It runs with `--permission-mode dontAsk --permission-prompts none`, so anything not allowed is denied, never asked about. A session exposing any other tool, or any CLI permission request, stops the review. Claude Code's sandbox needs `bubblewrap` and `socat`; as root it runs in its weaker nested mode, which keeps the file and network rules below. An unknown model fails at the first turn. Claude Code keeps its default tool-result caps (25,000 tokens, about 50 KB); above them it would give the model only a preview of a file it cannot read. Every GitHub page and source file is also on disk in the workspace, so the adapter delivers results up to 30,000 characters and replaces a larger one with a recoverable `RESULT_TOO_LARGE` tool error. For a GitHub page the error names the page's file in the `github` folder; for source it points to a narrower read or the file under `source`. The model then reads just the parts it needs with its own tools, which keeps its context small.
