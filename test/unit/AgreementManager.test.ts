...
describe("Unit: AgreementManager", function () {
    ...
    describe("getStateProof / tryGetStateProof", function () {
        ...
        it("proof requested below a participant join → tops out at the requested height", async function () {
            ...
            await spectator.p2pInstance.p2pSigner.joinChannel(confirmation);
    ...
    describe("getSnapshotFromMilestone", function () {
        ...
        it("multi-milestone proof → distinct snapshot per milestone; finalized + latest select the LAST", async function () {
            const h = TestSession.getHarness();
            await h.scenario.setupTwoLeaversAcrossMilestones();
            ...
            // getSnapshotFromMilestone returns a distinct snapshot per milestone
        ...
        it.skip("partial dispute window (commitment on-chain, struct not yet stored) → reduceLocally discards, not throws", async function () {
            ...
            await waitFor(
                async () =>
                    h.execOnHost(
                        ...
                        async (sm, a) => {
                            const { isExpired } =
                                await sm.isKillPeriodExpiredCached(a.forkId);
                            return isExpired;
                        },
                        { forkId }
                    ),
            ...
            const outcome = await h.execOnHost(
                ...
                async (sm, args) => {
                    ...
                    try {
                        const r = await sm.reduceLocally(args.forkId);
