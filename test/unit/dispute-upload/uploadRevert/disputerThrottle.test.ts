...
describe("dispute-upload / uploadRevert / disputerThrottle", function () {
    const evidenceTime = 2;

    describe("disputer already throttled; opens NEW window", function () {
        it(
            "second postDispute from same disputer within evidenceTime → dispute upload fails → ErrorDisputeThrottled",
            covers(
                {
                    uploadRevert: "ErrorDisputeThrottled"
                },
                async function () {
                    ...
                    try {
                        ...
                        expect(customError, "expected ErrorDisputeThrottled").to
                            .not.be.null;
                        expect(
        ...
        it(
            "second postDispute from same disputer after evidenceTime → dispute upload succeeds",
            covers(
                {
                    uploadRevert: "clean"
                },
    ...
    describe("disputer already throttled; JOINS existing window opened by another peer", function () {
        it(
            "postDispute reuses dispute.input.forkId from another peer's open window within evidenceTime → dispute upload fails → ErrorDisputeThrottled",
            covers(
                {
                    uploadRevert: "ErrorDisputeThrottled"
                },
