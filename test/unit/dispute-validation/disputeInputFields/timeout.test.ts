...
describe("dispute-validation / disputeInputFields / timeout", function () {
    it(
        "dispute.input.timeout.blockHeight != stateProof.latest + 1 → TimeoutNotLinkedToLatestState",
        covers(
            {
                blockHeight: "mismatch",
    ...
    describe("TimeoutTooEarly", function () {
        ...
        it(
            ...
            covers(
                {
                    minTimeStamp: "after-wait",
                    bogusKillProof: "TimeoutTooEarly"
                },
                async function () {
                    const h = TestSession.getHarness();
                    await h.scenario.preDisputeSetup();
    ...
    it(
        "leaver does not dispute a timeout after leaving the channel",
        covers(
            {
                disputer: "non-participant"
            },
            async function () {
                const h = TestSession.getHarness();
                // peer 0 leaves the channel but stays on-chain (leave snapshot suppressed),
        ...
        covers(
            {
                ...
                participantSignatureOnPreviousBlock: "present",
                proofType: "TimeoutParticipantNotNext"
            },
            async function () {
                const h = TestSession.getHarness();
                ...
                h.tamper.stubConstructDispute(
                    ...
                    (dispute, _sm, args) => {
                        ...
                        if (
                            String(previousBlockProducer) === args.zeroAddress
                        ) {
                            throw new Error(
