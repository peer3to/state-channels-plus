...
describe("dispute-validation / disputeInputFields / onChainSlashes", function () {
    it(
        "dispute.input.onChainSlashes includes address not slashed on-chain → DisputeOnChainSlashesNotSubset",
        covers(
            {
                onChainSlashes: "non-subset",
                proofType: "DisputeOnChainSlashesNotSubset",
    ...
    it(
        "dispute.input.onChainSlashes contains address not in latestStateSnapshot participants → InvalidDisputeReason",
        covers(
            {
                onChainSlashes: "not-in-snapshot",
    ...
    it(
        "dispute.input.onChainSlashes has > maxSlashCount distinct addresses → reduce must not OOB-panic, both offenders slashed",
        covers(
            {
                onChainSlashes: "oversized",
                postedAuditingData: "false"
            },
            async function () {
                const h = TestSession.getHarness();
