...
describe("dispute-validation / stateProof / Case 3 (signedBlocks-only)", function () {
    describe("stateProof.signedBlocks[0].previousBlockHash = random (wrong genesis anchor)", function () {
        it(
            "height 0 first block with wrong genesis link → DisputeInvalidBlockInStateProofApplyFraudProof",
            covers(
                {
                    stateProof: "first-block-wrong-anchor",
                    proofType: "DisputeInvalidBlockInStateProofApplyFraudProof",
                ...
                async function () {
                    ...
                    h.tamper.stubConstructDispute(3, async (dispute, sm) => {
                        ...
                        for (
                            let i = 1;
                            i < stateProof.signedBlocks.length;
                            i++
                        ) {
                            await d.rewriteSignedBlockAtIndex(
                                dispute,
                                i,
                                (bs) => ({
                                    ...bs,
    ...
    describe("stateProof.signedBlocks[0].transaction.header.transactionCnt != 0", function () {
        it(
            "first signedBlock height is not 0 → DisputeInvalidStateProof",
            covers(
                {
                    stateProof: "first-block-nonzero-height",
                    ...
                    h.tamper.stubConstructDispute(3, async (dispute, sm) => {
                        ...
                        await d.rewriteSignedBlockAtIndex(dispute, 0, (bs) =>
                            d.blockStructWithTransactionHeader(bs, {
                                transactionCnt:
                                    BigInt(
                                        bs.transaction.header.transactionCnt
                                    ) + 1n
                            })
                        );
                    });
    ...
    describe("stateProof.signedBlocks[-1].encodedBlock.stateSnapshotHash = ZeroHash (stateSnapshotHash mismatch)", function () {
        it(
            "stateSnapshotHash = ZeroHash → DisputeInvalidBlockInStateProofApplyFraudProof",
            covers(
                {
                    proofType: "DisputeInvalidBlockInStateProofApplyFraudProof",
                    carrier: "signedblocks",
    ...
    describe("stateProof.signedBlocks[-1].encodedBlock.messageBlocks injected with forged inbound message", function () {
        it(
            "messageBlocks injected with forged inbound message → DisputeInvalidBlockInStateProofApplyFraudProof",
            covers(
                {
                    proofType: "DisputeInvalidBlockInStateProofApplyFraudProof",
                    carrier: "signedblocks",
    ...
    describe("stateProof.signedBlocks[1].previousBlockHash = random (inter-block linkage break)", function () {
        ...
        it(
            "signedBlocks[1].previousBlockHash = random → DisputeInvalidStateProof",
            covers(
                {
                    proofType: "DisputeInvalidStateProof",
                    carrier: "signedblocks",
