...
describe("dispute-validation / stateProof / structural rules", function () {
    describe("stateProof.milestones and stateProof.signedBlocks are mutually exclusive", function () {
        it(
            "stateProof.milestones.length > 0 AND stateProof.signedBlocks.length > 0 → DisputeInvalidStateProof",
            covers(
                {
                    stateProof: "both-arrays-present",
                ...
                async function () {
                    ...
                    h.tamper.stubConstructDispute(
                        ...
                        (d) => {
                            ...
                            const src =
                                d.input.stateProof.milestones[0]
    ...
    describe("each milestone must have at least one blockConfirmation", function () {
        it(
            "stateProof.milestones[0].blockConfirmations = [] → DisputeInvalidStateProof",
            covers(
                {
                    stateProof: "empty-last-milestone",
                    proofType: "DisputeInvalidStateProof",
                    postedAuditingData: "true"
                },
                async function () {
