 ...
 */
describe("timeouts / detection", function () {
    describe("Basic Timeout Scenarios", function () {
        it(
            "should handle timeout when next peer to write does not author a block",
            covers(
                {
                    outcome: "normal"
                },
        ...
        it(
            "should demonstrate timeout creates disputes",
            covers(
                {
                    outcome: "normal"
                },
    ...
    describe("Network Disconnection Timeouts", function () {
        it(
            "should handle timeout when non-author peer disconnects (calldata posting)",
            covers(
                {
                    outcome: "no-timeout"
                },
                async function () {
                    ...
                    await h.transition.advanceState({
                        count: 2,
                        waitForPeers: [0, 1]
                    }); // Peers 0 and 1 write (peer 2 disconnected)
        ...
        it(
            "should handle timeout when author peer disconnects",
            covers(
                {
                    outcome: "normal"
                },
    ...
    describe("Forced Timeout (Junk Calldata)", function () {
        it(
            "should create forced timeout when peer posts junk calldata that is rejected",
            covers(
                {
                    outcome: "forced"
                },
                async function () {
                    ...
                    h.assert.storage.storedTimeout({
        ...
        it(
            "should handle timeout when previous peer posted junk calldata and next peer doesn't author block",
            covers(
                {
                    outcome: "normal",
                    previousCalldataRace: "extra-time-granted"
                },
                async function () {
                    const h = TestSession.getHarness();
    ...
    describe("Network Liveness", function () {
        it(
            "should maintain liveness when peer disconnects mid-transaction",
            covers(
                {
                    outcome: "no-timeout"
                },
