...
import {
    BlockPersistenceKey,
    IStoragePersistence
} from "@/storage/persistence/IStoragePersistence";

const sig = () => ethers.hexlify(ethers.randomBytes(65));

/** In-memory test double for IStoragePersistence, with introspection + failure injection. */
class FakePersistence implements IStoragePersistence {
    public readonly persisted = new Map<
        Hash,
        { encodedBlock: string; key: BlockPersistenceKey }
    >();
    public readonly deletedHashes: Hash[] = [];
    public readonly errors: unknown[] = [];
    public throwOnPersist = false;

    persistBlock(encodedBlock: string, key: BlockPersistenceKey): void {
        if (this.throwOnPersist) {
            throw new Error("persistBlock failed");
        }
        this.persisted.set(key.hash, { encodedBlock, key });
    }

    deleteBlock(key: { hash: Hash }): void {
        this.deletedHashes.push(key.hash);
        this.persisted.delete(key.hash);
    }

    async *loadAll(): AsyncIterable<{ encodedBlock: string }> {
        for (const { encodedBlock } of this.persisted.values()) {
            yield { encodedBlock };
        }
    }
...
describe("Persistence", () => {
    ...
    it("defaults to an in-memory persistence and behaves identically to today when unhydrated", () => {
        const storage = new BlockStorage();
        const block = blockAt(0);
        const hash = storage.storeBlock(block);
        expect(storage.getBlock(hash!)?.equals(block)).to.be.true;
    });

    it("hydrate() populates maps from the port and rebuilds forkIdToMaxHeightMap", async () => {
        const persistence = new FakePersistence();
        const writer = new BlockStorage(persistence);
        for (const height of [0, 5, 10]) {
            writer.storeBlock(blockAt(height));
        }

        const reader = new BlockStorage(persistence);
        expect(reader.getNextBlockHeight(forkId)).to.equal(0);

        await reader.hydrate();

        expect(reader.getLatestBlock(forkId)?.height).to.equal(10);
        expect(reader.getNextBlockHeight(forkId)).to.equal(11);
        expect(Array.from(reader.getIterator(forkId))).to.have.lengthOf(3);
    });

    it("storeBlock/insertSignature/setOnChainTimestamp persist post-mutation state, restorable via hydrate", async () => {
        const persistence = new FakePersistence();
        const writer = new BlockStorage(persistence);
        const block = blockAt(1);
        const hash = writer.storeBlock(block)!;
        expect(persistence.persisted.has(hash)).to.be.true;

        const newSig = sig();
        writer.insertSignature(newSig, hash);
        writer.setOnChainTimestamp(hash, 999);

        const reader = new BlockStorage(persistence);
        await reader.hydrate();

        const hydrated = reader.getBlock(hash);
        expect(hydrated?.confirmationSignatures.has(newSig)).to.be.true;
        expect(hydrated?.onChainTimestamp).to.equal(999);
    });

    it("deleteBlock drives persistence.deleteBlock, so hydrate no longer returns it", async () => {
        const persistence = new FakePersistence();
        const writer = new BlockStorage(persistence);
        const hash = writer.storeBlock(blockAt(2))!;

        expect(writer.deleteBlock(hash)).to.be.true;
        expect(persistence.deletedHashes).to.include(hash);
        expect(persistence.persisted.has(hash)).to.be.false;

        const reader = new BlockStorage(persistence);
        await reader.hydrate();
        expect(reader.getBlock(hash)).to.be.undefined;
    });

    it("a persistence call that throws invokes onError and does not throw into the synchronous caller", () => {
        const persistence = new FakePersistence();
        persistence.throwOnPersist = true;
        const storage = new BlockStorage(persistence);

        expect(() => storage.storeBlock(blockAt(3))).to.not.throw();
    ...
    it("hydrate() from a store truncated mid-write skips the corrupt entry and reports the highest contiguous height", async () => {
        ...
        const writer = new BlockStorage(persistence);
        const heights = [0, 1, 2];
        const hashes = heights.map(
            (height) => writer.storeBlock(blockAt(height))!
        );

        // Simulate a crash mid-write: the tip block's persisted bytes are truncated/corrupt.
        const tipHash = hashes[hashes.length - 1];
        const tipEntry = persistence.persisted.get(tipHash)!;
        persistence.persisted.set(tipHash, {
            ...tipEntry,
            encodedBlock: "0x1234"
        });
        ...
        expect(reader.getLatestBlock(forkId)?.height).to.equal(1);
        expect(reader.getNextBlockHeight(forkId)).to.equal(2);
        expect(persistence.errors).to.have.lengthOf(1);
    });
});
