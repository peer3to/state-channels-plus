...
export async function assertCustomRpcRuntimeFlow(
    runSdkInThread: boolean
): Promise<void> {
    const peers: PingPeer[] = [];
    let nodeHandle: NodeHandle | undefined;
    let discoveryHandle: DiscoveryHandle | undefined;
    let provider: ethers.JsonRpcProvider | undefined;
    try {
        if (hardhatNodeUrl) await waitForHardhatNode(hardhatNodeUrl);
        else {
        ...
        const makePeer = async (
            ...
            instance.events.on("p2pEventHooks", "onConnection", (address) => {
                if (typeof address !== "string")
                    throw new Error("Expected a serialized peer address");
                connectedTo.get(selfAddress)?.add(address);
            });
        ...
        await waitFor(
            () =>
                connectedTo.get(peer0Address)?.has(peer1Address) === true &&
                connectedTo.get(peer1Address)?.has(peer0Address) === true,
        ...
        expect(sumSelf.requester).to.equal(peer0Address);
        ...
        expect(sum0.requester).to.equal(peer0Address);
        ...
        expect(sum1.requester).to.equal(peer1Address);
        ...
        await waitFor(
            async () =>
                (
                    await peer1.hostRpc.pingService
                        .getReceivedPingNonces()
                        .request()
                ).includes("from-0"),
            TEST_PROTOCOL_TIMEOUT_MS
        );
        expect(
            await peer1.hostRpc.pingService.getReceivedPingNonces().request()
        ).to.deep.equal(["from-0"]);
        ...
        const recovered = await peer0.hostRpc.pingService
            .sum(9, 6, "after-failure")
            .request(peer1Address, { timeoutMs: TEST_PROTOCOL_TIMEOUT_MS });
        expect(recovered.sum).to.equal(15);
        expect(recovered.nonce).to.equal("after-failure");
        expect(recovered.requester).to.equal(peer0Address);
        ...
        if (!runSdkInThread) await LocalDiscoveryServer.cleanup();
        provider?.destroy();
        discoveryHandle?.stop();
        nodeHandle?.stop();
        if (nodeHandle) hardhatNodeUrl = undefined;
        if (discoveryHandle) localDiscoveryRegistryUrl = undefined;
    }
}
