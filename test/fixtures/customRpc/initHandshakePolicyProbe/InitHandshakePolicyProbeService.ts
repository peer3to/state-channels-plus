// @spec-test-coverage-ignore: worker-side support service for mapped InitHandshake disconnect-policy cases
 ...
 * transport, and its blacklist flag is exactly what the caller asserts on.
 */
type ProbePeer = {
...
export type AckTimeoutPolicyProbe = {
...
export type BlacklistedSignerPolicyProbe = {
    identityBlacklistedBefore: boolean;
    socketDestroyed: boolean;
    banCalls: boolean[];
    profileBlacklisted: boolean;
    connectionRemoved: boolean;
...
> {
     ...
     * timeout is a load/clock symptom, so the transport closes and the peer
     * keeps its profile: no blacklist, no Hyperswarm ban.
     */
    public async probeAckTimeoutAfterVerifiedAddress(): Promise<AckTimeoutPolicyProbe> {
    ...
    /**
     * An excluded identity dials back on a fresh connection and answers our
     * challenge. The exclusion must be re-applied to the arriving peer info,
     * otherwise the peer redials and re-runs the handshake forever.
     */
    public async probeBlacklistedResponseSigner(): Promise<BlacklistedSignerPolicyProbe> {
        const excluded = this.connectProbePeer();
        // Establish the exclusion on the identity, exactly as a punishment
        // elsewhere in the pipeline would: the first connection is closed and
        // the profile stays blacklisted by address.
        ...
        const identityBlacklistedBefore = this.p2pManager.isBlacklisted(
            excluded.wallet.address
        );

        // The same identity dials again: a new socket, a new peer info handle,
        // and a fresh profile that knows nothing about the exclusion.
        const returning = this.connectProbePeer(excluded.wallet);
        await this.answerHandshakeChallenge(returning);
        await this.waitForClose(returning);
        return {
            identityBlacklistedBefore,
            socketDestroyed: returning.socket.destroyed,
            banCalls: [...returning.peerInfo.banCalls],
            profileBlacklisted: returning.profile.isBlackListed,
            connectionRemoved: !this.p2pManager.openConnections.includes(
                returning.transport
            )
        };
