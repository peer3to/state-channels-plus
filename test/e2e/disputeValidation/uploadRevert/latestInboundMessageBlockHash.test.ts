...
describe("E2E: dispute validation / uploadRevert / latestInboundMessageBlockHash", function () {
    scenario(
        "dispute.input.latestInboundMessageBlockHash = ZeroHash AND lastInboundMessageBlockHeight = 0 → dispute upload succeeds (genesis anchor is always a valid ancestor)",
        {
            invariant: "no-honest-loss",
            target: [
                "DisputeInput.latestInboundMessageBlockHash:in-chain",
                "DisputeInput.lastInboundMessageBlockHeight:correct"
            ]
        },
        async function () {
