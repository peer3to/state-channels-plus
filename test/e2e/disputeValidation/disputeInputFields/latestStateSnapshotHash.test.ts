...
describe("E2E: dispute validation / disputeInputFields / latestStateSnapshotHash", function () {
    describe("no calldata", function () {
        describe("(1) stateProof empty — genesis (no milestones, no signedBlocks)", function () {
            describe("all peers are in sync", function () {
                scenario(
                    "[no calldata] dispute.input.stateProof = {} AND dispute.input.latestStateSnapshotHash = random → DisputeInvalidStateProof",
                    {
                        invariant: "attacker-pays",
                        target: [
                            "DisputeInput.latestStateSnapshotHash:mismatch",
                            "DisputeInput.stateProof:fully-empty"
                        ],
                        setup: ["calldata:absent", "auditor:synced"]
                    },
                    async function () {
        ...
        describe("(3) stateProof.milestones only — last milestone block commits to hash", function () {
            describe("all peers are in sync", function () {
                scenario(
                    "[no calldata] dispute.input.latestStateSnapshotHash = random → DisputeInvalidStateProof",
                    {
                        invariant: "attacker-pays",
                        target: "DisputeInput.latestStateSnapshotHash:mismatch",
                        setup: [
                            "calldata:absent",
                            "proof:milestones",
                            "auditor:synced"
                    ...
                    async function () {
                        ...
                        h.tamper.stubConstructDispute(1, (d, sm) => {
                            ...
                            svc.expectMilestonesOnlyStateProof(
                                d.input.stateProof
                            );
            ...
            describe("auditor peer 3 disconnected — local storage stale, pipeline still kills", function () {
                scenario(
                    "[no calldata] dispute.input.latestStateSnapshotHash = random → DisputeInvalidStateProof (killed by peer 3)",
                    {
                        invariant: "attacker-pays",
                        target: "DisputeInput.latestStateSnapshotHash:mismatch",
                        setup: [
                    ...
                    async function () {
                        ...
                        await h.network.disconnectPeer(
        ...
        describe("(2) stateProof.signedBlocks only — last signedBlock commits to hash", function () {
            ...
            describe("peers synced — auditor peer 0 has full signedBlocks chain locally", function () {
                scenario(
                    "[no calldata] dispute.input.latestStateSnapshotHash = random → DisputeInvalidStateProof (killed by peer 0)",
                    {
                        invariant: "attacker-pays",
                        target: "DisputeInput.latestStateSnapshotHash:mismatch",
                        setup: [
            ...
            describe("auditor peer 2 disconnected — local storage genesis-only, pipeline still kills", function () {
                scenario(
                    "[no calldata] dispute.input.latestStateSnapshotHash = random → DisputeInvalidStateProof (killed by peer 2)",
                    {
                        target: "DisputeInput.latestStateSnapshotHash:mismatch",
                        setup: [
                            "calldata:absent",
    ...
    describe("calldata posted", function () {
        describe("(1) stateProof empty — genesis (no milestones, no signedBlocks)", function () {
            describe("all peers are in sync", function () {
                scenario(
                    "[calldata posted] dispute.input.stateProof = {} AND dispute.input.latestStateSnapshotHash = random → DisputeInvalidStateProof",
                    {
                        target: [
                            "DisputeInput.latestStateSnapshotHash:mismatch",
                            "DisputeInput.stateProof:fully-empty"
                        ],
                        setup: ["calldata:posted", "auditor:synced"]
                    },
                    async function () {
                        const h = TestSession.getHarness();
        ...
        describe("(3) stateProof.milestones only — last milestone block commits to hash", function () {
            describe("all peers are in sync", function () {
                scenario(
                    "[calldata posted] dispute.input.latestStateSnapshotHash = random → DisputeInvalidStateProof",
                    {
                        target: "DisputeInput.latestStateSnapshotHash:mismatch",
                        setup: [
                            "calldata:posted",
            ...
            describe("peers not synced — auditor peer 1 disconnected (misses latest block)", function () {
                scenario(
                    "[calldata posted] dispute.input.latestStateSnapshotHash = random → DisputeInvalidStateProof (killed by peer 1)",
                    {
                        target: "DisputeInput.latestStateSnapshotHash:mismatch",
                        setup: [
                            "calldata:posted",
        ...
        describe("(2) stateProof.signedBlocks only — last signedBlock commits to hash", function () {
            describe("peers not synced — auditor peer 2 disconnected (calldata forced)", function () {
                scenario(
                    "[calldata posted] dispute.input.latestStateSnapshotHash = random → DisputeInvalidStateProof (killed by peer 2)",
                    {
                        target: "DisputeInput.latestStateSnapshotHash:mismatch",
                        setup: [
                            "calldata:posted",
