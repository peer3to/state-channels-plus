...
describe("E2E: dispute validation / disputeInputFields / forkId", function () {
    scenario(
        "current fork == genesis; dispute.input.forkId = random; honest peers stay on genesis",
        {
            invariant: "attacker-pays",
            target: "DisputeInput.forkId:input-only-junk",
            setup: "proof:genesis"
        },
        async function () {
            ...
            for (const p of h.getHonestPeers()) {
                const forkId = await h.control(p).query.getForkId().request();
