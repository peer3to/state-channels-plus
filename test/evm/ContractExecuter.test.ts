...
describe("EvmExecutionEnvironment", function () {
  ...
  before(async function () {
    ...
    const deployTx = await getSimpleNumberStorageDeploymentTransaction(ethers);
  ...
  it("should successfully execute a call to get a value", async function () {
    const getValueFunction = SimpleNumberStorage.interface.getFunction("getValue");
    const getValueData = SimpleNumberStorage.interface.encodeFunctionData(getValueFunction);
    ...
    const decodedValue = ethers.AbiCoder.defaultAbiCoder().decode(["uint256"], returnValue);
  ...
  it("should successfully execute a call to set a value", async function () {
    ...
    const setValue = 42n;
    const setValueFunction = SimpleNumberStorage.interface.getFunction("setValue");
    const setValueData = SimpleNumberStorage.interface.encodeFunctionData(setValueFunction, [setValue]);
  ...
  it("should successfully set state using bytes", async function () {
    ...
    const newValue = 99n;
    const encodedValue = ethers.AbiCoder.defaultAbiCoder().encode(["uint256"], [newValue]);

    const setStateFunction = SimpleNumberStorage.interface.getFunction("setState");
    const setStateData = SimpleNumberStorage.interface.encodeFunctionData(setStateFunction, [encodedValue]);
  ...
  it("should properly decode Solidity revert errors", async function () {
    ...
    const revertFunction = SimpleNumberStorage.interface.getFunction("revertWithMessage");
    const revertFunctionData = SimpleNumberStorage.interface.encodeFunctionData(
      revertFunction, [errorMessage]
    );
