...
const { check, exact, ownedPath } = require("./data");
...
class SourceTools {
    ...
    baseline = null;
    constructor(checkout, request, publicGitHub) {
    ...
    async read({ path, start = 1, count = 200 }) {
        ...
        const lines = await this.loadLines(path);
        return {
            path,
            start,
